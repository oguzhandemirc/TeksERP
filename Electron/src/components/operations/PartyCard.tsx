...
interface Props {
  ...
  branch?: {
    name: string;
    /** Müşterinin iç şube kodu = belgede "İhracat Kodu" — opsiyonel. */
    code?: string | null;
    city?: string | null;
    district?: string | null;
  } | null;
  /** Müşteri yoksa gösterilecek metin (ör. çuval → "Müşterisiz (genel stok)"). */
  emptyLabel?: string;
}

/**
 * Detay panellerinin üst kimlik kartı: müşteri (şirket) adı + kodu, altında şube
 * adı/konumu ve şube kodu. Şube yoksa yalnız müşteri satırı basılır.
 *
 * Sipariş ve çuval panelleri PAYLAŞIR — props yapısaldır (şehir/ilçe opsiyonel),
 * çünkü çuval arama satırı şubede city/district taşımaz.
 */
export function PartyCard({ customer, branch, emptyLabel = "—" }: Props) {
  const location = branch ? [branch.district, branch.city].filter(Boolean).join(" / ") : "";
      ...
      {branch && (
        <div className="grid grid-cols-2 gap-px border-t bg-border">
          <div className="bg-background px-4 py-2.5">
            ...
            <div className="mt-0.5 truncate text-sm font-medium text-foreground">{branch.name}</div>
