...
// modal yalnız müşteri kartlarıyla → satır → onay → `PATCH /customers/:id {type:"BOTH"}` → seçim forma.
...
export const convertConfirmText = (name: string) => `${name} kartının tipi ${companyTypeLabels.BOTH} olacak.`;
...
export function ConvertBackLink({ onClick }: { onClick: () => void }) {
  return (
    <p className="text-xs text-muted-foreground">
      Yalnız müşteri kartları listeleniyor; seçilen kart <b>{companyTypeLabels.BOTH}</b> olur ve tedarikçi olarak seçilir.{" "}
      <button type="button" className="underline underline-offset-2 hover:text-foreground" onClick={onClick}>
        ← {CONVERT_BACK_TEXT}
      </button>
    </p>
...
export function ConvertCustomerConfirm({ row, onCancel, onConverted }: ConfirmProps) {
  ...
  const mut = useMutation({
    mutationFn: (id: string) => customerService.update(id, { type: "BOTH" }),
    onSuccess: (res, id) => {
      void qc.invalidateQueries({ queryKey: ["customers"] });
      void qc.invalidateQueries({ queryKey: ["supplier-picker"] });
      void qc.invalidateQueries({ queryKey: ["supplier-select"] });
      toast.success(`${res.data?.name ?? "Kart"} artık ${companyTypeLabels.BOTH}`);
