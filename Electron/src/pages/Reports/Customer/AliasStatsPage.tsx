...
const columns: ColumnDef<TopRow>[] = [
  ...
  { accessorKey: "itemAliases", header: "Ürün Alias", cell: ({ getValue }) => fmtInt(getValue() as number) },
...
export function AliasStatsPage() {
  ...
  return (
    <ReportPageLayout
      ...
      description="Müşteri bazlı ürün/renk takma adlarının kullanım istatistiği."
      showDateRange={false}
    >
      <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
        <MetricCard label="Toplam Ürün Alias" value={fmtInt(s?.totalItemAliases ?? 0)} isLoading={isLoading} />
        <MetricCard label="Toplam Renk Alias" value={fmtInt(s?.totalColorAliases ?? 0)} isLoading={isLoading} />
        <MetricCard
          label="Ürün Alias'lı Müşteri"
