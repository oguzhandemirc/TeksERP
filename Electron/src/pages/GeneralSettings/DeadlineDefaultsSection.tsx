...
/**
 * Termin Varsayılanları — Genel Ayarlar'ın bir bölümü (eskiden ayrı sayfaydı).
 * Sipariş/iş emri açılışında termin boş bırakılırsa eklenecek gün sayıları.
 * Kendi Card'ını AÇMAZ — Genel Ayarlar zaten kategori kartı içine sarıyor.
 */
export function DeadlineDefaultsSection() {
  ...
  const woSaved = parseSetting(
    settings.find((s) => s.key === SETTING_KEYS.WORKORDER_DEFAULT_PLAN_DURATION_DAYS)?.value,
  );
  ...
  return (
    <div className="space-y-4">
      {/* Bilgi kutusu — kart-içi olduğu için tam Card değil, hafif bordürlü blok. */}
      <div className="flex gap-3 rounded-md border bg-muted/30 p-3 text-sm">
        ...
        <div className="space-y-1.5">
          <p className="text-muted-foreground">
            Sipariş veya iş emri açılırken termin alanı boş bırakılırsa, bu sayılar
            otomatik olarak başlangıç tarihine eklenir.
          </p>
          <p className="text-xs text-muted-foreground">
            <span className="font-medium text-foreground">Örnek:</span> Sipariş tarihi
            23.05.2026, termin 7 gün → otomatik termin{" "}
            <span className="font-medium text-foreground">30.05.2026</span>.
          </p>
        </div>
      </div>
...
function ReadOnlyView({ orderValue, woValue }: { orderValue: number; woValue: number }) {
  return (
