...
/**
 * AYAR YÜZEYİ KABUĞU — sol dikey kategori menüsü (BÖLÜM başlıkları altında
 * toplu) + sağ içerik. Kategoriler `settings-config.ts`te, bölümleme/rejim/arama
 * saf katmanda (`settings-groups.ts`).
 *
 * ⚠️ NEDEN PARAMETRELİ (2026-09-04): aynı kabuk artık İKİ ekran çiziyor —
 * "Genel Ayarlar" (`surface="settings"`) ve "Özellik Anahtarları"
 * (`surface="flags"`). İkinci bir kopya yazılsaydı arama/aktif sekme/kirli
 * taslak onayı gibi ölçülmüş dört kural iki yerde yaşardı ve biri güncellenmeden
 * kalırdı; kural dosyanın kendi başlığında yazılı olduğu için kopyanın bedeli
 * SESSİZ olurdu.
 *
 * ⚠️ SATICI YÜZEYİ (`vendor`) BU KABUĞU KULLANMAZ: modül anahtarları + kurulum
 * beyanı kendi ekranında (Sistem → Modüller) yaşar ve orada sekme şeridi değil
 * kurulum fotoğrafı (profil farkı · bağımlılık okları · gizlenecek ekranlar)
 * gösterilir.
 */
export function SettingsSurfacePage({
  ...
  return (
    <PageShell>
      <PageHeader
        title={soleCategory ? soleCategory.label : title}
        actions={
