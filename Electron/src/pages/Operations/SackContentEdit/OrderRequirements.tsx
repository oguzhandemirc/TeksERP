...
/**
 * Paketleme rehberi — müşterinin açık siparişleri (ne isteniyor). Her kalem:
 * İstenen / Çuvallanmış (packed havuz rezervi) / Açık (istenen − sevk − çuvallanmış) /
 * Depoda (serbest stok). Salt bilgi; paketlemeyi zorlamaz (gevşek model).
 */
export function OrderRequirements({ orders }: { orders: OpenOrder[] }) {
  ...
  return (
    <section className="overflow-hidden rounded-lg border border-primary/30 bg-card">
      ...
      <div className="max-h-[30vh] divide-y overflow-auto">
        {orders.map((o) => (
          <div key={o.order.id} className="p-3">
            <div className="mb-1.5 flex items-center gap-2 text-xs">
              <span className="font-mono font-semibold">{o.order.orderNumber}</span>
              {o.order.deadline ? (
                <span className="text-muted-foreground">termin {safeFormat(o.order.deadline, "dd.MM.yyyy")}</span>
              ) : null}
            </div>
            <table className="w-full text-xs">
              <thead>
                <tr className="text-left text-[11px] uppercase tracking-wide text-muted-foreground">
                  ...
                  <th className="py-1 text-right font-medium">Çuvallı</th>
                  <th className="py-1 text-right font-medium">Açık</th>
                  <th className="py-1 text-right font-medium">Depo</th>
                </tr>
              </thead>
              <tbody>
                {o.lines.map((l) => (
                  <tr key={l.lineId} className="border-t">
                    ...
                    <td className="py-1 text-right tabular-nums text-emerald-600">{m(l.packed)}</td>
                    <td className="py-1 text-right font-medium tabular-nums">{m(l.openQty)}</td>
                    <td className={`py-1 text-right tabular-nums ${l.covered ? "text-emerald-600" : "text-amber-600"}`}>
                      {m(l.warehouseAvailable)}
                    </td>
                  </tr>
                ))}
              </tbody>
