...
/**
 * Radix Select boş string'i değer olarak KABUL ETMEZ (`value=""` = "seçim yok"
 * demektir ve placeholder'ı geri getirir) — "kat yok" seçeneğinin kendi sentinel
 * anahtarı olmak zorunda. Katalog kodlarıyla çakışmaması için `__` öneki.
 */
const NO_FOLD = "__none__";

/**
 * Spec düzeltme — yanlış girilmiş renk/kalite/en/kat/özelliği düzeltir (mevcut
...
export function RelabelSpecForm({ ctx, onSaved }: { ctx: RelabelContext; onSaved: () => void }) {
  ...
  // KAT — boş string = "kat yok" (temizle). Katalog kodu saklanır, ad değil.
  const [foldType, setFoldType] = useState<string>(ctx.foldType ?? "");
  ...
  // Kat kataloğu — sabit liste YOK (fabrika panelden 6-KAT ekleyebiliyor).
  // Boş dönerse seçici yerine uyarı basılır; sessiz iki tuş göstermek, eklenmiş
  // değeri yok saymak olurdu (bkz. lib/fold-catalog.ts).
  const foldsQ = useQuery({
    queryKey: FOLD_CATALOG_QUERY_KEY,
    queryFn: loadFoldValues,
    staleTime: 5 * 60_000,
  });
  const foldValues = foldsQ.data ?? [];
  ...
  const mut = useMutation({
    mutationFn: async () => {
      await relabelService.applySpec(ctx.id, {
        ...
        // KAT: yalnız DEĞİŞTİYSE gönder. Katalog okunamadıysa (foldValues boş)
        // seçici çizilmez ve state ctx'ten geldiği gibi durur → koşul da sessiz
        // kalır; alanı koşulsuz göndermek o durumda kata dokunma riski taşırdı.
        // Boş string = "kat yok" → null (temizle), backend'in üçlü sözleşmesi.
        ...(foldType !== (ctx.foldType ?? "") ? { foldType: foldType || null } : {}),
    ...
    <div className="space-y-3 rounded-lg border p-4">
      <div className="flex items-center justify-between">
        ...
        <span className="text-xs text-muted-foreground">
          renk · metraj · kalite · en · kat · özellik{canKartela ? " · kartelalık" : ""}
        </span>
      </div>
      ...
      {/* KAT — kolonda yaşayan SEÇİM karakteristiği (pivot değil), o yüzden
          yukarıdaki salt-okunur CHOICE çiplerinin AKSİNE buradan düzeltilebilir.
          Katsız doğmuş eski topların (2026-08-13 kesim hatası) düzeltme kapısı. */}
      <div className="grid grid-cols-2 gap-3">
        <FormField
          label="Kat"
          hint={ctx.foldType ? undefined : "Bu topta kat bilgisi yok — seçerek ekleyebilirsin."}
        >
          {foldsQ.isLoading ? (
            <div className="text-xs text-muted-foreground">Kat kataloğu yükleniyor…</div>
          ) : foldValues.length === 0 ? (
            <div className="rounded-md border border-dashed px-3 py-2 text-xs text-muted-foreground">
              Kat kataloğu tanımlı değil (Tanımlar → Kumaş Özellikleri → KAT).
            </div>
          ) : (
            <Select
              value={foldType === "" ? NO_FOLD : foldType}
              onValueChange={(v) => edit(setFoldType)(v === NO_FOLD ? "" : v)}
              disabled={disabled}
            >
              <SelectTrigger>
              ...
              <SelectContent>
                <SelectItem value={NO_FOLD}>— (belirtilmemiş)</SelectItem>
                {foldValues.map((f) => (
