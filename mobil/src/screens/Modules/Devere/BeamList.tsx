// =============================================================================
// LEVENT LİSTESİ — iki sekme: Planlı (sar / taslak sil) · Bugün sarılan (iptal, yetenek izniyle)
// =============================================================================
    ...
    <View style={styles.rowCard}>
      ...
      {planned ? (
        <View style={styles.actions}>
          <Button mode="outlined" compact icon="delete-outline" disabled={locked} onPress={() => state.setModal({ kind: 'delete', beam })}>
            Sil
          </Button>
          <Button mode="contained" compact icon="rotate-right" disabled={locked} onPress={() => state.openWind(beam)} contentStyle={styles.tallButton}>
            SAR
          </Button>
        </View>
      ) : (
    ...
    <View style={styles.root}>
      <View style={styles.tabs}>
        <SegmentedButtons
          ...
          buttons={[
            { value: 'planned', label: `Planlı (${state.planned.length})`, icon: 'clipboard-list-outline' },
            { value: 'today', label: `Bugün sarılan (${state.todayWound.length})`, icon: 'check-circle-outline' },
          ]}
        />
      </View>
      {loading ? <ActivityIndicator style={styles.spinner} /> : null}
      {state.plannedError ? <Text style={styles.error}>Liste yüklenemedi — yenileyin.</Text> : null}
      <FlatList
        data={rows}
        keyExtractor={(b) => b.id}
        ...
        ListEmptyComponent={
          loading ? null : (
            <Text style={styles.empty}>{tab === 'planned' ? 'Planlı levent yok — "Yeni levent" ile plan açın.' : 'Bugün sarılmış levent yok.'}</Text>
          )
        }
      />
    </View>
...
const styles = StyleSheet.create({
  ...
  error: { color: colors.dangerText, textAlign: 'center', marginTop: spacing.sm },
});
