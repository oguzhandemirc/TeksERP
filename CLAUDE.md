...
- Yalnız soft delete (`isActive:false` / `RollStatus.CANCELLED`); `SCRAP` gerçek fire kararıdır, arşivleme değil. Hard delete DÖRT ölçülmüş sınıfla sınırlıdır — ① bağımlılık-guard'lı silme (master-data `DELETE /:id/permanent`, boş çuval, cihaz unpair) · ② alias / karar satırı (satırın kendi içeriğinden başka içeriği yok; "sil" = kararı geri al) · ③ pivot / çocuk satır replace (üst kayıt yaşar; `ItemPrice` satırı da burada, `isActive` EKLENMEZ) · ④ deftere hiç yazmamış taslak (atomik claim'li). Sınıf tablosu ve sekiz sitenin tamamı `docs/standart/VERITABANI.md` §9'da; alan emsalleri `docs/kurallar/top-duzeltme.md`'de.
