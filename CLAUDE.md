...
- Yalnız soft delete (`isActive:false` / `RollStatus.CANCELLED`); `SCRAP` gerçek fire kararıdır, arşivleme değil. Bilinçli istisnalar (bağımlılık-guard'lı `DELETE /:id/permanent`, boş çuval silme, cihaz unpair, pivot replace, `ItemPrice` satırı) `docs/kurallar/top-duzeltme.md`'de.
- Her CUD → `AuditService.log()` (tek istisna `UserPreference`); audit best-effort ve tx DIŞINDA; sayaç `/api/admin/health`te. Kalıcı sayaç/rapor `SystemLog`tan değil KALICI kolondan okunur (audit 6 ayda arşivlenir).
- Sevk rakamı HER yüzeyde BRÜT; iade ayrı belgeyle kapanır, çıkış belgesi düzeltilmez; storno ≠ iade. Olay defterinde kronoloji `createdAt`'tir (`eventDate` kullanıcı girdisi); ters kayıt daima bugüne yazılır; deftere yazan her ileri kaynağın `*_CANCEL` ters yolu olmalı.
...
- Advisory kilit tx'in **İLK ifadesi**dir (sonra alınan kilit hiçbir şey kazandırmaz, TOCTOU); uzaylar ayrı ve envanteri `helpers/period-guard.helper.ts` başlığında (8021 KK1 mükerrer · 8022 parti no · 8023 sevkiyat · 8024 oturum · 8025 izin · 8026 cari dönem · 8027 alış siparişi · 8028 kasa/banka; ⚠️ master-data merge 8027'yi paylaşıyor ve envanterde yok — açık iş). Tek tx'te birden çok kilit deterministik sırada; PG 40P01/40001 → 409 "tekrar deneyin".
- Durum ↔ sayaç çifti olan modelde iki yazar da karşı koşulu kendi atomik WHERE'ine koyar + DB CHECK seddi (çift yüklem kuralı).
- **İdempotency:** kayıt yaratan uçlar `clientToken @unique` taşır (16 model); istemci token'ı MANTIKSAL DENEME başına bir kez üretir; token yalnız sonucu belirsiz bırakan hatada (ağ/zaman aşımı/5xx) yapışır, kesin 4xx'te yapışmaz; replay dört durumlu (`helpers/token-replay.helper.ts`).
