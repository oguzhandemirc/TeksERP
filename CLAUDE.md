...
- **İdempotency (2026-07-14):** Kayıt-yaratan uçlar istemci `clientToken @unique` taşır (Roll, Order, WorkOrder, KartelaDispatch) — timeout-retry'de mükerrer kayıt önlenir; sayaç-bazlı kartela düşümü için `SwatchStockReduction` olay modeli. Durum geçişleri **atomik claim** (`updateMany WHERE {id, beklenen-durum}` + `count===0 → 409`; `findUnique→if→update` YASAK).
