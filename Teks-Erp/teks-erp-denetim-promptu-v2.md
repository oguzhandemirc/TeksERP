...
- **Nihai çıktı**: tek bir rapor artefaktı (Bölüm 7 yapısında) + bulguların JSON dosyası (`audit/findings.json`) + repro scriptleri.
...
Aşağıdaki okuyucu ajanları **paralel** başlat. Her biri yalnızca haritalar; bulgu yazmaz, yargı vermez. Çıktıları ortak bir bağlam dosyasına yazılır (`audit/00-map/*.md`).
...
Çıktı: 10/10 denemede toplam sevk sipariş miktarını aştı. Ek: `audit/repro/042.log`
...
```
Dev DB: 17 satır. Prod (salt-okunur): 4 satır. Ek: `audit/data/042.csv`
...
5. ⑤ SENTEZ ile Bölüm 7 yapısında raporu üret; `audit/findings.json` ve repro scriptlerini bırak.
