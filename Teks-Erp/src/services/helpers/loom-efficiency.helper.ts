...
// =============================================================================
// A = APT / POT · P = (picks − gapUnits) / capApt · E = (picks − gapUnits) / capPot
...
//    `/ targetUnitCapacity*` bölmesi bu dosya dışında GEÇEMEZ; `…Pct ?? 0` ve
...
export function computeMachineKpis(t: LoomKpiTerms): LoomKpis {
  ...
  const netPicks = t.unitsActual - t.gapUnits;

  const availabilityPct = t.potSec > 0 ? toPct(t.aptSec / t.potSec) : null;
  if (availabilityPct === null) olculemedi.A = "planlı süre (POT) sıfır";

  const performancePct = t.targetUnitCapacityApt > 0 ? toPct(netPicks / t.targetUnitCapacityApt) : null;
  ...
  const effectivenessPct = t.targetUnitCapacityPot > 0 ? toPct(netPicks / t.targetUnitCapacityPot) : null;
...
export function aggregateMachineKpis(rows: LoomKpiTerms[]): LoomKpiAggregate {
  ...
  for (const r of rows) {
    const net = r.unitsActual - r.gapUnits;
    if (r.potSec > 0) { sum.apt += r.aptSec; sum.pot += r.potSec; } else olculemedi.A += 1;
    if (r.targetUnitCapacityApt > 0) { sum.picksA += net; sum.capA += r.targetUnitCapacityApt; } else olculemedi.P += 1;
    if (r.targetUnitCapacityPot > 0) { sum.picksP += net; sum.capP += r.targetUnitCapacityPot; } else olculemedi.E += 1;
  }
  return {
