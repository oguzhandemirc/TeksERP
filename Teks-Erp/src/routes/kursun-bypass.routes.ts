 ...
 *   get:
 *     tags: [KursunBypass]
 *     summary: Kurşun Dağıtım ekranının tek payload'ı (bayrak + istasyonlar + bekleyen + dağıtılmış)
 *     description: |
 *       Electron ve mobil AYNI yanıtı tüketir. `waiting` satırları `eligible` +
 *       `blockReason` taşır (uygunluk kurallarından hangisi düştü); `assigned`
 *       satırları `stale` + `staleReason` taşır (adım kapandı / açık top kalmadı
 *       → UI iptal önerir). `flagEnabled=false` ise yalnız YENİ atama kapalıdır,
 *       mevcut dağıtımlar tamamlanabilir.
 *     security: [{ bearerAuth: [] }]
 *     responses:
 ...
 *   post:
 *     tags: [KursunBypass]
 *     summary: İş emrinin kurşun adımını fiziksel bir kurşun istasyonuna dağıt
 *     description: |
 *       Zaten dağıtılmışsa satır güncellenir (istasyon değişimi) — bu durumda
 *       `production.kursunBypassEnabled` bayrağına BAKILMAZ. Adımın `stationId`'si
 *       atanan istasyona repoint edilir (istasyon hacim raporları + kurşun
 *       kuyruğu bedavaya doğru çalışsın); atama öncesi istasyon iptalde geri yüklenir.
 *     security: [{ bearerAuth: [] }]
 *     requestBody:
 *       required: true
 *       content:
 ...
 *           schema:
 *             type: object
 *             required: [workOrderId, stationId]
 *             properties:
 *               workOrderId: { type: string, format: uuid }
 *               stationId:   { type: string, format: uuid, description: Station.kind = PROCESS_QC }
 *               notes:       { type: string, maxLength: 500 }
 *     responses:
 *       201: { description: Dağıtım kaydedildi }
 *       400: { description: Bayrak kapalı / adım uygun değil / istasyon uygun değil }
 *       401: { description: Yetkisiz }
 *       404: { description: İş emri veya istasyon bulunamadı }
 ...
 *     description: |
 *       Atomik claim (yalnız açık kayıt) + adımın istasyonunu atama öncesi
 *       istasyona geri yükleme. İstasyon bu arada başka yolla değiştiyse geri
 *       yükleme yapılmaz — hata değil, audit'e not düşülür.
 *     security: [{ bearerAuth: [] }]
 *     parameters:
 *       - in: path
 *         name: id
 ...
 *       `KURSUN_BYPASS_FINISHED:<uuid>` marker'ıyla kapanır, istasyon yetenekleri
