...
//   §5 AST tripwire: `/ potSec|aptSec|targetUnitCapacity*` bölmesi helper dışında YOK ·
...
//   · `toPct(netPicks / t.targetUnitCapacityApt)`nin bölmesi `machine-shift-stat.service.ts`e
...
export const BOLME_DESENI = /\/\s*(?:[\w$]+\.)*(potSec|aptSec|targetUnitCapacity\w*)\b/;
...
function main(): void {
  ...
  const t1 = { potSec: pot, aptSec: apt, unitsActual: picks, gapUnits: 0, targetUnitCapacityApt: (target * apt) / 60, targetUnitCapacityPot: (target * pot) / 60 };
  ...
  // ── §2 payda 0 → null ─────────────────────────────────────────────────────
  const k2 = computeMachineKpis({ potSec: 0, aptSec: 0, unitsActual: 100, gapUnits: 0, targetUnitCapacityApt: 0, targetUnitCapacityPot: 0 });
  ...
  const k2b = computeMachineKpis({ potSec: 100, aptSec: 50, unitsActual: 10, gapUnits: 0, targetUnitCapacityApt: 0, targetUnitCapacityPot: 0 });
  ...
  const k3 = computeMachineKpis({ potSec: 100, aptSec: 100, unitsActual: 1500, gapUnits: 0, targetUnitCapacityApt: 1000, targetUnitCapacityPot: 1000 });
  ...
  const rows = [
    { potSec: 100, aptSec: 50, unitsActual: 1000, gapUnits: 0, targetUnitCapacityApt: 2000, targetUnitCapacityPot: 4000 },
    { potSec: 100, aptSec: 100, unitsActual: 100, gapUnits: 0, targetUnitCapacityApt: 100, targetUnitCapacityPot: 100 },
    { potSec: 0, aptSec: 0, unitsActual: 0, gapUnits: 0, targetUnitCapacityApt: 0, targetUnitCapacityPot: 0 },
  ];
  ...
  const aggGap = aggregateMachineKpis([{ potSec: 100, aptSec: 100, unitsActual: 1000, gapUnits: 200, targetUnitCapacityApt: 1000, targetUnitCapacityPot: 1000 }]);
  check("§4e gapUnits PAYDAN düşer (800/1000)", aggGap.performancePct === 80 && computeMachineKpis({ potSec: 100, aptSec: 100, unitsActual: 1000, gapUnits: 200, targetUnitCapacityApt: 1000, targetUnitCapacityPot: 1000 }).performancePct === 80);
  ...
  check("§5l ⭐ sentetik ihlal: `picks / t.targetUnitCapacityApt` yakalanır", BOLME_DESENI.test("picks / t.targetUnitCapacityApt"));
