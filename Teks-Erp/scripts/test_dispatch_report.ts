...
async function main() {
  ...
  const customer = await prisma.customer.create({
    data: { code: `TST-RPT-${ts}`, name: `TEST RAPOR MÜŞTERİ ${ts}` },
    select: { id: true },
  });
  const itemA = await prisma.item.create({
    data: { code: `TST-RPT-A-${ts}`, name: `MC 156 ${ts}`, itemType: "FABRIC", unit: "MT" },
    select: { id: true },
  });
  const itemB = await prisma.item.create({
    data: { code: `TST-RPT-B-${ts}`, name: `NEPS VUAL ${ts}`, itemType: "FABRIC", unit: "MT" },
    select: { id: true },
  });
  ...
  try {
    ...
    check(
      ...
      d.header.customerName === `TEST RAPOR MÜŞTERİ ${ts}` &&
