...
async function main() {
  ...
  const sack1 = await prisma.sack.create({
    data: { sackNo: `TEST-AEX-SK1-${ts}`, shipmentId: shipment.id, seq: 1, manualCode: "AMB10001", weightKg: 65.8 },
    select: { id: true },
  });
  const sack2 = await prisma.sack.create({
    data: { sackNo: `TEST-AEX-SK2-${ts}`, shipmentId: shipment.id, seq: 2, manualCode: "AMB10002", weightKg: 40 },
    select: { id: true },
  });
