...
3. **Koş (backend):** `cd Teks-Erp && npx tsx scripts/run-all-tests.ts <ad-parçası>` — tek testte tip kapısı atlanır; birden çok test için ad parçalarını ayrı ayrı koş. DB isteyen bekçiler `.env`deki dev DB'ye (`tekserp_demo`) yazar ve kendi fixture'ını temizler; canlı DB'ye ASLA. Tam paket `npm test` saatler sürer, yalnız istendiğinde.
