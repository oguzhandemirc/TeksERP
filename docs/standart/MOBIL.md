...
- **[MO-02]** Kabuk yalnız navigasyon, yerleşim ve alt görünümlerin bağlanmasıdır — iş kuralı, mutasyon ve HTTP kabukta durmaz · zorlama: eslint:`max-lines` (warn 300 — `mobil/eslint.config.js:237`) + `scripts/check-lint-baseline.mjs` · kanıt: `HizliIsEmriScreen.tsx` 140, `Siparis/SiparisScreen.tsx` 134, `UpdateSettingsScreen.tsx` 305 (bugünkü üst sınır) · devralınan: yok
...
- **[MO-12]** Çakışma 409'u TEK yüzeyde konuşur — ekranın kendi modalı; toast basmaz · zorlama: bekçi:`src/offline/mutations.test.ts:359` ("çakışma 409'u TOAST BASMAZ — ekran modalla soruyor") + `KK1/EntryConflictModal.test.tsx` (modalın dört durumda GERÇEKTEN çizildiğini ölçer; "tek yüzey" iddiasını değil) · kanıt: `offline/queryClient.ts:52` (kural yorumu), `KK1Screen.tsx:3120` (iki 409'un ortak yüzü); alan kararı `docs/kurallar/kk1.md` · devralınan: yok

## 5 · Okutma geri bildirimi

- **[MO-13]** Okutma sonucu `services/scanFeedback.signalScan` ya da `hooks/useScanFeedback` ile verilir; okutma yolunda `Haptics.*` doğrudan çağrılmaz · zorlama: insan:"okutma yolu" AST'den ancak beyaz listeyle ayrılır — `components/BarcodeScannerView.tsx:286` (opt-in yakalama onayı) ve `HizliIsEmri/useQuickWorkOrder.ts:657` (yakalama tıkı) meşrudur · kanıt: `services/scanFeedback.ts:6-18`; bugünkü tüketiciler `KK1Screen.tsx:607,615` · `FasonSevkScreen.tsx:792` · `useQuickWorkOrder.ts:604` · devralınan: yok (İ-20 kapandı 2026-09-05; bekçi `src/test/scan-haptics.guard.test.ts`, `DEVRALINAN` listesi BOŞ)
- **[MO-13a]** "Soru" bir okutma sonucu DEĞİLDİR: onay modalı açan dal (KursunQc reopen · FasonSevk kumaş uyuşmazlığı · FasonKabul çoklu adım) sinyal vermez, sonucu cevaptan sonra doğar; ara adım da sinyal vermez (Tambur kurşun bypass'ı kartı yeniden çözer, sonucu orası söyler) · zorlama: insan:AST "soru mu sonuç mu" ayrımını yapamaz · kanıt: `KursunQcScreen.tsx:409-411`, `FasonSevkScreen.tsx:768-770`, `TamburScreen.tsx:1022-1024` · devralınan: yok
...
- **Kapandı (İ-20, 2026-09-05):** okutma yolundaki 11 dosyanın tamamı `signalScan`e çevrildi (`Depo` · `KursunQc` · `Paketleme` · `FasonKabul` · `FasonSevk` kart yolu · `Tambur` · `KartelaSevk` · `IadeGirisi`); ham `Haptics` okutma yolunda yalnız üç gerekçeli muafiyette kaldı (`BarcodeScannerView#handleScanned`, `useQuickWorkOrder#handleScan` yakalama tıkları ve `LoginScreen#submitCard` KİMLİK yolu). Çevrimin dal-dal dökümü: `olcum/faz0-acik-olcumler.json → I_20_mobil_haptics`, kapanış kaydı `docs/history/perf-2026-09-05/kalan-mobil-okutma.json`.
...
- **[MO-25]** Tanımlayıcı ASCII'dir — Türkçe karakter yalnız UI metninde, hata mesajında ve yorumda · zorlama: eslint:`@typescript-eslint/naming-convention` (error) — `custom: { regex: '[çğıöşüÇĞİÖŞÜ]', match: false }` HER kayıtta tekrarlanır; mobilde `no-restricted-syntax` yuvasını ham hex (warn) tuttuğu için yasak buraya taşındı (`mobil/eslint.config.js:105-112,190-202`) · kanıt: ölçüm 2 ihlal / 1 dosya (`Sevkiyat/SackContentsModal.tsx` `styles.boş`) → düzeltildi, bugün 0; negatif sonda: yasak yalnız `default` kaydında dursaydı `variable`/`function`/`typeLike` onu GÖLGELERDİ · devralınan: yok
...
Tablodaki birim-başına sayılar İNSAN hedefidir; MEKANİK kapı tek bir sayıdır ve **300**'dür (`max-lines`, warn + tavan). Kapıyı 400'e çıkarmak devralınan 44 dosyalık tavanı gevşetirdi ve üç projedeki tek ortak sayıyı bozardı — belge configi izler.

- **[MO-30]** Bu sınırlar yeni dosyalarda ve mevcut dosyaya eklenen yeni bölümlerde zorunludur; devralınan dosya baseline'da donar ve tavan yalnız düşer · zorlama: eslint:`max-lines` (warn **300**, `skipComments`+`skipBlankLines` — `mobil/eslint.config.js:237`) + `scripts/check-lint-baseline.mjs` · kanıt: tavan `mobil/lint-baseline.json` `max-lines: 44` (5 dev ekran adlı muafiyette); skipComments açık/kapalı farkı 21 dosya — "yorum bir değerdir" sayıyla doğrulandı · devralınan: 44 (tavan, yalnız düşer)
- **[MO-31]** Tek React bileşeni fonksiyonu ≤ 300 satır ve ≤ 12 `useState` taşır; fonksiyon boyu kuralı dev ekranlarda da AÇIK kalır (dosya muafiyeti fonksiyonu muaf etmez) · zorlama: eslint:`max-lines-per-function` (warn 80) + baseline; `useState` sayısı insan:hook sayımı bileşen sınırını AST'de güvenilir çizmiyor · kanıt: `TamburScreen.tsx` tek React fonksiyonu L374-5107 = 4.734 satır / 81 `useState`; 166 ihlal / 115 dosya, 42'si beş dev ekranda · devralınan: 162 (`mobil/lint-baseline.json` `max-lines-per-function` tavanı)
...
- **[MO-38]** Beklenmeyen promise `void` ile işaretlenir; yeni ve dokunulan dosyada `no-floating-promises` sert, devralınanda tavan · zorlama: eslint:`@typescript-eslint/no-floating-promises` (tip bilgili, warn) + baseline · kanıt: 291 ihlal / 36 dosya (168'i beş dev ekranda); tip bilgisinin lint maliyeti ölçüldü: +3,3 sn (%25) · devralınan: 277 (`mobil/lint-baseline.json` `@typescript-eslint/no-floating-promises` tavanı)
...
- **[MO-40]** `src` altında `console.log/info/debug` YASAK — kullanıcıya bilgi toast/şerit ile verilir; `console.warn` ve `error` bilinçli TEŞHİS kanalıdır ve config'te açıkça muaftır (`allow: ['warn','error']`), `scripts/`/`plugins/`/`*.config.js` tamamen muaftır · zorlama: eslint:`no-console` (error + allow listesi) · kanıt: ölçüm 2026-09-05 — `src` altında 11 çağrının 11'i de `console.warn` (7 dosya), `log`/`info`/`debug` = **0**; muaf olmasalardı bilinçli teşhis kanalı susardı · devralınan: yok (düzeltilir)
...
**Bilinen borç (bu standardın kapsamı dışında, her biri ayrı iş):** beş dev ekranı bölmek (23.862 satır; plan kayıtlı) · **3 kuyruksuz mutasyona karar** (İ-21: `TamburScreen` reportError/deleteError, `DepoScreen` unmarkKartela — gerekçe mi kuyruk mu, ölçüm bekliyor) · `ReasonPresetPicker` birleştirmesi 2/4 (İ-23; kalan `TamburScreen` ReasonStep) · merkezi `queryKeys` fabrikası · alias kararı · ham hex codemod'u (jeton sözlüğü eşleşmesi ister) · mobilde şema doğrulama katmanı yok (`zod`; `ESZAMANLILIK.md` § bilinen boşluklar) · `loadAllForPicker` 500 tavanı mobilde SESSİZ kesiyor (`docs/history/perf-2026-09-05/OZET.md` §8).
