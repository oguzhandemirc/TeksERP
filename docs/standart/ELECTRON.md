...
- **[EL-40]** Bir panel testinin ölçütü SÜRESİ değil MARJIDIR: `süre × beklenen yük faktörü < timeout` (vitest varsayılanı 5 sn). Marjsız test yük altında SAHTE KIRMIZI verir ve gerçek bir kırmızıdan ayırt edilemez · zorlama: insan:marj bugün ölçülüyor, tavanı yok · kanıt: ölçüm 2026-09-13 — `WorkOrderCompleteDialog > karar + sebep…` yük altında 492 → 3351 ms (**6,8×**, marj 1,49×) ve ea'nın commit kapısında düştü; `user.type` 23 → 6 karakter ile 422 → **191 ms** (sapma 111 → **9 ms**, marj 3,8×). ⚠️ `user.paste` (156 ms) REDDEDİLDİ — tuş başına `onChange` semantiğini kaybettirir: *testin sorduğu soruyu değiştiren bir hızlanma, retry'ın kuzenidir*; `userEvent.setup({ delay: null })` ise KAZANÇ VERMEDİ (maliyet gecikme değil, tuş başına React render). ⚠️ **SINIF, tek vaka değil** — paketin en yavaş beşi aynı aileden (diyalog + Radix Select + `userEvent`) ve ADIYLA kayıtlı: `devir seçilince sipariş bağı toggle'ı` 1473 ms · `karar + sebep girilince payload` 1300 ms (düzeltildi) · `iki top: her satırın kararı kendi topuna` 1240 ms · `depo seçilince kalite seçici` 1236 ms · `iki top: yalnız seçilen satır devre` 1198 ms. Tavan YOK: yük faktörü sakin makinede ölçülmeden konmaz — **bir sınıfı adlandırmak, ölçmeden tavan koymaktan iyidir**
