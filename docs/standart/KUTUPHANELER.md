...
- **[KU-04]** Bir paketi bu tabloya yazmadan `package.json`a ekleme; tablo ile `dependencies` arasındaki fark bir arızadır · zorlama: bekçi:`test_dependency_contract.ts` (üç projede `dependencies` ∖ tablo = ∅; `devDependencies` araç sınıfı gerekçeli muaf listesinde) · kanıt: bugün `package.json`ı denetleyen TEK bekçi `Electron/src/test/discovery-ipc-contract.test.ts` ve yalnız `bonjour-service`e bakıyor · devralınan: yok
...
- **[KU-06]** Electron'da ham `fetch` yazılmaz; HTTP `axios` + `services/apiClient.ts` üzerindendir · zorlama: bekçi:`ELECTRON.md` § apiClient bekçisi · kanıt: ölçüm — `Electron/src|electron|shared`ta `fetch(` 0 · devralınan: yok
...
- **[KU-19]** `uuid@13` ESM-only'dir ve bugün yalnız Node ≥ 22.12'nin `require(esm)` desteğiyle koşuyor; `engines.node` tabanı bu gerçeği söyleyene kadar yükseltme yapılmaz · zorlama: bekçi:`test_dependency_contract.ts` (engines tabanı + gerçek `require`) · kanıt: `Teks-Erp/package.json:5` `"node": ">=22"` ↔ derlenmiş `dist/services/order.service.js:51` `require("uuid")`; Node 22.0–22.11'de üretim kırılır · devralınan: yok
...
- **[KU-20]** `importFiles: 0` gördüğünde paketi ölü ilan etme; üç kanal doğrulanmadan silme yapılmaz · zorlama: bekçi:`test_dependency_contract.ts` ölü-paket bölümü üç kanalı da tarar ve bilinen istisnalar ADI + GEREKÇESİYLE allowlist'tedir · kanıt: keşif 13 paketten 1'ini yanlış işaretledi — `react-native-ble-plx` CANLI çıktı (`mobil/src/services/bluetooth.service.ts:1` + `app.json` `plugins[1]`) · devralınan: yok
