...
- **[IL-04]** Bir sabit listesi (statü kümesi, izin kümesi, kilit uzayı) elle kopyalanmaz; tek modülden export edilir ve kapsamı **AST tripwire** ile ölçülür — elle saymak sessizce bayatlar · zorlama: bekçi:`test_permission_catalog` · kanıt: `constants/module-flags.ts`, `constants/document-design.ts`
...
- **[IL-11]** Bir yeteneğin "VAR" sayılması için üçü birden gerekir: motor + en az bir çıkış yüzeyi + izin ataması · zorlama: insan · kanıt: 2026-08-26 yarı mamul (üç ucu birden açık kalmıştı: rota yok · izin 0 kullanıcıda · masaüstü çıkışı yok)

## 4 · Ölç, sonra karar ver

- **[IL-12]** Bir yasak yazmadan önce mevcut kodda kaç meşru kullanımı olduğunu ÖLÇ; ölçüm yasağı çürütürse yasak yazılmaz ve **gerekçe kayda geçer** · zorlama: insan · kanıt: `Teks-Erp/eslint.config.mjs` başlığı (`toLocaleUpperCase("tr")`: 56 kullanım, bir kısmı zorunlu → kural reddedildi)
- **[IL-13]** "Şu dosyada şu var / şu bekçi yeter" cümleleri **varsayımdır**; kanıt dosya:satır ya da koşulmuş komuttur · zorlama: insan · kanıt: 2026-08-25 prod notu turu (6 cümle ölçümle düzeltildi)
- **[IL-14]** Yerleşim/görünüm hatasında tahmin değil ölçüm: `adb exec-out screencap` + `uiautomator dump` · zorlama: insan · kanıt: 2026-08-25 `SegmentedButtons` vakası (ilk teşhis yanlıştı)
...
- **[IL-17]** Sabit `UPPER_SNAKE` · tip ve sınıf `PascalCase` · geri kalan `camelCase`; boolean `is/has/can/should` ön eki alır; async fonksiyon adı fiille başlar · zorlama: eslint:`naming-convention` · devralınan: bilinçli kaçışlar (`__xForTests`) allow-desenindedir
...
- **[IL-20]** Fiil sözlüğü sabittir: `assertX` (void + throw guard) · `resolveX` (tek değer türet) · `buildX` · `loadX` · `collectX` · `normalizeX` · `computeX`; çağıranın transaction'ında koşmak ZORUNDA olan fonksiyon `*Tx` ile biter ve ilk parametresi `tx: Prisma.TransactionClient`'tır · zorlama: insan · kanıt: `settings-password.service.ts:107`, `sack-tag.service.ts:499`
...
| Mobil: ekran kabuğu · görünüm · ekran-hook · saf mantık · servis | 250 · 500 · 400 · 200 · 400 | 400 | 600 |
| Fonksiyon parametresi | ≤ 4 | `max-params` | fazlası `opts` nesnesi |
...
- **[IL-22]** Bölme **fırsatçıdır**: dokunulan bölüm helper'a/bileşene çıkarılır, dokunulmayan yerinde kalır. Mega servisi bölmek ayrı bir iştir ve bu turda YAPILMAZ · zorlama: insan · kanıt: `README.md` § bilinen borç
- **[IL-23]** Muafiyet sınıfı gerekçelidir: katalog/registry dosyaları (hook çağırmayan, çoğunluğu veri olan tablolar) bölünmez — bölmek zarar verir · zorlama: insan · kanıt: `Electron/src/pages/GeneralSettings/settings-config.ts` 1.445, `Electron/src/routes/content-routes.tsx` 1.218

## 7 · Yorum

Politika `docs/KOD-KURALLARI.md` § Yorum politikası'nda; burada yalnız özeti:

- **[IL-24]** Yorum 1–3 satır **NEDEN** söyler ve koruduğu satırın hemen üstünde durur; tarih ve ölçüm anlatısı arşive gider, kodda en fazla tek satırlık çapa kalır · zorlama: insan:uzunluk mekanik ölçülebilir ama "neden mi ne mi" ölçülemez · devralınan: ileriye dönük kural — toplu temizlik YAPILMAZ (ölçüm: backend servislerin %25'i yorum, 793 tarihli yorum satırı)
- **[IL-25]** Dosya bir `// ====` banner'ı ile açılır: ne olduğu + NEDEN'i, 3–10 satır · zorlama: insan · kanıt: `sack-tag.service.ts:1-18`
...
- **[IL-29]** Süperadmin parolası/PIN'i/TOTP'si ve ayar şifresi repoya, log'a, sürüm notuna, audit yüküne GİRMEZ; `.env` uyarısı yalnız **anahtar adını** basar · zorlama: bekçi:`test_superadmin_provision §8` · kanıt: kök `CLAUDE.md` § Sır hijyeni
