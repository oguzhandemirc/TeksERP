...
- **[IL-33]** Türkçe harf katlaması TEK yardımcıdan geçer (`upperTr`/`lowerTr`; `"tr-TR"` literali yalnız o dosyada), kod karşılaştırması `foldCodeForCompare` ile yapılır — ham `toLocale*Case` yazan, i/İ tuzağının simetrisini ELLE korumayı üstlenir ve TEK YANLI bir düzenleme onu sessizce bozar · zorlama: bekçi:`test_tr_case.ts` §1 (yardımcı semantiği: i/İ · ı/I gidiş-dönüş) + §2 (yazan `import.service` ↔ 15 okuyan adaptör SİMETRİSİ) · bekçi:`test_yerel_ayar_bagimliligi.ts` §3 (yardımcısızlık cırcırı) · kanıt: simetri ölçüldü 2026-09-14 — kusur bugünkü kodda değil gelecekteki TEK YANLI düzenlemede; göç inince ham katlama 113 → 54 düştü · devralınan: 54 (ham `toLocale*Case` çağrısı)
