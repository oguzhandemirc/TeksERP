...
- **~~12 bekçi ortamdan besleniyor~~ → ALTISI ONARILDI (2026-09-06).** Temiz DB ölçümünün ortaya çıkardığı kümenin altısı düzeltildi ve kök nedenleri **üç sınıfa** ayrıldı: ① **bekçi ön koşulunu varsayıyordu** — `test_auto_draft_shipment` `finance.enabled`in, `test_goods_receipt_invoice` `iplik.enabled`in, `test_roll_po_line_trace` `ticaret.enabled`in ortamda açık olduğunu varsayıyordu (üçü de artık kendi ön koşulunu kurup `finally`de birebir geri yüklüyor). ⚠️ Sonuncusunda etkin değer İKİ bayrağın çarpımıydı (`ticaret && bayrak`) — tek bayrağı açmak yetmiyordu. ② **başka bir bekçinin teardown'ı migration verisini SİLMİŞTİ** — dört modül anahtarı satırı (`production.enabled`, `kumasTeknik.enabled`, `tezgah.enabled`, `depo.multiEnabled`) dev DB'den kalkmış ve o günden beri silik kalmıştı; migration'ın kendi INSERT'üyle geri yüklendi (`depo.multiEnabled` VERİDEN türetilir — aktif depo > 1). ③ Kalanlar (`test_fold_catalog`, `test_kanban_card_projection`, `test_record_provenance`, `test_scan_code_case`, `test_scrap_grade_label`, `test_wip_scorecard`) hâlâ açık.
