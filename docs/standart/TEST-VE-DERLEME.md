...
- **[TD-10b]** Bekçi/paket koşumunun hedefi FİXTURE DB'dir ve kapı ÜÇ AYAKLIDIR: (1) host yerel mi (2) ad `_test` ile bitiyor mu (3) hedefteki top sayısı fabrika eşiğinin (500) altında mı — üçünden biri düşerse koşucu DURUR; kaçış `BEKCI_HEDEF_ONAY=1` ve hedef adı + top sayısı log'a basılır, hacim ölçülemezse bu da yazılır ("ölçülemedi" ≠ "geçti") · silen temizlik yolları (`clean_test_residue.ts`) aynı kapıdan geçer ve `--apply` verilmeden de durur · zorlama: bekçi:`test_script_guards.ts §6–§8` (her ayak iki yönlü) · gerekçe: ortak ağacın `.env`i fabrikanın canlı yedeğini gösteriyor, host kapısı onu yerel sayıyor ve `db-guard` `_dev`i geliştirme hedefi sayıp geçiriyordu
