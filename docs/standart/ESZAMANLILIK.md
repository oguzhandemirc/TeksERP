...
- **[ES-26]** Bloklanma ≠ pencere kapandı: koşullu `updateMany`nin `NOT EXISTS`/ilişki alt sorgusu, ifade bir satır kilidinde bloklanıp uyandığında ESKİ snapshot'la değerlendirilir (PG READ COMMITTED EvalPlanQual yalnız hedef satırı tazeler) — kilit **ayrı bir ifadede önce** (`SELECT … FOR UPDATE`) alınır, claim SONRA gelir ki taze snapshot alsın; sonda "bloklandı"yı değil SONUCU ölçer (`[ES-20]`); düzen READ COMMITTED'a bağlıdır (ifade başına snapshot) — tx `RepeatableRead`e alınırsa ikinci ifade de eski snapshot'ı görür ve düzeltme ölür, o yüzden yalıtım düzeyi bu sitelerde yorumla beyan edilir · zorlama: insan:tek ifadeyle bloklanan claim yeşil görünür, yalnız yarış sondasında kırmızı · kanıt: `src/services/machine-doff.service.ts` (`revokeDoff`: ayrı `FOR UPDATE` + `updateMany … rolls:{none:{}}`), `scripts/test_machine_doff_source.ts §5b-1..5`; negatif sonda 2026-09-13: ayrı kilit kaldırılınca §5b-2/3/5 kırmızı (iptal bloklandı, uyanınca eski snapshot'la damgaladı — top iptal edilmiş doff'a bağlı), claim tarafındaki `FOR UPDATE` düz SELECT'e çevrilince §5b-4/5 kırmızı · devralınan: yok
