...
| **A2** · S · Sistem → Özellik Anahtarları → Devere / Levent → "Levent tezgah bağı defteri" AÇ · "Tezgahtan inen top leventten otomatik düşsün" AÇ → Kaydet → Yenile ✅ | Ayar şifresi bu kurulumda KAPALI (`settingsPasswordRequired=false`) — kaydette şifre sorulmaz. | İki satır açık; Yenile sonrası aynı. | `system_settings` → **`devere.mountTracking`** ve **`devere.autoConsume`** `true` (adlar ölçüldü); `GET /api/feature-flags` → `devereMountTracking`/`devereAutoConsume` true. | ① Ayar şifresi PROFİL: kurulumda açılırsa her kaydette sorulur; kapalıyken tek tık. |
...
| **D2** · T · Planlı → SAR ⏳(d5) · **panel ikizi N1 ✅** (Leventler → Yeni Levent → sağ tık Sar: makine + iplik çıkışı 30 kg + dip 1 kg → READY; aynı defter satırları) | 500 m, adet 1, iplik çıkışı: depo, TEST-L1, 30 kg; dip 1 kg (sebep); devere makinesi → Sarımı Kaydet. | "Bugün sarılan"; panelde TEST-L1 kalan 91 kg. | `warp_beams.status=READY`; `yarn_movements` → −30 (çıkış) + 1 (dip iadesi); lot bakiyesi 91; `warp_beam_events` → WOUND. | ① Makine seçici ön-dolu değil; dip iadesi sebep zorunlu; ~10 dokunuş. ② **O** son sarımın lot/depo/kg'sini ön-doldur. ③ **K** tek makine ön-seç; tek sebepli katalogda sebep sorulmasın. |
...
| **J5** · M · Raporlar → Kasa & Banka Defteri (Cari: TEST Müşteri · Yön: Yalnız giriş) · KDV Dönem Özeti (Yön: Satış · Oran: %20) ✅ (Excel ✋) | Şerhler ekranda: "SÜZGEÇ — Cari: … · SÜZGEÇ — Yön: Yalnız giriş · Bu süzgeçler YALNIZ hareket dökümünü daraltır"; sayfa hesap ÖZETİ çizer (satır = kasa/banka hesabı), TEST Kasa satırında 600 giriş. KDV: "SÜZGEÇ — Yön: Satış · KDV oranı: %20"; **cari seçici YOK** (ölçüldü 0). | Excel meta ✋ (dosya insan gözü). | `GET /api/reports/finance/cash-book?…&cariId&yon=IN` → `meta.secenekler.cariId` TEST Müşteri'yi listeler, `totals.totalIn` ≥ 600 (toplamlar dönemin tamamı — süzgeç dökümü daraltır); `GET …/vat-summary?yon=SALES&oran=20.00` → %20 satırı KDV = matrah × 0,20, `meta.secenekler.oran` dolu; şema `.strict()` — `cariId` 400 (fail-closed). | ① Tarih parametresi ISO datetime ister (`2026-09-01` 400) — panel doğru gönderiyor, elle API kullanan için not. ② **K** (raporlar, küçük) Yön/Tür süzgeç şerhi yalnız bir EKSEN (cari) seçiliyken çiziliyor (`axisNotes` eksen yoksa `ek` satırlarını da düşürür) — tek başına "Yalnız giriş" seçen kullanıcı şerhi görmez. ③ **K** (küçük) KDV oranı şerhi seçenekler gelene dek ham kodu basıyor ("20.00"), sonra "%20"ye dönüyor. |
...
| **L4** · P · Raporlar → Sipariş Karnesi (Müşteri çoklu seçici → TEST Müşteri; "Müşteri varsayılanı" seçici → Yurtiçi; X "Süzgeci temizle") · Sipariş İptal Karnesi (İptal sebebi ekseni) ✅ | Adres çubuğu ölçülemez (MemoryRouter); yerine şerhin düşmesi ölçüldü. | Şerhler: "SÜZGEÇ — Müşteri: …", "SÜZGEÇ — Sevk hedefi: Yurtiçi … Müşteri kartındaki VARSAYILAN hedef — sevkin fiili hedefi değil." X → müşteri şerhi düşer. İptal Karnesi: **sebep seçicisi pasif, "Pencerede sebep yok"**; sayfa "iptallerin yalnız %0'ında sebep kodu var" uyarısı. | `GET /reports/sales/order-intake?customerId&destination=DOMESTIC` → `meta.secenekler.customerId` TEST Müşteri, özet 1 sipariş / 100 m (E1); `…/order-cancellation` → `meta.secenekler.reasonCode=[]`, `summary.reasonFillPct=0` (30 günde 4 iptal, sebepsiz). `meta.varsayilan` YOK — varsayılan niteleyicisi şerh metninde. | ① Veri boşluğu ölçüldü: fabrikanın iptallerinde sebep kodu HİÇ yok — rapor bunu gizlemiyor, soluk basıp uyarıyor (iyi). ② **K** (01/6e) iptal ekranında sebep zorunlu değilse rapor hep boş kalır; ya sebep zorunlu ya varsayılan "belirtilmedi" kodu. |
...
| Adım | Yap | Bekle | Backend doğrulaması | Çıkarım |
|---|---|---|---|---|
| **M1 · M1b · M1c** · S+P+S · Sistem → Modüller → Raporlar → "Kalite Karnesi" anahtarını kapat (anında yazar, ayrı Kaydet yok; ayar şifresi bu kurulumda kapalı) → yönetici: ⌘K "Kalite Karnesi" · Raporlar → Kalite → tekrar aç ✅ | Sürücü üç alt adım (rol değişimi = yeniden giriş). "Doğrudan adres" ölçülemez — panel sekme başına MemoryRouter, hash yok; yerine uç ölçüldü. | Palette 0 sonuç; Kalite kategorisinde yalnız Fire Karnesi + Plan-Sapma Karnesi; kategori karosu kalır (diğer raporlar açık). Açınca anahtar döner. | `reportsClosedKeys=["quality/scorecard"]` (`system_settings.reports.closedKeys`); `GET /api/reports/quality/scorecard` → **403 `REPORT_DISABLED`** (404 değil — fail-closed, varlık doğrulanır); açınca 200, liste boş. | ① Rapor kapatma tek tık, anında yazılıyor; şifre gerekmeyen kurulumda ikinci onay yok — sade. |
| **M2 · M2b · M2c** · S+P(+T d5) · Sistem → **Modüller** → "Dokuma işi modülünü aç" KAPAT → Kaydet → aynı sayfada Raporlar bölümü → yönetici ⌘K "Dokuma İşleri" → tekrar aç ✅ (tablet Bölüm Seçimi ⏳ d5) | Modül anahtarları **Özellik Anahtarları'nda DEĞİL**, Sistem → Modüller sayfasındadır (güzergâh düzeltildi). | **4** dokuma raporu (randıman · duruş pareto · vardiya karnesi · karne) kilit bandıyla pasif; palette "Dokuma İşleri" 0 sonuç; açınca kilit kalkar (0). | `dokumaEnabled=false`; `GET /api/weaving-orders` → **403 `MODULE_DISABLED`**; açınca 200. | ① Kilit bandı raporun altında "modül kapalı" gerekçesiyle — iyi. |
| **M3 · M3b · M3c** · S+P(+T d5) · Sistem → Modüller → "Emanet / konsinye mülkiyet modülünü aç" KAPAT → yönetici: Kumaş Stoğu → Manuel Top Ekle → API: I7'de depoya dönen emanet top → müşterisiz çuval → BAŞKA müşteriye sevk → tekrar aç ✅ (tablet Ham Giriş ⏳ d5) | Sevk denemesi API ile (`POST /shipping/sacks` → `/sacks/:id/scan` → `POST /shipping/shipments`) — kapalı modülde panel yolu aynı diyaloğu açar, ölçüm sunucuda. | "Sahibi (emanet mal ise müşteri)" alanı ÇİZİLMEZ (0). Çuval 201, okutma 200, sevk **409 `OWNER_MISMATCH`** + barkod listesi — **kapalı modül kapıyı KALDIRMAZ** (ölçüldü). Açınca alan döner. | `emanetEnabled=false`; `rolls.ownerCustomerId` korunur, `shipmentId` NULL kalır; `emanetEnabled=true`. | ① Kapatmak yalnız yeni sahiplik YAZIMINI kapatır, mevcut sahipliğin kapısı yaşar — çekirdek kural doğru işliyor. |
...
|---|---|---|---|
| **N1** · P · Leventler → Yeni Levent (İçeride sarıldı) → sağ tık **Sar (WOUND)** (makine TEST-DV1 · iplik çıkışı TEST-L1 30 kg · dönen dip 1 kg + sebep) → sağ tık **Sarımı iptal et** (önizleme dip iadesini ADIYLA listeler, gerekçe ≥ 3) ✅ | Panel D1/D2 ikizi + WOUND_CANCEL. **Güzergâh düzeltildi:** iptal edilen levent PLANNED'a DÖNMEZ — `CANCELLED` terminaldir (`one_wound_uq`, dokuma.md: bir levent bir kez doğar, yenisi açılır). | Sarım sonrası READY, lot −29; iptal sonrası CANCELLED, lot başa. | `warp_beam_events` → WOUND, WOUND_CANCEL (iki satır, `reversesEventId`); `yarn_movements` → WARP_ISSUE 30 · WARP_RETURN 1 · WARP_ISSUE_REVERSAL 30 · WARP_RETURN_REVERSAL 1 (dört satır, silme yok); lot bakiyesi başa. |
| **N2** · M · Operasyon → Sevkiyatlar → satır → Detay → **Sevki Geri Al** (SoD `shipping:undo-dispatch`) ❌ **kısmi İHLAL** | (a) iade almış sevkiyat: diyalog **"Bu sevkiyattan 1 iade alınmış — geri alınamaz"**, düğme pasif — storno ≠ iade kapısı ✓. (b) taze sevkiyat (API ile kuruldu): gerekçe (≥ 3) → "Geri Al ve Kapat" (onay kapalı rejimde varsayılan: sevkiyat PLANNED'da beklemez, kapanır). | Toplar depoya; `dispatchedAt` damgası KORUNUR; olay defteri `DISPATCHED, UNDISPATCHED`. **İHLAL (küçük):** önizleme yalnız "1 çuval · 1 top" sayısı + raf hedefi basıyor; çuval numarası/top barkodu YOK — payload `sacks[].sackNo` taşıyor, ekran basmıyor (çekirdek: "etkilenen HER kayıt listelenir, soyut sayı yetmez"). | `shipments.status` PLANNED/CANCELLED, `dispatchedAt` NOT NULL; `shipment_events` → UNDISPATCHED; `rolls` SHIPPED 0. |
| **N3** · M · Muhasebe → Faturalar → onaylı fatura → **İptal (storno)** → onay ✅ | Onay diyaloğu "TERS kayıt yazılır… Orijinal satır silinmez" der; gerekçe alanı YOK (ölçüldü). | Fatura `CANCELLED` (enumda VOIDED yok); ekstrede iki satır; kapama serbest. | `invoices.status=CANCELLED` + `cancelledAt`; `cari_transactions` → `INVOICE_CANCEL` credit 1.200 (INVOICE satırı durur), bakiye 600 → −600 (tahsilat açıkta); `payment_allocations.revokedAt` damgalandı (silinmedi); sevk `summary.totalMeters` değişmedi. |
...
| **O4** · P · Kapalı modülün ucu ✅ (M2b içinde) | Palet "Dokuma İşleri" yok; `GET /api/weaving-orders` → 403. | Karo/palet yok; uç 403 `MODULE_DISABLED` (`details.code`). | HTTP 403 `MODULE_DISABLED` (M2b ölçtü; `router.use(requireDokumaEnabled)` GET/POST aynı kapı). |
...
| **O7** · P · API · sipariş kalemi birim kuralı ✅ | `POST /api/orders`: ① `unit:"BOBIN"` ② ürünsüz + birimsiz ③ ürünlü + birimsiz. | ① 400 "Geçersiz kalem birimi: BOBIN (MT, KG veya ADET)." ② 400 "…birimi çözülemedi — ürün seçilmemiş." ③ 201, birim ürün kartından `MT` — sessiz varsayılan yok, KOPYA (OrderLine.unit kararı). | İki red yazmadı; `order_lines` yalnız +1. Panel E1 formu birimi ürün kartından doldurur, boş bırakma yolu yok — kural sunucuda. |
