...
## Dilim 2 — davranış bayrakları (gece kararı; tasarım §9 + karar #1/#10)

Keşif üç "çıkışsız kapı" ve iki bekçi kör noktası ölçtü. Kararlar:

| # | Karar | Emsal / gerekçe | Geri alma |
|---|---|---|---|
| D2-1 | **Enum bayrak** kalıbı `sameTypeSessionPolicy`; ama `test_feature_flag_contract` yalnız boolean+sayısal ölçüyor → **üçüncü ayak (`aEnum`, iç ad `enumKey:`) ZORUNLU**, mevcut `sameTypeSessionPolicy` de kapsama alınır. | 2026-08-04 `kk1DuplicateGuardEnabled` vakası (dört kapıdan biri düşerse sessiz). | — |
| D2-2 | `shipping.orderRequirement` yazılır ama **`warn` rejimi ÖNCE tamamlanır**: Hızlı Sevk uyarı üretmiyor, Electron/mobil `warnings`i hiç göstermiyor, Electron'daki `orderless` kutusu ÖLÜ (gövdeye gitmiyor), "Siparişe Bağla" ucu hiçbir istemcide çağrılmıyor. Yani bugünkü "varsayılan davranış" fiilen "uyarının çoğu görünmüyor". | Ölçüm. | — |
| D2-3 | `block` kapısı YALNIZ kurulumda (`createShipment`, `from-rolls`, `setShipmentOrders` boş dizi) — `dispatchShipment`e KONMAZ: bayrak açılmadan kurulmuş PLANNED sevkiyat bina içinde kilitlenirdi (tasarım §11 "geçmişe etki eden bayrak yok"). `orderless:true` beyanı `block`ta da muaf. | §11. | — |
| D2-4 | Hiçbir yeni bayrak Adnan'da AÇILMAZ; `block` ve `manualWeightRestricted` için reçetede ön koşul listesi (APK + panel). Tablet Paketleme bugün `orderIds: undefined` sabit gönderiyor — bayrak açılsa fabrikada hiç sevkiyat kurulamazdı. | Tasarım §3 kural 9. | — |
| D2-5 | `weighRequired`/`manualWeightRestricted` **ebeveynsiz** (Sevkiyat çekirdek blok) → §3.6 resolver YAZILMAZ, düz okuyucu (mekanik uygulayan biri ölü bir `modulAcik` sabiti doğururdu). `manualWeightRestricted` için **yeni izin kodu YOK** (`shipping:write` ↔ `mobile:tarti-paket` ayrımı) — 2026-08-01 kurşun-bypass dersi. `openSack` arka kapısı da aynı yüklemden geçer (yoksa fail-open + izlenebilirlik yalanı). | Ölçüm + emsal. | — |
| D2-6 | `invoiceMode` için **migration GEREKMEZ** (`Invoice.externalNo` şemada var); kapsam DÖRT kolon × İKİ tablo (`Shipment` + `DirectShipment`) ve İKİ yazar (elle iz + iç fatura onayı) — tek yardımcı `assertInvoiceTraceAllowed`. | Keşif ölçümü. | — |
| D2-7 | `quality.gradeRequiredEnabled` **DAR kapsam** (KK1 girişi · Tambur kalan-kuyruk · depo kesimi · WO kapanış `WAREHOUSE`/`A1_STOCK`). Kapsam dışı ve bekçide NEGATİF sonda: `cutOpenFabric`, fason kabul doğumu (bilinçli null), `finalizeRollsAtLastStep` (WO kapanışını kilitlerdi), iade kabulü (`returnGradingEnabled` ile anlam çatışması). Bayrak **çekirdek** (KK1 motoru — karar #2 "motor çekirdek, sunum modülde"). | Ölçüm: kalite 7 yüzeyde, ikisi bilinçli mimari karar. | Kapsam tek satır. |
| D2-8 | `batch.requiredEnabled` **REDDEDİLDİ**, yerine **`batch.autoCreateEnabled`**: 0 açık parti varken sunucu partiyi kendisi doğurur. Gerekçe: sistemde parti YARATAN uç yok (`batch.routes` yalnız move/merge/split) → "zorunlu" çıkışsız kapı olurdu; adı da yaptığı işi söylemeli. Gerçek "zorunlu" (yeni uç + APK) Dilim 3'e. | Çıkışsız kapı sınıfı. | Ad değişikliği tek satır. |
| D2-9 | §3.6 resolver YALNIZ ebeveyni OLAN bayraklara (ticaret/production altı, 7 bayrak). `fason.*`/`kartela.*` anahtarsız modüle ait → bu dilimde çekirdek sayılır (modül seti P1'de yediyle sınırlıydı). Panel kategorilerine `regime:` EKLENMEZ (P5'in `moduleKey` kilidi var). §14'ün **yorum körlüğü ÖNCE düzeltilir** (tüketici taraması yorumları temizlemiyor — iki sahte tüketici ölçüldü); resolver'a geçmek §14'i vakumen yeşile düşürür, asıl ölçüm `test_module_flags §8` ikizi. | Ölçüm: §14 BFS'i 443 dosyanın 350'sine ulaşıyor. | — |
