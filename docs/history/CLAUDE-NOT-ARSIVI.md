...
## 2026-09-14 — TABLET LEVENT SARIM EKRANI İNDİ (devere tablet dilimi, §11; 6e, hüküm 1e) [ÇEKİRDEK] + [PROFİL]

**[ÇEKİRDEK] Hükümler (1e, D1–D7 hepsi önerilen (a)):** plan kökenleri üçü de tablette (her seçeneği uygulayan firma gerçek) · form bağlamı TEK uç `GET /api/warp-beams/tablet-context` tek izinle (`mobile:devere`; operatöre `warpspec/warehouse/customer:read` dağıtılmaz, panel bu ucu kullanmaz) · iptal tablette VAR ama ayrı yetenek `mobile:devere-iptal` + önizleme zorunlu + gerekçe ≥3 (süpervizör şablonunda; dar rolde olan izin `ROLE_COVERAGE_EXEMPT`e girmez — bekçi ısırdı, muaf silindi) · ekran OTURUMSUZ (devere StationKind değil, `producesWarpBeam` yeteneği; `StationKind.DEVERE` RED) · taslak sil tablette VAR (④ sınıfı) · tablet + telefon · tek sha.
**[ÇEKİRDEK] Üç ek şart:** ① `FeatureFlags.devereEnabled` DEFAULT false, HAM bayrak (üretim zincirine bağlı değil — panel `useOperationsVisibility` aynası, K3 emsali), "kapalıyken sıfır fark" iki yönlü sondayla · ② bağlam ucu cevabı OPT-IN allowlist: **allowlist `map`tir, `select` değil** (ölçüldü: yalnız `select`e `taxNumber` eklemek sızdırmadı, `map`e eklemek `test_warp_beam_tablet §2b/§2e` 2 ❌) · ③ `minVersion` HAYIR (yeni yüzey bayrakla kapalı doğar; WARP_* enum değerleri eski tablette hiçbir listeyi beslemez).
**Kuyruk YOK gerekçesi:** iplik çıkışı deftere yazar ve eksi-bakiye kapısı sunucudadır — kuyrukta bekleyen sarım, arada başka bir çıkışla bakiyeyi aşar ve boşalırken sessiz 409 üretir; doff/duruş kararıyla aynı (`networkMode:'always'`). `WARP_RETURN` sebep zemini BOŞ kalır: online-only ekranda gömülü zemin bir karar korumaz.
...
## 2026-09-15 — RAŞEL TAKIMI "N ADET" İNDİ: N ayrı levent tek tx'te, iplik ÷ N, kardeş bağı açık kolon (6e; hüküm 1e S1–S3) [ÇEKİRDEK]

**Karar:** raşel takımında N levent aynı cağlıktan aynı anda sarılır; cağlığa yüklenen kg TOPLAMDIR ve levent başına nominal tam boydur ⇒ pay `÷ N` — "her levente tam miktar" toplamı N katına çıkarırdı (RED). Kuruş SON levente, Σ = toplam birebir (`splitShares`); dip iadesi de ÷N (S3). N ayrı levent = N ayrı WOUND: her kardeş bağımsızdır (kendi tüketimi, iptali; takım iptali yok), tek tx'te doğar — kardeşte gövde meşgulse hiçbiri doğmaz. Kardeş bağı `WarpBeam.setKey` AÇIK kolon (S1: `beamRole`u "SET:…" ile aşırı yüklemek Faz 5 kataloğuyla iki anlam olurdu); üst sınır 24 (S2). Token yalnız ilk leventte; replay `siblings` ile döner. Adet 1 = bugün: istek/yanıt bayt bayt (panel/tablet alanları göndermez).
**Ölçüldü:** `test_audit_muafiyeti §7` dosya cırcırı — kardeş doğuşunu helper'a koymak "audit'siz yazıcı" sayısını 2 artırıyordu; doğuş + audit (CREATE WARP_BEAM) servise alındı, helper saf/salt-okuma kaldı. `test_iplik_regime_gate §4a` yeni yazım kolunu (`warp-beam-wind-write.helper`) "yeni yüzey" saydı → gerekçeli beyan (aynı kapı, bölünmüş dosya).
...
**[PROFİL] Sıfır fark:** Faz 3 menüsü ve kolon içeriği `devere.mountTracking` kapalıyken görünmez (menü prop'u, `currentMachine` hep null); ayar satırları devere kategorisinde. Panel geri alma adayını sunucunun LIFO yükleminin aynasıyla (`undoCandidates`) hesaplar — aday listesi boşsa "neden" adıyla (son durum olayı WOUND → sarım iptali kendi ucundan).
**Ölçüldü:** panel `WarpBeamStatus` union'ı F1'in `SHIPPED_OUT`unu taşımıyordu — rozet sözlüğü `Record<…>` olduğu için fasondaki levent listede `undefined.badgeClass` ile patlardı; E2 yedi durumu aynaladı ve `warp-beam-regime.test` iki yönlü eşitliği ölçüyor.

## 2026-09-15 — DEVERE FAZ 3 TEZGAH BAĞI E1 İNDİ: levent yuvaya takılır/sökülür/tüketilir/biter, "şu an ne" kolonu olayla aynı claim'de (6e; hüküm 1e) [ÇEKİRDEK] + [PROFİL]

**[ÇEKİRDEK] Karar:** saha soruları #13–#16 cevapsız kaldı; kullanıcı doktrini "her ihtimali bayrak ve seçeneklere bağlı uygula" → ölçüm yolu (`lengthSource`), bağlama yöntemi, artık beyanı (metre | tartı | 0) ve dispozisyon sebebi hepsi AKSİYON ANINDA veri; yalnız "defter açık mı" ve "yöntem zorunlu mu" bayrak. Durum kolonu (`currentMachineId/currentPosition`) defterden türetilmez ama olayla AYNI `updateMany` claim'inde yazılır (`place`) — iki yazar yok; mutabakat §42 LIFO ikizini okur. Geri alma LIFO: yalnız en yeni aktif DURUM olayı (`activeForwardStatusEventTx`), tüketim satırı LIFO dışı (CONSUMED_CANCEL), düzeltme ters bağ almaz — karşı düzeltmeyle kapanır (sebep zorunlu). Kalanı eksiye düşürme yasağı tek kapıda (`assertCoversRemaining`), mutabakat §43 ikinci hat; terminalde kalan 0 (§44). Söküm kapısı tezgah tasarımının düzeltilmiş (1) maddesi: açık koşumda son levent sökülemez, eksik yuva uyarır; koşum açılışı leventsiz açılır ama uyarır (bağ defteri geriden doldurulabilir).
**Ölçüldü:** F61 deseni yeniden ısırdı — `slotRaceTo409` önce `meta.target` okudu, adapter-pg constraint adını `driverAdapterError.cause.constraint`te getirdi, §7 yarış sondası ham P2002 gördü; `p2002Mentions` ile kapandı (elle `meta.target` okuma YOK). `test_warp_beam_mount §2e` yalnız kodu değil `busyBeamNo`yu da ölçer — yoksa ön kontrolün düşmesi sed sayesinde yeşil kalırdı (sonda ②).
**[PROFİL]** `devere.mountTracking` DEFAULT false = bugün (satır yok ↔ 409, ölçüldü) — kapalı bayrak yüzey istemez, E1 tek başına iner (1e kuralı; lot A2/A3 emsalinden farkı: orada açık bayrak tableti kilitliyordu). `devere.mountTrackingRequired` DEFAULT false. Referans fabrika devere kapalı ⇒ sıfır fark.
**Bekçi:** `test_warp_beam_mount` 47 (dört sonda) · `test_machine_run_beam_overlap` 8 (iki sonda + AST tripwire) · `test_consistency §42–§44` · `db_invariants` +5 · `defter_ters_yol` beyanı · `mobil_enum_aynasi` +3 union. Kural satırı `dokuma.md` (LEVENT TEZGAH BAĞI); tasarım `DEVERE-LEVENT-TARAMASI §13`.

## 2026-09-14 — DEVERE FAZ 2 İPLİK LOTU A1 İNDİ: `YarnLot` + `YarnMovement.lotId/bobbinCount`, türetilen bakiye, `devere.lotRequired` (6e; hüküm 1e) [ÇEKİRDEK] + [PROFİL]

**[ÇEKİRDEK] Hükümler (1e, L1–L5 önerilen (a)):** lot etiketli çıkış lot × depo bakiyesini aşamaz → **409 sert, eksi-bakiye bayrağından BAĞIMSIZ** (depo bakiyesi fabrika tercihi; lot etiketi "bu kg lot X'ten çıktı" BEYANIDIR, lotta olmayan kg'nin beyanı yalandır ve türetilen bakiye eksiye düşerse liste yalan söyler) · lot doğuşu mal kabul satırında METİN (`lotNo`) + `[itemId, lotNo]` upsert, 8029 kod-kapsam kilidi `yarnLot` tx'in ilk ifadesi (P2002 yarışı yok), tedarikçi yalnız ilk doğuşta · yeni izin kodu YOK · dip iadesi lotu çıkış lotlarından değilse uyarı. Ek: `lotNo` TRIM + boş → null ama NORMALİZE EDİLMEZ (irsaliye metni olduğu gibi, "YAN 1029-K") · `supplierId` NULL meşru · `devere.lotRequired` DEFAULT = bugünkü davranış ÖLÇÜLDÜ (satır yok ↔ false yanıtı birebir, `test_yarn_lot §7`).
**Tasarım (§3.5 aynen):** bakiye AYRI TABLOYA İNMEZ, `Σ yarnMovementSign × qtyKg WHERE lotId` ile türetilir (`yarn-lot.helper`); `YarnStock` tek yazar kuralı değişmez. Kimlik kapısı `assertLotMatchesItemTx` (lot.itemId = hareket.itemId; DB'de çapraz-tablo CHECK kurulamaz → mutabakat §40 ikinci hat, §41 lot × depo eksi bakiye). Ters yollar lotu KOPYALAR: `reverseGoodsReceiptYarnTx` net kalem × depo × lot, `groupYarnLinesTx` depo × lot (× sebep) ⇒ WOUND_CANCEL lot bazında (aksi hâlde lot bakiyesi şişerdi). Sayım/fatura yolları lot-agnostik kaldı (lotsuz OUT yazarlar — lotlu stok lotsuz düşebilir; A2/A3'te lot seçimi gelirse aynı allowlist).
**Guard sırası (load-bearing):** lot toplamı depo satırının `FOR UPDATE` kilidinden SONRA okunur — iki eşzamanlı lotlu çıkış aynı kg'yi iki kez düşüremez (depo satırı serileştirir, lot o satırın altındadır).
**[PROFİL]** `devere.lotRequired` davranış bayrağı: profillere/modül tablosuna GİRMEZ (modul-bayrak.md); panel satırı A2 (devere kategorisi + `HideableModule`), o güne dek `test_feature_flag_contract` PANEL_EXEMPT gerekçeli. Referans fabrika: devere kapalı ⇒ sıfır fark; lot kaydı bayraksız her kurulumda mümkün.
**Bekçi:** `test_yarn_lot` 39 (dört sonda dosya başlığında) · `test_consistency §40/§41` (+ `consistency-check.sql`) · `db_invariants` (`yarn_movements_bobbin_positive`) · merge-map `YarnLot.itemId` CONFLICT / `supplierId` MOVE. Mobil dokunuş 0; minVersion HAYIR (nullable kolonlar, opsiyonel alanlar).

## 2026-09-14 — `WeavingOrder.cancelReason` ↔ `ReasonPreset` borcu ÖLÇÜLDÜ: kind açılmaz, borç tetik satırına döndü (01; hüküm 1e) [PROFİL]

**Soru (dokuma.md borç satırı, DOKUMA-IS-EMRI §3 şema şerhi):** P2b-2 `ReasonPresetKind.MACHINE_STOP` indikten sonra dokuma işi iptali kendi `kind`ini hak ediyor mu — cevap KULLANIM ÖLÇÜLEREK verilecekti.
...
LİSTEYE DÖNMEK oldu.
## 2026-09-14 — FASON DOKUMA G2 indi: sevk/makbuz BAŞLIĞI polimorfik, iş emri yolu daraltma kapısıyla bayt bayt aynı, TOP makbuzdan doğar [ÇEKİRDEK]

**Kullanıcı sorusu (18:35):** *"dokuma veya deverenin fasonda yapılacağı durumları göz önünde bulundurdun mu? her fabrika kendisi yapmıyor."* Ölçüm: dokuma işi fasona ZATEN atanabiliyordu (`WeavingOrder.executionKind = SUBCONTRACTED` + `subcontractorId`, panel formu), levent gidiş/dönüş F1 ile vardı; **asıl engel** fason sevk/makbuz başlığının yapısal olarak iş emri adımına bağlı olmasıydı (`workOrderId`+`stepId`+`batchId` NOT NULL) — dokuma işi bir `WorkOrder` değildir (rotası/partisi yok), yani fason dokuma sevki hiç açılamıyordu.

**Karar (1e hükmü):** yeni belge AÇILMADI (DEVERE:232 "fason için yeni alan açılmaz"); başlık POLİMORFİK oldu — iş emri kolonları NULLABLE + `weavingOrderId`, tam biri (CHECK `*_header_ck` ikinci hat). Canlı tabloda NOT NULL düşürme: migration `20260914200000_fason_dokuma_header` idempotent (`DROP NOT NULL` · `IF NOT EXISTS` · `DO $$` kısıtlar), sonda DB'de iki kez deploy edildi (ikincisi no-op); FK'lar değişmedi (RESTRICT), yeni FK'lar RESTRICT.

**Ripple ÖLÇÜLDÜ ve kapıya çevrildi:** tsc 154 hata / 146 satır / 10 dosya (113'ü `subcontractor.service.ts`). Çare `!` değil daraltma: `helpers/dispatch-header.helper.ts` `assertWorkOrderBound(row)` (asserts-tipi; skalerler `string`, ilişkiler NonNullable) iş emri yolunun HER yükleyicisinin ardında — dokuma belgesi o yola girerse **409 `DISPATCH_NOT_WORK_ORDER_BOUND`**. Tip düzeltmesi aynı anda semantik kapıdır: daraltmasız `where: { workOrderId: null }` sessizce derlenir ve yanlış satırı okurdu. Listelerde `workOrderBoundOnly(rows)`: iş emri/adım/parti KAPSAMLI sorgularda çalışma zamanında no-op (SQL DEĞİŞMEDİ), `select`e yalnız `workOrderId: true` eklendi. Kapalı (closure) daraltmasında `const closedDispatch = closedItem.dispatch` bağı (TS özellik yolunu closure içine taşımaz). 66 fason/parti/tambur/sevk bekçisi yeniden koşuldu, hepsi yeşil.

**Fason dokuma servisi (`subcontractor-weaving.service.ts`, 300 tavanı altında):** sevk (levent kalemi F1 defteri, `dispatchWarpBeamItemsTx`) · sevk iptali (`SHIP_OUT_CANCEL`, dönmüş levent varsa 409 LIFO) · kabul: makbuz KALEMİ yok, TOP `createInitialEntry(forcedEntrySource: WEAVING, parentReceiptId)` ile her satır kendi tx'inde doğar (mal kabul emsali, `failed[]`), **partisiz** (Batch iş emrine bağlı — plan "parti kabulde doğar" demişti, ÖLÇÜLDÜ: `Batch.workOrderId` NOT NULL; in-house WEAVING topu da partisizdir, parti iş emrine katılırken doğar) · makbuz iptali yalnız doğan topların HEPSİ `CANCELLED` ise (topun ters yolu `ROLL_CANCEL` kendi önizlemeli kapısından; ikinci stok sebebi açılmadı) · özet: çözgü metresi bazında mutabakat (`sentM − returnedM − bornM`, `warnings` take-up şerhi). İş claim'i tx'in ilk ifadesi (`SUBCONTRACTED ∧ açık`, touch), `PLANNED→IN_PROGRESS` tek yazar `markWeavingOrderInProgressTx` (bekçi `test_weaving_order §0b` ısırdı). Top↔dokuma işi bağı tek okuyucu `weavingOrderOfRoll` (DOFF · RECEIPT); `Roll.weavingOrderId` açılmadı (in-house'ta ikinci kaynak olurdu). Route `/api/subcontractor-weaving` `requireDokumaEnabled`, yeni izin kodu YOK (`weavingorder:write` ∨ `mobile:fason-sevk/kabul`); levent dönüşü F1'in ucuyla aynen.

**Kapı ısırıkları:** `test_iplik_regime_gate §2` / `test_ticaret_regime_gate §2` — router geçişli olarak yarn/PO modeline uzanıyor (inventory + subcontractor.service zinciri) → MUAF gerekçeli satır (goods-receipt emsali) · `test_swagger_spec` — flow mapping içinde `[]` YAML'ı bozar (`description: "bornRolls[] + canCancel"` tırnaklı) · `test_weaving_order §0b` tek yazar · `test_identity_ledger` B-d — harita satırı dokuma bölümüne düşünce `dokuma.md` Backend listesi de ister. Bekçi `test_subcontractor_weaving` 29/0, üç negatif sonda kırmızı görüldü (cancel daraltması · claim yüklemi · `parentReceiptId` yazımı); beklenmeyen hata da ❌ satırı basar (dil ölmez).

⇒ **Ders: canlı tabloda NOT NULL düşürmenin bedeli tsc hatası değil, o hataların her birinin bir SEMANTİK kapı olma fırsatıdır — `!` ile kapatılan her hata, yanlış satırı sessizce okuyan bir yol bırakırdı.**

## 2026-09-14 — G2p: fason dokuma PANEL yüzeyi dokuma işi detayında, fason liste sayfalarına DOKUNULMADI [ÇEKİRDEK]

1e hükmü: mevcut Fason Sevk / Fason Kabul formları iş emri adımı + top kümesine göre kuruludur; dokuma modunu oraya eklemek ROLL yolunun panel tarafını da polimorfik yapar ve ekran testlerini ikiye bölerdi. Yerine `WeavingOrders/fason/` (sheet + levent seçici + satır editörü + iptal/dönüş diyalogları; `useFasonMutations` `warnings` basar), satır menüsü "Fason" yalnız `SUBCONTRACTED` işte (`isFasonSectionVisible`). Top kabul satırı sunucu Zod'uyla birebir (metre · en · kg · kalite · renk; sessiz allowlist yok), `failed[]` amber şerit. Levent dönüşü F1'in `beams/:beamId/return` ucuyla — o ucun izin kümesine `weavingorder:write` eklendi (dokuma işi yazarı da dönüş yazabilsin). **Beyan:** dokuma belgeleri fason LİSTE sayfalarında görünmez (`workOrderBoundOnly`), yalnız dokuma işi detayından yönetilir; bu bir vaat değil bugünkü sınırdır. Regime kapısı: `/api/subcontractor-weaving` `DOKUMA_UC_METINLERI`ne, `fason/service.ts` izinli istemci listesine (negatif sonda: başka dosyada mount dizesi → §7b ❌). Electron lint: `max-lines-per-function` 80 yeni kodda serttir — prettier'ın açtığı JSX üç bileşeni aştı, alt bileşenlere bölündü (`FasonLists` · `FasonModals` · `FasonReceiptRows` · `BeamPicker`); prettier'ı klasöre koşturmak devralınan dosyaları da yeniden biçimler — yalnız yeni dosyaya koş.

## 2026-09-14 — G2t: tablet FASON DOKUMA KABUL ekranı — yalnız kabul + levent dönüşü, kuyruk yok, allowlist bağlam [ÇEKİRDEK]

Yeni ekran `FasonDokuma` (`mobile:fason-kabul` ikinci ekranda, `dokumaEnabled`, oturumsuz — Devere emsali); mevcut `FasonKabul`/`FasonSevk` ekranlarına DOKUNULMADI (iş emri yolu tablette de bayt bayt). Bağlam TEK uç `GET /api/subcontractor-weaving/tablet-context`: açık SUBCONTRACTED işler + dönmemiş leventli açık sevkler, `map` allowlist (bekçi §8a′ anahtar kümesini TAM ve DAR ölçer — `select` genişletilince sızmaz, negatif sonda görüldü), modül kapısı serviste ikinci hat (§8c). Top kabulü G2'nin `POST /receipts` ucu (satır metre·en·kg·kalite; renk işin rengi — `receiptSchema` ile birebir), levent dönüşü F1'in ucu; `clientToken` mantıksal deneme başına (parmak izi iş+satır sayısı+toplam metre, belirsiz hatada yapışır). **Kısmi kabul:** sunucunun `failed[]` satırları FORMDA KALIR ve sebebiyle listelenir; doğan toplar kabul edilmiştir (`fason.md` "kısmi kabul topu tüketmez" ilkesinin kabul-tarafı aynası). Sevk açma/iptaller panelden. Sürüm maddesi tablet 1.0.7; minVersion YOK (yeni ekran). Kapı: `test_screen_catalog §3b` yeni mobil ekranı `screenModules` aynasına zorladı; mobil tema `SCREEN_TINT` her ekran anahtarını ister (tsc ısırdı).


## 2026-09-15 — Vardiya karnesi HAT KIRILIMI: eklemeli çocuk tablo, veri tetikli, opt-in okuma [ÇEKİRDEK]

1e'nin 2026-09-14 hükmü (özet §1/1) karne taneciğini makine×vardiya tuttu ve "hat kırılımı gerekirse EKLEMELİ çocuk tablo" dedi; bugün açıldı (`MachineShiftLineStat`, migration `20260915010000`). Üç karar:

- **Tetik VERİDİR, bayrak değil.** Satır yalnız `Machine.productionLineCount > 1` makinede doğar; tek hatlıda çocuk tablo boş kalır ve helper çıktısının geri kalanı bayt bayt eskidir (`test_machine_shift_terms` 41/41 dokunulmadan yeşil; negatif sonda: count kapısı düşürülünce tek hatlıya satır doğar → kırmızı). Referans profilde tüm tezgahlar tek hatlı ⇒ tablo boş.
- **Hat yalnız ÜRETİM terimlerini taşır.** Duruş `MachineStopEvent` makine düzeyindedir (`one_open_per_machine`, hat kolonu yok) — çift enli tezgah durunca iki hat da durur. Bu yüzden POT/APT/duruş ebeveynde kalır, hat satırı atkı · kapasite · hedef · metraj · koşum sayısı taşır; hattın A'sı ebeveynin (aynı çıkar), P/E'si hattın (`computeMachineKpis`e ebeveynin pot/apt + hattın pay/paydası — bölme helper dışına çıkmadı, AST tripwire yeşil). Hat kümesi `1..count ∪ koşumların hatları`: count sonradan düşürülse bile hat 3'ün koşumu kaybolmaz, Σhat = makine değişmezi korunur (bekçi ölçer).
- **Kendi mührü YOK, silme YOK.** Çocuk DURUM'dur (`updatedAt` var), ebeveynle aynı tx'te ve claim'in ARKASINDA upsert edilir (mühürlü satıra yazılmaz); mühür anındaki hâli Seal defterinin `terms.lines` fotoğrafına girer ("aynen yeniden bas"); küme dışında kalan eski satır silinmez — böyle bir satır ancak koşumsuz hattır ve sıfır taşır. M3 elle düzeltme ebeveyne yazar, hatlara dokunmaz; Σhat ≠ karne olduğunda `byLine` okuması uyarı satırı basar (sessiz sapma yok).

Okuma OPT-IN `?byLine=1` (Zod enum `["1","0"]`, tanınmayan değer 400): randıman + vardiya karnesi satırına `hatlar`, M1 listesine `lines`; opt-in yoksa alan hiç konmaz (eski istemci bayt bayt aynı gövde). Pareto duruş ekseninde, hat bilmez — parametreyi almaz. Sürüm maddesi YAZILMADI: backend-only, panel yüzeyi yok (kapsam kararı: API-only maddeye kapsam yok). Tasarım §4 ③b/N9/P-yapı yeniden soruldu, yine geçersiz (çocuk tablonun mührü yok ⇒ "kısmi mühür" hâlâ yapısal olarak yok).
## 2026-09-15 — Tablet paketleme grubu: "Hemen Sevk Et" artık seçili grubu gönderir, tek yüklem, bayrak kapalıyken bayt bayt eski [ÇEKİRDEK]

`CAKILI-VARSAYIM-TARAMA.md` §4'ün kardeşi: backend havuz cevabında `packingGroupId` gönderiyor, tablet okumuyordu; mobil bayrak yorumu *"sevk butonu yalnız AÇIK GRUBU gönderir"* diyordu ama ekran havuzun tamamını gönderiyordu (yorum ile kod ayrışmış, guard yoktu). Kapanış üç karar:

- **Tek yüklem, üç tüketici.** Görünür liste (`filteredSacks`), okutma hedefi (`sacksRef` → `ensureSack`) ve sevk kümesi (`shippableSacks`) aynı `sacksInSelection(sacks, selection, enabled)` çıktısından türer. Üçü ayrı yazılsaydı operatör gördüğünden başkasını sevk eder ya da görmediği çuvala okuturdu — "ayrışan yüzey" sınıfı. Bekçi §16b üçünü birden ölçer (ham `sacks.filter`e dönüş kırmızı).
- **Kapalı = bayt bayt eski.** `usePackingGroupsEnabled` varsayılanı false (fail-closed); kapalıyken grup ucu hiç çağrılmaz (`enabled:` bayrak), şerit çizilmez, yüklem girdiyi AYNI REFERANSLA döndürür (jest `toBe`). Eski backend alanı göndermez → `undefined` gruplanmamış sayılır; eski tablet yeni backend'le bugünkü gibi çalışır — `minVersion` gerekmez.
...
sondalarıdır.*
## 2026-09-15 — G1 İPLİK FASONA GİDER/DÖNER: üçüncü kalem türü, iplik defteri dört tür, fasondaki bakiye TÜRETİLİR [ÇEKİRDEK]

`SEKTOR-YOL-HARITASI` :153 boşluğu ("fasona iplik gönderilince hiçbir iz doğmuyor, elle ADJUST_OUT") 1e hükmüyle (H1–H5) kapandı. Zemin F1'in polimorfik kalemi ve G2'nin polimorfik başlığıydı; G1 üçüncü kolu açtı: `kind=YARN` + `yarnItemId`/`warehouseId`/`lotId?`, `kind_ref_ck` DROP+ADD (eski iki kol bayt bayt), `YarnMovementKind` +4 (`SUBCONTRACT_OUT/_CANCEL/SUBCONTRACT_RETURN/_CANCEL` — iki KARŞI OLAY çifti, `reverses*` bağı yok, wind servisinin storno kalıbı), `YarnMovement.dispatchItemId` + iki yönlü CHECK + kalem başına tek çıkış partial unique, `ReasonPresetKind.YARN_SUBCONTRACT_RETURN` (12 ayna, seed KALAN_IPLIK · KALITE · IPTAL). Pencere 20260915020000–023000 (190000 KULLANILMADI: migration hijyeni "bekleyen damga uygulanmış en büyükten büyük" ister, 200000 zaten inmişti).

Kararlar ve bedelleri:
- **H1 birim.** YARN kalemde `dispatchedQty` KG'dır; ikinci bir `qtyKg` kolonu iki kaynak olurdu, `dispatchedQty_pos` CHECK'i de gevşetilmezdi. Bedel: `totalQty` metre toplamı olarak KALIR (`§19` yüklemi `kind<>'YARN'`), başlık `yarnTotalKg` ayrı taşır, kalem DTO'su `unit:"KG"`yi AÇIK yazar — okuyucu birimi tahmin etmez. İplik kalemi `items` listesine hiç girmez (`roll`/`warpBeam` null bağıyla eski panel kırılmasın), `yarnItems` ayrı alandır.
- **K1 (b) sanal depo YOK.** Fasondaki iplik bir `YarnStock` satırı değildir; `yarnAtSubcontractor` iptal edilmemiş sevklerin `SUBCONTRACT_*` satırlarını kalem × lot toplar (çıkış − iptal − dönüş + dönüş iptali). Sanal depo açılsaydı her fason firması için depo satırı doğar, sayım/transfer ekranları o depoyu gerçek sanır ve eksi bakiye kapısı fasondaki ipliği de kilitlerdi. G1c sarılan leventi ayrı kalem olarak düşecek (H5, beyanlı `theoreticalKg`/WEIGHED).
- **H3 sebep kodu.** `defter.md:17` "iade ayrı satır + sebep kodu" ipliğe de uygulanır; serbest metin önerim reddedildi, katalog kind'ı açıldı (Pareto'lu). CHECK `yarn_movements_fason_return_reason_ck` dönüş ve stornosunda kodu zorunlu tutar; storno aynı kodu kopyalar.
- **H4 kapı.** Tek yazıcı `applyYarnMovementTx` KALDI: iplik modül kapısı ve eksi bakiye kapısı (`SUBCONTRACT_OUT` + `_RETURN_CANCEL` GATED — ölçüt "bakiyeyi düşüren ileri yol") aksiyon anında. Fason router'ı `requireIplikEnabled` ile KAPILI SAYILDI ama yalnız iplik uçlarında (dönüş · storno · bakiye); sevk POST'u top/levent için kapısız kalır, `yarnLines` dolu ∧ iplik kapalı → gövde kapısı 403 (levent/devere emsali). 1e'nin "MUAF satırı" hükmü yerine per-route kapı yeterli çıktı — `test_iplik_regime_gate §2` dosyadaki çağrıyı sayar; yeni yazar `DEFTER_YAZARLARI`nda gerekçeli. F1 §0d′ (beam servisi yarn.service'i ithal etmez) aynen; iplik servisi AYRI dosyadır.
- **H2 depo.** Dönüş varsayılan çıkışın deposu/lotuna; fason başka depoya teslim edebilir (`warehouseId` opsiyonel). Dönüş ucu panel-yalnız ve token'sız (`yarn.routes` create emsali); tablet yüzeyi doğarsa `clientToken` kolonu o dilimde.
- **LIFO.** Dönmüş (net dönüşü > 0) kalemli sevk iptal edilemez — tx-dışı sinyal (`countReturnedYarnItems` → `returnedYarnCount`) + tx-içi hat; storno grup net'iyle (aynı depo/lot/sebep), ikinci storno 409 `YARN_RETURN_NOT_OPEN`. Sevk iptali `SUBCONTRACT_OUT_CANCEL` yazar, kalem silinmez.

Ölçüm: `test_subcontractor_dispatch_yarn` 35/0 (§0 statik iki yönlü: fason türleri ↔ CHECK ↔ işaret ↔ GATED · §3 iplik sevki · §4 dönüş/storno/H2 · §5 iptal · §6 üç DB seddi · §7 iplik kapalı); üç negatif sonda kırmızı görüldü (işaret · OUT yazımı · Σ≤giden). **Sonda dersi:** kapıyı düşüren sonda beklenmedik BAŞARI üretir; `beklenenHata` başarıyı yutunca sevk kayıtsız kalır, `finally` WO'yu silemez (FK) ve kalıntı büyür — 1 dispatch + 2 WO + kart/fasoncu/istasyon kaldı, `test_consistency §19` yakaladı; bekçi `sevkHatasi` ile başarıyı da kaydeder. Panel yüzeyi (G1p) ve sarılan levent bağı (G1c) ayrı sha; sürüm maddesi G1p ile.

## 2026-09-15 — G1c: fasona sardırılan leventin WOUND'u iplik kalemine bağlanır, sarılan kg bakiyeden AYRI düşer [ÇEKİRDEK]

H5 hükmü: fasoncudaki iplik türetilmiş bakiyesi leventlere sarılanı da düşer — tahminle değil, BEYANLA. `WarpBeamEvent.dispatchItemId` zaten vardı (F1, dört fason türünde zorunlu); `warp_beam_events_fason_item_ck` iki yönlüden "fason4 ⇒ bağ dolu; bağ dolu ⇒ fason4 ∪ {WOUND}"a genişledi (migration 20260915024000, DROP+ADD, dört fason türü bayt bayt; WOUND bağlı ya da bağsız, başka hiçbir tür bağ taşıyamaz — `WOUND_CANCEL` bağlı 23514 bekçide). Kalemin TÜRÜ DB'de doğrulanamaz (çapraz tablo) ⇒ servis kapısı `assertYarnDispatchItemTx`: kalem YARN · sevk iptal edilmemiş · fasoncu leventinki (`WARP_YARN_ITEM_MISMATCH`) · iplik kartı çözgünün ipliği; köken kapısı (`WARP_YARN_ITEM_ORIGIN`) tx dışında, in-house/other girdi kontrolüyle birlikte `assertOriginInputs`ta (fonksiyon 80 satır tavanı için ayrıldı). Bakiye: `sarilanKg` = Σ storno edilmemiş (`reversal: null`) WOUND `theoreticalKg`; `sarilanKaynak` THEORETICAL/WEIGHED/KARMA (kgSource beyanı — WOUND yalnız nominal kg saklar, tartılmış kg ayrı kolon değil; "ölçüldü mü hesaplandı mı" raporda görünür); `remainingKg` = çıkış − dönüş − sarılan. İptal edilmiş sevkin kalemine bağlı sarım bakiyeye girmez (OUT satırı yoksa satır yok). Alan opsiyonel ⇒ eski tablet/panel bayt bayt, minVersion YOK; tablet devere ekranına seçici G1p/tablet dilimiyle. Bekçi §8 42/0, iki negatif sonda (fasoncu eşleşmesi · reversal süzgeci). Ders: 6e'nin E1'i `windWarpBeam`i 80 satır tavanına yakın bırakmış; beş satır ekleme tavanı aştı — yeni kapı ayrı fonksiyona çıkarıldı, `max-lines-per-function` yeni kodda serttir.
