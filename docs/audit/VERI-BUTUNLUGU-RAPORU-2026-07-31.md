...
7. **`Sack`/`Shipment` clientToken** (A4) — migration + replay + mobil/Electron istemci güncellemesi (token'ı deneme başına bir üret). **Bekliyor — istemci koordinasyonu gerektirdiği için ayrı onay.**
