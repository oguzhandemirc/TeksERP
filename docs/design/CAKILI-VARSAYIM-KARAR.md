...
**2. Bugün korunur (ölçüldü):** migration `code = 'GENEL'` satırına `isDefault = true` damgalar (geçiş literali, son kullanım; ~~GENEL yoksa hiçbir satır damgalanmaz~~ **GEÇERSİZ → 2026-09-14 (1e hükmü): GENEL yoksa YARATILIR, pasifse AKTİFLEŞTİRİLİR — her kurulumda bir varsayılan olmalı**). Bu fabrikada `[0]` zaten GENEL → tipsiz ekleme aynı satıra yazar, terfi aynı sırayı verir: **sıfır fark**. ~~Varsayılansız kurulumda (ikinci müşteri, GENEL yok) tipsiz ekleme **400** — bugün liste sırasından uyduruyordu; fail-closed, sürüm notu maddesi.~~ **GEÇERSİZ → 2026-09-14 (1e hükmü):** 400 yalnız SONRADAN varsayılan silinir/pasife alınırsa geçerli, o da ea K3 ile ret/uyarıya bağlanıyor. Bedel: taze DB'de migration katalog satırı doğurur → seed aynı kodu yazınca P2002 (CI kırığı, d9 seed'i upsert'e çevirdi).
...
### A — Çelişmeli doğrulama (2026-09-14, 47; ea `f87cb6e5` → origin `f87cb6e5`, taban `cb435108`)
...
**Hüküm (1e, 2026-09-14):** §2 sapması — migration davranışı KALIR (GENEL yoksa yaratır, pasifse aktifleştirir; her kurulumda bir varsayılan olmalı); "varsayılansız kurulumda 400" yalnız sonradan varsayılan silinir/pasife alınırsa geçerli (ea K3 ile ret/uyarıya bağlanır); karar §2 cümlesi GEÇERSİZ damgalı, arşivde tek not. K2/K3/K4 → ea tek sha. **Ölçülmeyen komşu etkileşimi (d9, CI):** taze DB'de migration GENEL'i doğuruyor, `prisma/seed.ts` `defectType.createMany` aynı kodu yazıyor → P2002 — provam dolu dump'taydı (GENEL vardı → UPDATE dalı), boş-DB senaryosu eksikti; d9 seed'i `upsert`e çeviriyor. Ders: bir migration'ın kendi içinde idempotent olması komşusuyla (seed) çakışmayacağını göstermez.

**Kalemlerin son durumu (ea, tek commit, sha 1e'de):** K2 `assertDefaultWriteConsistent` POST/PATCH başında (`isDefault:true` + `isActive:false` → 400, kayıt yazılmaz — ön kontrol; setDefault ayrı tx kalır) · K3 varsayılan tipe PATCH `isActive:false` · DELETE · kalıcı silme → 400 `DEFAULT_DEFECT_TYPE_DEACTIVATE` "önce başka tipi varsayılan yapın" (RET seçildi, uyarı değil) · K4 KursunQc `code === 'GENEL'` → `isDefault === true`; bekçi 12 → 22 (§5 gerçek DB tx geri alma + statik, §6 statik), üç mutasyon ısırdı.
