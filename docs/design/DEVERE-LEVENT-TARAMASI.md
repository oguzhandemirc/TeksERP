...
- **K5 (0c, küçük):** aynı `physicalBeamNo` (tr_fold) ikinci READY → ham Prisma P2002 (ifade indeksi; middleware kolon adı yerine `tr_fold` basar); tx geri alınıyor (PLANNED kalır) ✓ ama Türkçe ön kontrol yok. **K5b (kullanıcı bulgusu 2026-09-18, 6e):** gövde tekilliği tasarım gereği yalnız CANLI leventte; plan/düzenle REDDETMEZ (plan = rezervasyon) ama PLAN ANINDA UYARIR — `createWarpBeam`/`updateWarpBeam` `ApiResponse.warnings`: gövdede canlı levent varsa (`findLivePhysicalBeamTx`, `assertPhysicalBeamFreeTx`in fırlatmayan kardeşi, aynı tr_fold sorgusu) + aynı gövdeye başka PLANNED varsa; panel her uyarıyı ayrı toast basar (`useWarpBeamMutations.settle`). Bekçi lifecycle §12b (üç kontrol, iki sonda) + `useWarpBeamMutations.test` (bir sonda).
...
6. **Raşel takımı (#23)** "N adet" sayacı — İNDİ 2026-09-15 (6e, hüküm 1e S1–S3): `count` 1..24, kardeşler aynı tx'te doğar+sarılır, iplik ÷ N (kuruş sonda), `WarpBeam.setKey` (migration 20260915040000), panel `WindDialog` + tablet `WindModal` "Adet" + gövde öneki; bekçi `test_warp_beam_set` 17/0 (üç sonda). Borç kapandı.
...
- **E2 PANEL İNDİ — 2026-09-15 (6e; taban E1 origin sha'sı):** Leventler satır menüsü +7 eylem (`WarpBeamRowMenu`, `mountTracking` prop'u — bayrak kapalıyken menü Faz 1b ile birebir; tak/sök/tüket/düzelt/bitir `warpbeam:write`, hurda/geri al `warpbeam:cancel`) · yedi diyalog `tezgah/` (ortak kabuk `BeamDialogShell`; Tak: `GET /loom-machines` + yuva 1..slots + yöntem/saat `mountTrackingRequired` kapısı panelde de; Sök: ölçülen kalan → fark ön-izahı; Tüket: kalanı aşamaz ön kapısı; Düzelt: `WARP_BEAM_ADJUST` kataloğu zorunlu; Bitir: artık üç yol (ölçülen · tartı · yok) + dispozisyon; Hurda: `scrap-preview` + `WARP_BEAM_SCRAP` zorunlu; Geri al: detaydan `undoCandidates` LIFO aynası — en yeni aktif durum olayı yalnız tezgah ailesindense aday, CONSUMED satırları tek tek) · kolonlar "Tezgah / yuva" + sarımdan farklıysa kalan · yedi durum rozeti (SHIPPED_OUT panelde de ilk kez — F1 aynası eksikti) · tek mutasyon `act` (aynı `settle`: toast + warnings + invalidate) · Genel Ayarlar › Devere / Levent +2 satır (`devereMountTracking` · `devereMountTrackingRequired`, `FLAG_MODULE` devereEnabled; PANEL_EXEMPT silindi, `test_feature_flag_contract` 78/0) · istasyon formu `producesWarpBeam` / `consumesWarpBeam` + makine formu `warpBeamSlots` (yalnız devere açıkken çizilir; iki sayfa allowlist satırı) · **backend yazma yüzeyi (H4):** `devere-station-fields.middleware` route'ta AÇIK doğrulama (yuva 0..32 tam sayı, yetenek boolean, diğer alanlar dokunulmaz; istasyon TÜRÜ kuralı yazılmadı — ölçülecek) + swagger · `GET /warp-beams/loom-machines`. Ölçüm: `test_station_devere_fields` 14/0 (iki sonda) · `beam-undo.test` 8/0 (iki sonda) · Electron vitest 229 (WarpBeams · GeneralSettings · Stations · Machines) · `settings-groups.test` devere üç satır · tsc/eslint temiz · sürüm maddesi panel 2026-09-13 turu (`check-surum-notlari --panel=1.3.2 --tablet=1.0.7` 21/0).
- **E3 TABLET İNDİ — 2026-09-15 (6e; taban E2 origin sha'sı):** backend `tablet-context` +`loomMachines[{id,code,name,stationName,warpBeamSlots}]` (allowlist `map`, `test_warp_beam_tablet §2b` ALLOW +1) + `mountTracking` + `mountTrackingRequired` (sunucudan; §5 satır-yok false ölçüldü) · mobil `FeatureFlags.devereMountTracking` aynası (DEFAULT false) + `useDevereMountTracking()` (devere modülüne BAĞLI) · `warpBeam.service` +mount/dismount/consume/exhaust/mountedOnMachine (şemalar birebir) · saf `tezgahPayload` (Tak/Sök/Tüket/Bitir formları, doğrulama, yük, parmak izi) · Levent Sarım ekranı üçüncü sekme "Tezgahta" (`MountedTab`: MOUNTED makine·yuva rozeti, READY'ye TAK; `MountModal`: loom makine PickerModal + yuva numpad + yöntem segment + sayaç; `useMountForm` token deneme başına, hata sınıfları +Faz 3 kodları) · Tezgah ekranı liste bölmesine `BeamPanel` (`useBeamPanel`: `GET /warp-beams/mounted/:machineId`, Sök/Tüket/Bitir modalları; tartı kaynağı tablette YOK, panelde) · tüketim token'lı, söküm/bitiş DB'de tek · panel `statusMeta` yedeği "Bilinmeyen durum (ham ad)" (1e hükmü, 5e bulgusu). Ölçüm: `tezgahPayload.test` 9/0 (üç sonda) · mobil jest 983/983 · `test_warp_beam_tablet` 40/0 · tsc/eslint temiz · sürüm maddesi TABLET 2026-09-13 turu (+ "eski istemci ne yapar" cümleleri panel/tablet; check 21/0). **Faz 3 E1–E3 TAMAM** — `devere.mountTracking` açılabilir (önce panel + tablet güncellenir).

## 14 · Faz 4 — TOP TEZGAHTAN DOĞAR: `CONSUMED(rollId)` OTOMATİK — İNDİ 2026-09-15 (6e; hüküm 1e H1–H5)

- **Şema (`20260915030000_warp_beam_auto_consume`, idempotent):** `WarpBeamEvent.rollId` (FK `rolls` Restrict, index; `Roll.warpBeamEvents` geri bağ) · `WarpSpec.takeUpPct Decimal(5,2)?` + CHECK `warp_specs_take_up_pct_range` (0 ≤ x < 100). Kullanılan oran ayrı kolon DEĞİL: satırda `lengthM` (çözgü) + `fabricLengthM` (kumaş) — take-up türetilir.
- **Tetik:** KK1 doğuş tx'i (`createInitialEntry`, WEAVING + `doffEventId`) — `claimDoffForRollTx` → `roll.create` → depo defteri → `autoConsumeForRollTx`; uyarılar KK1 yanıtının `warnings`ına. Doff yazmaz (§3.8b); fason dokuma WEAVING (doff yok) satır yazmaz.
- **Hükümler:** H1 take-up'ın evi `WarpSpec` (kumasTeknik yer tutucu; NULL → ESTIMATED + uyarı) · H2 çok hat payı `fabric ÷ productionLineCount` VERİ, beyanlı yaklaşıklık (`MachineShiftLineStat` inince kardeş hat topları kendi payını düşer — bekçi §7 iki hat/iki top toplam = tam boy) · H3 kalan yetmezse kısmi + uyarı, KK1 engellenmez · H4 SCRAP'ta ters YAZILMAZ · H5 metre düzeltmesi BORÇ (dokuma.md).
- **Levent seçimi:** `beamsMountedDuring(doff.machineId, doffedAt, doffedAt)` — defterden, doff anı (bekçi §6: KK1 sökümden sonra gelse de doğru levent, READY leventte durum değişmez; claim from=to=status).
- **Ters yol:** `softDelete` CANCEL → `cancelConsumedForRollTx` (her aktif CONSUMED(rollId) → CONSUMED_CANCEL, rollId kopyalı, gerekçe iptal gerekçesi); restore → `reconsumeForRollTx` (yeni ileri satır). `defter-beyan` WarpBeamEvent yazarları +1.
- **Bayrak [PROFİL]:** `devere.autoConsume` DEFAULT false = bugün — ÖLÇÜLDÜ: satır yok → KK1 yanıtı `["success","data","message"]` bayt bayt; panel satırı devere kategorisinde (aynı sha, PANEL_EXEMPT yok); Electron `FeatureFlags`/`FLAG_MODULE`/`settings-groups.test` (dört satır).
- **Panel:** Çözgü Kartları formu "Take-up (%)" alanı (Zod 0–100, sunucu `assertInputs` + CHECK); levent olay DTO'su +`roll {id, barcode}`.
- **Ölçüm:** `test_warp_beam_auto_consume` 19/0 (dört sonda: okuyucu sabit true → §1a bayt bayt ❌ · take-up dalı → §0a/§2b/§5 ❌ · `mountedBeamsOnMachineTx` → §0d/§6 ❌ · SCRAP'ta da ters → §0c/§5d ❌) · `test_consistency §45` (+sql ikizi; ad-hoc ihlal sondası ısırdı) · `db_invariants` 225 · `defter_ters_yol` 275 · `warp_beam_mount` 47 (§0b yazar listesi +1) · `machine_doff_source` 32 · `feature_flag_contract` 78 · lifecycle/tablet/schema_drift/swagger yeşil. Tablet dokunuşu 0; minVersion HAYIR.
- **Uyarı yüzeyi (2026-09-15, 5e bulgusu; hüküm 1e (a)):** KK1 tablet + panel elle giriş `onSuccess` `res.warnings`ı aynen basar (`entryWarnings` · `buildEntryWarningToast`); mekanizma vardı, yüzeyi yoktu — H3 kısmi tüketim sessizdi. Sürüm maddesine "bağlı levent yoksa tüketim yazılmaz, uyarı verilir" (5e).
- **Açık (Faz 5+):** atkı sayacı → metre (`unitsPerCm`) tüketimi — "sayaç asla stok yazmaz" ilkesi gereği tüketim de topun ÖLÇÜLEN metresinden düşer, sayaçtan değil; `producedM` yalnız rapor. Tezgah veri çıkışı (#8) gelirse `lengthSource` LOOM_COUNTER olarak beyanlı ikinci kaynak.
