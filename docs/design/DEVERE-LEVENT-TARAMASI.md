...
> **Durum: ÖNERİ** — kod yok, migration yok. 2026-09-11 `SEKTOR-YOL-HARITASI.md` taramasının kapatmadığı boşluğu kapatır ("`WarpBeam` yaşam döngüsü hiç incelenmedi"). Kararları ajan verdi (ölçüt: ① sektör standardı ② ölçeğimiz ③ kod deseni) ve **itiraza açıktır**; kısa karar kaydı `docs/history/CLAUDE-NOT-ARSIVI.md` → "2026-09-11 — Devere/levent tasarım kararları".
>
...
- **Profiller** `taban()`/`acik()` ile üretilir, anahtar elle yazılmaz (`module-profiles.ts:94-113`; kural dosyasındaki "açıkça yazılır" cümlesi bayat). `tam` spread ile devere'yi **kendiliğinden açık** alır. `dokuma` profiline elle eklenir. `basit` (adnansahin), `standart` ve `perde` kapalı kalır.
...
|---|---|---|---|---|
| **1 — Levent doğar** (EN KÜÇÜK ANLAMLI ADIM) | `devere.enabled` + grandfathering · denye alanı · `WarpSpec` CRUD · levent planı · sarım kaydı (nominal kg ön-dolu, gerçekleşen kg + `kgSource`, kalem/depo bazında) · `WOUND`/`WOUND_CANCEL` (önizlemeli) · hazır levent listesi (çözgü kartına göre gruplu) · `Station.producesWarpBeam` · §4.9 sayım düzeltmeleri · panel ekranı | "İplik nereye gitti" ve "hangi leventler hazır" ilk kez cevaplanır; tartılan sarımda devere firesi kg görünür | 3 tablo · 2 enum değeri · 4 kolon · 9 şema-dışı nesne | — |
...
## 9 · Karar gerektiren çelişkiler (yöneticiye / kullanıcıya)

1. **"Rota şablonuna iki istasyon eklenir" ↔ devere adımından top geçmez** (§3.3, ÖLÇÜLDÜ). Öneri: Dokuma rotada ADIM ve giriş noktası (Faz 4, yeni tezgah çıkışı motoruyla); Devere istasyon KATALOĞUNDA (makine, oturum, yetenek), adım değil. Kural harfiyen korunacaksa çekirdek adım kodu değişir (seçenek A/B, ikisi de reddedildi). Kök `CLAUDE.md` ile `MODUL-BAYRAK-TASARIM.md §5.2`'deki "devere/çözgü/haşıl yeni mimari istemez — istasyona istasyon, rotaya adım" cümlesi **top rotası için doğru, levent için eksik**: levent kendi tablosunu ve defterini ister. Düzeltme kullanıcı kararıdır; bu belge CLAUDE.md'ye dokunmadı.
2. **Bayrak adı.** Yönetici "`dokuma.enabled`" diyor, repoda yok; dokuma tarafının yer tutucusu `tezgah.enabled` (monitörizasyon, üretime bağlı, yüzeysiz). Öneri: `tezgah.enabled` monitörizasyon olarak kalır; Faz 4'ün "top tezgahtan doğar" motoru `dokuma.enabled` (üretime bağlı) ile gelir. DB anahtarı kimliktir, var olanın anlamı genişletilmez. Devere tasarımı iki seçenekte de değişmez (bağımlılığı yalnız iplik).
3. **Grandfathering kural satırı** (`modul-bayrak.md:52` "sabit false yasak") yeni doğan modüle düz okunursa yanlış yasak üretir; kod ve bekçi yer tutucular için zaten tersini uyguluyor. "Dünkü davranışı olmayan (yeni doğan) modülde değer = false" diye daraltılmalı. `/karar-notu` işi.
4. **`Perde üreticisi` profili** iplik/devere/tezgah kapalı tanımlı. Hedef kitle kararı (perde **dokuma**) bu profili ya da yeni bir "Perde dokuma" profilini gerektirir. `tam` profili devere'yi otomatik açık alır; bu bir ürün kararıdır.
5. **Tül = dokuma mı raşel mi** (#21). Bursa'da tül perdenin bir kısmı raşel çözgülü örmeyle üretilir. Hedef kitle "perde dokuma" diye yazıldı; raşel üreticisi hedefse levent tüketen makine dokuma tezgahıyla sınırlanmamalı (tasarım buna hazır: `consumesWarpBeam` + `warpBeamSlots`).
