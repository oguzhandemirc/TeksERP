...
### 3.3 · Oturum: yeni `StationKind.WEAVING` — ⓪ BACKEND İNDİ (01, 2026-09-14)

> **İndi:** `WEAVING` enum değeri (migration `20260914010000_station_kind_weaving`, tek ifade, geri alınamaz) · `SESSIONABLE_STATION_KINDS` + `STATION_KIND_PERM.WEAVING = mobile:dokuma` + `MOBILE_SESSION_PERMS` · izin kataloğunda `mobile:dokuma` (SCREENLESS gerekçeli — ekran doğunca satır düşer; süpervizör şablonunda, operatör paketi tablet dilimiyle) · **yalnız makineyle** (istasyon-modu 400 `STATION_MACHINE_ONLY`; tezgah = makine) · **rota kapısı** `assertStationsRoutable` (400 `STATION_NOT_ROUTABLE`; `route.service.validateSteps` + `workorder.service.assertRouteRefsActive`) · aynalar: station adapter · swagger · Electron `enums.ts`/`audit-labels` (`STATION.kind` override: "Dokuma Tezgahı" ≠ RollEntrySource "Dokumadan İndi")/`station-colors`/`Stations/schema.ts` · **panel istasyon formu türü yalnız `dokuma.enabled` açıkken çizer** (`stationKindVisibility.ts`, mevcut değer korunur; 1e hükmü) · mobil `StationKind` union. **Tablet dilimine kalan (0c):** `SessionStationKind` + iki harita (`stationScreens.ts`) + `mobile:dokuma-geri-al` + SCREEN_CATALOG satırı. Bekçi `test_station_kind_weaving` 7/0, dört negatif sonda. minVersion HAYIR (ölçüldü: eski tablet tezgahta oturum açamaz, çökmez; eski panel etiketi undefined basar ama referans fabrikada WEAVING istasyonu yok — onu yeni panel açar); sürüm notu maddesi tablet+panel (ea).
...
### 3.8d · Çelişmeli doğrulama — A1–A6 kapanışı (`f8f3ac79`) ↔ §3.8b/§3.9 hükmü (2026-09-14, 47)

**Sonuç: mekanizma AYAKTA (8029 · replay uyarısı · iki pencere · P2028 · HTTP alanı · minVersion). Bir İHLAL ölçüldü ve aynı gün kapandı: hüküm (a)'nın "muayene istasyonu damgası → KABUL" yarısı koda inmemişti, masa KK1'in %99'u 409 alacaktı (D1 → `fa3f9d1f`, sonda ile teyit). Üç küçük kalem açık (D2–D4).**
Yöntem: klon DB'de 22 bekçi + `fa3f9d1f`/`ff221f7f` sonrası 14 bekçi yeniden (liste aşağıda) · dört sonda (`_sonda_47_is3.ts`, scratchpad'de, commit edilmez) · fabrika kopyasında salt-okunur sayım · bağımsız Opus okuyucu (KK1 bağı merceği; eşzamanlılık ve defter mercekleri ÖLÇÜLEMEDİ — 6 okuyucunun 5'i oturum limitine takıldı, o iki mercek yalnız 47'nin statik okuması + bekçiler + sondalarla kapatıldı).

#### Ölçüldü ✅

| ne | nasıl |
|---|---|
| 8029 `nextDoffCodeTx` tx'in İLK ifadesi; `withBarcodeRetry` her denemede yeni tx (kilit yeniden alınır), yalnız `code_key` P2002 kemeri; kilit anahtarı ve kod öneki aynı kaynaktan (`stamp.value` günlük önek); 25 paralel 25 tekil / 0 hata; elle tutulan kilit kaydı bloklar | bekçi §6b-1/2/3 · `advisory_lock_namespaces` 14/0 (uzay açılmadı) · statik: `openDoff` tx callback'inde `nextDoffCodeTx` ilk satır |
| replay `warnings` yeniden türetilir (sıralı + P2002 dalı); koşumlu-işsiz dördüncü kova | bekçi §1d/§1e/§2c |
| `DOFF_HAS_ROLLS` çare önermez, liste 20'de kırpılır SAYI kırpılmaz; P2028 → 409 `DOFF_LINK_IN_PROGRESS` | bekçi §5b'/§5e/§5f; `mapRevokeTimeout` yalnız `P2028`i eşler, başka hata olduğu gibi geçer |
| `revokeDoff` ayrı `FOR UPDATE` + claim ([ES-26]); iki pencere; kilit sırası KK1 tx-B `lockAgainstMergeTx` → doff satırı → barkod sayacı, `openDoff` yalnız 8029(doffEvent\|önek), iptal yalnız doff satırı — döngü yok | bekçi §5b ×2; statik |
| HTTP `initialEntrySchema.doffEventId` taşınır, `semiFinished` ile 400; controller yalnız `doffEventId` verilince `forcedEntrySource: WEAVING` — "doff'suz WEAVING topu HTTP'den yazılamaz" DOĞRU; şema `ZodEffects` oldu, `.shape/.extend` tüketicisi yok (repo grep: yalnız controller `.parse` + bekçi `.safeParse`) | bekçi §4f; okuyucu (b)/(c) |
| minVersion GEREKMEZ | panel `types/enums.ts` + `audit-labels.ts` + Rolls süzgeç seçeneği WEAVING; tablet union WEAVING; tablet KK1 listesi süzgeci `ff221f7f` ile `KK1_LIST_ENTRY_SOURCES` tek kaynağına geçti ve WEAVING'i içeriyor (görünürlük boşluğu KAPANDI); `mobil/src`te `doffEventId` gönderen yer yok — bağ yolu tablet dilimine kadar sahadan erişilemez (bilinçli) |
| **hüküm (a) — `fa3f9d1f` sonrası dört dal:** muayene (RAW_QC) makinesi damgalı KK1 → KABUL · aynı tezgah (WEAVING) damgası → KABUL · damgasız → KABUL · başka tezgah (WEAVING) damgası → 409 `DOFF_NOT_LINKABLE` | sonda P1a–P1d (`fa3f9d1f` öncesi P1a 409 veriyordu); bekçi §4c/§4c2/§4e dört dallı fikstür, negatif sonda ("tezgah mı" koşulu kaldırılınca §4c2 kırmızı) |
| FULL geri alma: açık kumaş ebeveyni 100 → kesim 40 + 30 + kalan 30; üç çocuğun `TAMBUR_CUT` girişi `TAMBUR_UNDO` ile bağlı terslendi, her çocuk net 0, ebeveyn 100 IN_PRODUCTION | sonda P3 (bekçi §2/§3/§7b yalnız SINGLE ölçüyor — hüküm dosyası §12) |

Bekçiler (klon DB, `origin/main` `c2635fc1`): `machine_doff_source` 31/0 → `fa3f9d1f` sonrası 32/0 · `station_kind_weaving` 7/0 · `mobil_enum_aynasi` 35/0 · `stock_ledger_production_entry` 10/0 · `stok_defteri_bag_olcumu` 33/0 → 39/0 · `defter_ters_yol` 185/0 → 186/0 · `advisory_lock_namespaces` 14/0 · `kk1_duplicate_guard` 24/0 · `semi_finished_entry` 16/0 · `token_replay_cancelled` 18/0 · `roll_entry_station` 13/0 · `machine_run` 41/0 · `swagger_spec` 12/0 · `route_auth_coverage` 15/0 · `stock_ledger_tambur_undo` 65/0 · `stock_ledger_transform` 28/0 · `rescue_stuck` 10/0 · `raw_tambur_cut` 15/0 · `tambur_undo` 58/0 · `consistency_derived` 25/0 · `audit_labels` 22/0 · `production_regime_gate` 40/0 · `stock_ledger_manual_move` 16/0 · `permission_catalog` 24/0 · `role_template` 21/0 (ikisi klonda `mobile:dokuma` uzlaştırması sonrası — ortam) · `screen_catalog` 37/0 · `migration_enum_add_value` 11/0 · `db_invariants` 190/0. ⚠️ Sonda dosyası `scripts/` altında dururken `test_defter_ters_yol §8a` onu "sınıfsız defter yazan script" sayıp kırmızı verir — sonda scratchpad'de tutulur, commit edilmez.

#### Çürüdü ❌ — kalemler

- **D1 · KAPANDI (`fa3f9d1f`, 01, aynı gün).** `f8f3ac79`'daki `claimDoffForRollTx` her dolu `createdMachineId`yi doff'un makinesiyle karşılaştırıyordu; §3.9 hükmü *"eşleşme YALNIZ KK1 cihazı bir tezgaha bağlıysa … damgası muayene istasyonunun olan masa KK1'de KABUL"* diyordu ve "tezgah mı" yüklemi şemada kurulamıyordu (`StationKind`ta tezgah türü yok). **Saha gerçeği (fabrika kopyası, son 60 gün, salt-okunur):** KK1 (`SUPPLIER_RECEIPT`) 2205 top, 2190'ı damgalı ve hepsi RAW_QC istasyonunun tek makinesi — damga kaynağı `getStampContext` (aktif çalışma oturumunun makinesi), makineli istasyonda istasyon-oturumu 400 ve mobilde oturum ZORUNLU ⇒ tablet KK1'de damga daima dolu, "damgasız → kabul" dalı yapısal olarak ERİŞİLEMEZ (okuyucu bulgusu). ⇒ masa KK1'den doff bağı her seferinde 409 olacaktı (sonda P1a, `f8f3ac79`: `409 DOFF_NOT_LINKABLE`); bekçi §4c/§4e göremiyordu (iki makine de PROCESS_QC, §4e yalnız `null`). **1e hükmü (2026-09-14):** eşleşme yalnız damgadaki makinenin istasyonu `StationKind.WEAVING` ise; diğer her damga masa KK1 → KABUL. `fa3f9d1f`: enum değeri (migration `20260914010000_station_kind_weaving`) + helper `station.kind !== WEAVING → return` + dört dallı fikstür (§4c tezgah-tezgah 409 · §4c2 muayene damgası KABUL · §4e damgasız KABUL) + negatif sonda; `dokuma.md` kural satırı ve §3.3 güncellendi. 47 sondası dört dalı `fa3f9d1f` üstünde teyit etti (yukarıda).
- **D2 · Replay damga uyarısını düşürüyor (küçük, açık).** `doffedAt` makul aralık (36 saat geri / 5 dk ileri) dışındayken ilk cevap *"…makul aralık dışında; sunucu saati kullanıldı"* uyarısını taşır, replay yalnız koşum uyarısını yeniden türetir (sonda P2: ilk=true, replay=false). Çevrimdışı yeniden gönderimde operatörün gördüğü tek cevap replay olduğu için "damga sunucu saatine düştü" bilgisi kaybolur — A.3 ile aynı sınıf, etkisi küçük (kayıt zaten sunucu saatiyle). `resolveReplay` bunu kayıttan türetemez (`doffedAt` düşürülmüş değer, beyan saklanmıyor). Kalem (01/1e): ya sözleşme "replay yalnız KOŞUM uyarısını taşır; damga uyarısı ilk cevaba özgüdür" diye DARALTILIR (47 önerisi, tek cümle §3.8b ② adım 1) ya da damga beyanı kolona alınır (şema; önerilmez).
- **D3 · KK1 karne sayacı WEAVING'i saymıyor (açık, `d9`/rapor dilimi).** `dashboard.service.ts` KK1 istasyon karnesi "bugün gelen ham top" sayacı `entrySource IN ('SUPPLIER_RECEIPT','MANUAL_ENTRY')` — WEAVING topu (ve `SEMI_FINISHED`) dışarıda; yorumu *"KK1Screen 'Son Kayıtlar' ile aynı kapsam"* der, oysa tablet süzgeci `ff221f7f` ile dört kaynaklı tek sabite geçti (`KK1_LIST_ENTRY_SOURCES`), backend'de eşdeğer tek kaynak yok. "Altıncı enum değeri" sınıfı sessiz boşluk: tezgahtan inen top KK1 karnesinde görünmez. Kalem: sayaç kapsamı tek kaynağa bağlanır (backend sabiti + tablet aynası bekçisi), §3.8b minVersion envanteri bu sayacı da sayar.
- **D4 · Bayat cümleler (küçük).** `schema.prisma` `Roll.doffEventId` şerhi *"Yazan uç P3'te YOK (şema-only)"* — uç `inventory.controller.ts` ile indi · `inventory.controller.ts` iki yorum *"düz `z.object`"* der, şema artık `superRefine` ile `ZodEffects` (davranış aynı: bilinmeyen anahtar sessizce düşer) · `dokuma.md` ⑤ tablosundaki "dört yeni kural satırı" fiilen 2 yeni + 2 genişletilmiş satır · `dokuma.md` bağ satırı "1e hükmü 2026-09-13" ↔ §3.9 "HÜKÜMLER 2026-09-14" tarih tutarsızlığı.

#### Ölçülemedi ⚠️

- Eşzamanlılık ve defter mercekleri bağımsız okuyucuda ÖLÇÜLEMEDİ (oturum limiti); 47'nin statik okuması + bekçiler + sondalar tek kaynak. Açık kalan sorular: `deriveRunWarnings` replay'de koşum o arada geri alınmışsa yine "GİRMİYOR" der (doğru yön, ölçülmedi) · `mapRevokeTimeout` iptal tx'inde kilit beklemesi dışında P2028 üretebilecek başka ifade yok (`diagnoseRevokeRefusal` tam liste okur — 1000 top üst sınır, ölçülmedi).
- HTTP uçtan uca `doffEventId` (Zod → controller → servis) canlı istekle ölçülmedi; şema + controller dalı statik, bekçi §4f.
...
#### K · Çelişmeli doğrulama — dilim İNDİ: tablet doff (`213b0416`) + KK1 doff seçici (`ebf17eff`) (2026-09-14, 47)

**Sonuç: dört kapı, "kapalıyken sıfır fark" tablet ayağı, kuyruk yok, SIMULATED beyanı, clientToken parmak izi, DOFF_HAS_ROLLS modalı ve hüküm (a) tablet ucu ÖLÇÜLDÜ ve ayakta. İki kalem gerçek (E1 backend kapısı · E2 liste satırında tezgah kimliği yok), üç küçük.**
Yöntem: klon DB'de backend bekçileri — `mobile_screen_permissions` 6/0 · `screen_catalog` 37/0 · `permission_catalog` 24/0 · `role_template` 21/0 · `route_auth_coverage` 15/0 · `swagger_spec` 12/0 · `dokuma_regime_gate` 36/0 · `machine_doff_source` 32/0 · `station_kind_weaving` 7/0 · `mobil_enum_aynasi` 35/0 · `identity_ledger` 4/0 · `kural_bekci_atfi` 8/0 · `production_regime_gate` 40/0 · **`loom_lists` 10/1 (E6)** · mobil jest 8 dosya 51/51 (`Dokuma/*` · `KK1/doffLink` · `useVisibleScreens` · `stationScreens` · `screenModules` · `kk1EntrySources`) + mobil `tsc` temiz · **HTTP sondası** (`_sonda_47_is4.ts`, scratchpad; izole sunucu `PORT=4147`, klon DB; üç kullanıcı: yalnız `mobile:dokuma` · `+mobile:dokuma-geri-al` · yalnız `mobile:kk1`; 13 ölçüm) · statik okuma. Bağımsız Opus okuyucu koşulmadı (1e: bekçi + sonda yeter).

**HTTP sondası (bayrak AÇIK):** U1 `POST /machine-doffs` 201, kod `DF+GGAAYY+NNNN`, koşumsuz uyarı 1 · aynı token replay 201 aynı id + "zaten" + uyarı taşıdı · U1 revoke 403 (yetenek izni yok) · U3 (`mobile:kk1`) `GET ?unlinked=true` makine VERİLMEDEN 200, indirme listede, `meta.total` · U3 `GET /machine-doffs` parametresiz 400 `MACHINE_ID_REQUIRED` · U3 KK1 `initial-entry` + `doffEventId` 201 → `entrySource=WEAVING`, bağ kuruldu, `createdMachineId` null (masa KK1, cihazsız istek) · `semiFinished` + `doffEventId` 400 · bağ sonrası indirme bağsız listeden düştü · U2 revoke toplu doff 409 `DOFF_HAS_ROLLS` `total=1`, barkod adıyla, mesaj çare önermiyor · U2 revoke topsuz doff 200. **Bayrak KAPALI:** U1 `POST /machine-doffs` 403 `MODULE_DISABLED` · U3 `GET ?unlinked=true` 403 `MODULE_DISABLED` · U3 KK1 `doffEventId`siz 201 (bugünkü davranış, sıfır fark) · **U3 KK1 + `doffEventId` 201 → WEAVING top doğdu ve bağlandı (E1).**

| ne | sonuç | nasıl |
|---|---|---|
| ① route+izin: `POST /machine-doffs` `loom:doff ∨ mobile:dokuma`; revoke `loom:doff-revoke ∨ mobile:dokuma-geri-al`; `GET /machine-doffs` + `mobile:kk1`; `GET /machine-runs` + `mobile:dokuma` | ✅ | `test_mobile_screen_permissions` 6/0 (revoke yetenek muafı) · sonda H1/H2/H3/H6 |
| ② `SCREEN_CATALOG` `Dokuma` (`modul: dokumaEnabled`, `mobile:dokuma`, capability `mobile:dokuma-geri-al`); `mobile:dokuma` muafı düştü, `loom:doff*` gerekçesi güncel | ✅ | `test_screen_catalog` 37/0 · `screenModules.test` |
| ③ kart/route: `SCREEN_MODULE.Dokuma`, navigator YALNIZ `visibleScreens`i kaydeder, oturum otomatik yönlendirmesi (`SCREEN_BY_STATION_KIND.WEAVING → Dokuma`) aynı listeye bakar ⇒ bayrak kapalıyken ekran ne kartta ne navigator'da ne yönlendirmede — **panelin D1 sınıfı boşluğu tablette YOK** | ✅ | `useVisibleScreens.test` ⭐ (dokuma kapalı → `[]`, data yok → Dokuma yok, üretim kapalı → dokuma da elenir) · `MainNavigator.tsx` statik |
| ④ bayrak: `FeatureFlags.dokumaEnabled` default false, `resolveMobileModuleState` etkin değer `production && dokuma`, `useDokumaEnabled` | ✅ | `screenModules.test` · `test_screen_catalog §3b` |
| kuyruk YOK: `networkMode:'always'`, `submit` çevrimdışı kilit + sebep (ağ/sunucu), `offline/mutations.ts`te doff kaydı yok | ✅ | statik + grep 0 |
| SIMULATED: `useDoffMeter` simüle cihazda `source:'SIMULATED'`, cihazdan `MACHINE`, elle/boş `OPERATOR`; `counterSource` her yükte | ✅ | `doffPayload.test` (11) |
| clientToken parmak izi = backend `resolveReplay` üçlüsü (makine · hat · parça); yalnız belirsiz hatada yapışır; 90 sn pencere; `CLIENT_TOKEN_COLLISION` modal + taze token | ✅ | `doffAttempt.test` (7) · sonda H7 |
| DOFF_HAS_ROLLS modalı: `classifyDoffFailure` `total ?? barcodes.length`, barkod adıyla; listede `rollCount > 0` satır kilitli ve düğmesiz | ✅ | `doffPayload.test` · sonda H6a |
| hüküm (a) tablet ucu: KK1 `unlinked=true` listesi makine vermeden, masa KK1 damgasız → bağ kabul; bayrak kapalıyken payload'a alan hiç girmez; yarı mamulde gizli/400 | ✅ | `doffLink.test` (5) · sonda H3/H4/H4b/H8c |
| oturum yükü `machine.productionLineCount` (ek alan, eski tablet yok sayar); hat seçici yalnız çok hatlı makinede | ✅ | `work-session.service.ts` SESSION_INCLUDE · `useDoffEntry` |

**Kalemler**

- **E1 · Backend: bayrak KAPALIYKEN KK1 ucu `doffEventId` kabul ediyor (0c/01 → 1e kararı).** `POST /api/rolls/initial-entry` `requireDokumaEnabled` arkasında değil (doğru — KK1 üretim modülünün ucu) ama `claimDoffForRollTx` bağ kurarken modül bayrağına bakmıyor: sonda H8d — `dokuma.enabled=false` iken `doffEventId` verilince 201, `entrySource=WEAVING`, bağ kuruldu. Tablet bunu göndermez (`doffLinkPayload` kapalıyken boş) ama "kapalı modül → 403 `MODULE_DISABLED`" değişmezi bu yolda BACKEND'de yok; senaryo: doff'lar açıkken doğmuş, modül kapatılmış, eski/tekrar gönderilen istek WEAVING top üretir. Öneri: `doffEventId` verildiyse controller (ya da helper) `readDokumaEnabled()` → kapalıysa 403 `MODULE_DISABLED` (`modul:"dokuma"`), bekçi `test_dokuma_regime_gate`e "KK1 bağ yolu bayrak kapalıyken 403" sondası. Alternatif (1e): tarihsel bağ meşru sayılır ve cümle sözleşmeye yazılır.
- **E2 · `DoffLinkPicker` satırı TEZGAHI söylemiyor (0c).** `listUnlinkedDoffs` `DOFF_SELECT` ile döner: `machineId` var, makine **kodu/adı yok**; `doffRowLabel(item)` `machineName`siz çağrılıyor ⇒ satır "`DF…` · saat · N parça · hat 1". Masa KK1 hükümce **tüm tezgahların** bağsız indirmelerini görür; çok tezgahlı fabrikada operatör hangi tezgahın indirmesini bağladığını ekrandan okuyamaz — yanlış bağ riski (bağ ileri damga, geri alınmaz; yanlış top iptal + yeniden giriş). Öneri: liste satırına `machine: { code, name }` (backend `DOFF_SELECT`e ilişki ya da liste-özel select) + `doffRowLabel(item, item.machine.code)`; `doffLink.test` satır etiketi testi zaten `machineName` kolunu ölçüyor, üretim yolu onu beslemiyor.
- **E3 · Bayat yorum + istemcide uydurulan uyarı (küçük, 0c).** `useDoffMutations.ts` *"Replay dönüşü `warnings` taşımıyor (§3.8c A.3)"* — 01 `f8f3ac79` ile replay uyarı taşıyor (sonda H7: replay `warnings=1`); satır altındaki yedek metin *"Koşum seçilmedi — …"* backend `WARN_RUN_MISSING` metniyle farklı ve backend koşumsuzda uyarıyı hep gönderdiği için fiilen ölü dal. Kalem: yorum + yedek düşer, `res.warnings ?? []`.
- **E4 · Geri alma çevrimdışı kilitli değil (küçük).** İNDİR `isOnline` ile kilitli ve sebebi anlatıyor; geri al düğmesi yalnız `canRevoke && !locked`; çevrimdışıyken istek hemen düşer ve genel toast basılır (kuyruk yok kuralı korunuyor, davranış tutarsız). Kalem: aynı kilit + sebep.
- **E6 · `test_loom_lists §5` origin'de KIRMIZI (0c `ebf17eff`; CI'a düşer).** §5 metin yüklemi `router.get("/", requireAnyPermission("loom:run", "loom:doff", ...MOBILE_DOKUMA)` deseninde `)`yi hemen bekler; `ebf17eff` doff GET'ine `, ...MOBILE_KK1` ekledi ⇒ `doff=false`. Davranış doğru (`mobile:dokuma` hâlâ guard'da, `test_mobile_screen_permissions` 6/0), kırmızı yüklemin kırılganlığı: iki terimli metin deseni (kapının dördüncü ölüm biçiminin tersi — burada sahte kırmızı). Kalem (0c/d9): desen `\.\.\.MOBILE_DOKUMA[^)]*\)` gibi genişletilir ya da guard dizisi bir sabitten okunur; 0c'nin commit mesajı bu bekçiyi koşmamış (`loom_lists` listede yok — `dokuma.md` Backend satırında var).
- **E5 · Replay tespiti mesaj metnine bağlı (küçük).** `doffResultFeedback` `/zaten/i` ile backend `REPLAY_MESSAGE`i tanıyor; metin değişirse alt başlık sessizce "kaydedildi"ye döner (veri etkilenmez). Kalem: backend cevabına `replay: true` alanı ya da `message` sabiti paylaşılır — tablet dilimi genişlerken.
