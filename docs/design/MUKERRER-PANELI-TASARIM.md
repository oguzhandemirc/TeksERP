...
| Birleştirme | Tüm referanslar yeniden bağlanır, kaynak pasif/yönlendirilmiş, tam audit; **geri alma yok** (SFDC/Dynamics'te de yok) | ✅ mevcut (tombstone + merge-map) + önizleme + onay kapısı; geri alma VAR (`master-data-unmerge.service`: satırlar operasyon defterinden geri yazılır, mezar taşı ÖNCE kalkar) |
