...
- **Karşı-fabrika:** üç mağazaya sevk eden perde/döşemelik müşterisi. **Mekanizma:** ② VERİ + arayüz (bayrağa bağlı şube seçici, havuz şubeye bloklanır). **Kırılma:** irsaliyede yanlış teslim noktası / tablette sevk tıkanır. **Olasılık:** YÜKSEK. ~~Kardeş: paketleme grubu tablette yok, "Hemen Sevk Et" havuzun TAMAMINI gönderir (`PaketlemeScreen.tsx:220,300`; backend `packingGroupId` gönderiyor, mobil okumuyor; guard yok).~~ **Kardeş KAPANDI 2026-09-15 (01):** tablet `packingGroupId`yi okur, grup şeridi + tek yüklem `sacksInSelection` (liste · okutma hedefi · sevk kümesi), bayrak kapalı/seçim yok → bayt bayt eski; `test_packing_group §16` + jest.
...
- **Uzak CI 2026-08-10'dan beri koşmuyor** (ölçüldü 2026-09-13, 1e, `gh`): son 10 koşum 10/10 FAILURE, 2–39 sn (gerçek paket ~6,5 dk); son başarılı 2026-08-10 09:50; sebep 6/6 job "recent account payments have failed or your spending limit needs to be increased" — job'lar hiç başlamıyor. ⇒ Push öncesi tek gerçek kapı yerel `npm test` (+ Electron vitest + mobil jest tam). "Arka durağı var" cümlesi de bir iddiadır ve ölçülür. **Aynı gün geri döndü:** 501/505, dört kırmızı (dağıtım 1e'de).
- **OTA imzalama sertifikası repoda yok** (ölçüldü 2026-09-13, d9 gerçek temiz klonla): `mobil/app.json`un işaret ettiği dosya `keystore/` altında, ignore'lu ⇒ temiz klonda imzalı OTA üretilemez, CI'daki mobil kırmızısı buydu; beyanlı atlama yazıldı, paketleme kapısına taşınması ayrı dilim. Aynı kök bu turda worktree'de de ısırdı (`update-feed-url.test.ts` `keystore/` sembolik bağı olmadan kırmızı) — imza malzemesinin repo dışında olması bilinçli, ama "temiz klon paket üretebilir" varsayımı yanlış.
...
- **Kapıların birbirinden bağımsız koştuğu varsayımı** (ölçüldü 2026-09-13, 1e): üç oturumun commit kapısı aynı anda koşarken ~48 sn'lik adım 94 sn sürdü ve zamanlamaya duyarlı bir test (`WorkOrderCompleteDialog.test.tsx`) sahte kırmızı verdi — tek başına 11/11. Paylaşımlı ağaçta yalnız dosyalar değil kapılar da birbirine girer. Kural: **yük altında düşen bir testin kırmızısı, tek başına koşturulmadan raporlanmaz**; simetriği sabah ölçülmüştü (yeşil de ortamın özelliği olabilir) — bir kapının her iki cevabı da ortamın fonksiyonudur.
Yedi madde, hepsi "var sanılan": bir korumanın varlığı, en az yokluğu kadar ölçülmelidir.
- **Envanter borcu (2026-09-13, üçüncü kez çıktı):** bazı bekçiler ön koşulunu ORTAMDAN bekliyor (seed fixture'ı `PATOS`/`MUS-001`/aktif Item) — bugün görülenler `test_goods_receipt`, `test_goods_receipt_invoice`, `test_iade_enhancements`, `test_input_rolls_directship`, `test_inventory_shipment_scope`; taze/6e-şablonlu DB'de kırmızı, kod değil. Kaç bekçinin aynı bağımlılığı taşıdığı sayılmadı — d9'un "bekçi × VERİ" listesinin kardeşi; d5 emsali `5999690c` (bekçi ön koşulunu kendi kurar).
