...
> Yedi bağımsız salt-okuma taraması: Electron main/preload (21 dosya tam) · finans aritmetiği · rapor içerikleri (22 rapor tam) · etiket/refakat kartı canvas'ı (34 dosya tam) · mobil offline kuyruğu (14 dosya tam) · desenle geçilmiş mobil ekranlar A (23 dosya, 8.9k satır) ve B (40 dosya, 14.6k satır). Yaklaşık 70 aday; **birleşik ilk 10'un her dosya:satırı kaynaktan elle doğrulandı**; alan içi ikinci kademeler ajan beyanıdır. Sıralama = kırılma × karşı-fabrika olasılığı. **RİSK listesidir**; hiçbir madde bu kurulumda tetiklendiği ölçülmüş değildir.
>
> İki gözlem: (a) `rows[0]`/"ilkini al" cihaz seçimi **iki kod tabanında aynı** (panel kantarı `useMachineScale.ts:84`, tablet kantarı `useMachinePeripherals.ts:62-74`; tabletin METRE dalı aynı gerekçeyle kaldırılmış, kantar dalı duruyor) — dünkü "aynı oran iki kod tabanı" gözleminin cihaz katmanındaki hâli. (b) Karar ②'nin kökü bu turda bulundu: **kalem kartı formu birimi tipten türetip EZİYOR** (aşağıda 9) — kopyalanacak KG kalem panelden yaratılamaz.
...
## İkinci kademe — alan başına en güçlüler (ajan beyanı, seçilmişleri elle doğrulandı ✓)
...
`mobil/src/types/models.ts:3` union `'1.KALITE'|'A1'|'2.KALITE'|'FIRE'` · `Depo/DepoScreen.tsx:759,764,267,457,490` · `Sevkiyat/SackContentsModal.tsx:108` · `HizliIsEmri/wizard/ScannedRollsModal.tsx:70` (`A1_STOCK → '2. KALİTE'`) · `reports/quality-scorecard:298-302,313` (`sortOrder[0]` = "1. kalite", `role` ile çelişir) · `reports/scrap-scorecard:186,341` (`status='SCRAP'` ↔ `targetStatus` fabrika ayarı). Etiket alanında yeni site bulunmadı.
