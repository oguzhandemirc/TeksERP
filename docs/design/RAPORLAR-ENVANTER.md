...
**Yöntem:** §0/§1'deki her sayı AYNI yöntemle (grep, kaynak komutu tabloda) yeniden ölçüldü; bir koşumun çıktısından kapsam iddiası türetilmedi (popülasyonu statik tarama sayar). Ölçülen ağaç: origin/main **`69ae8b88`** (ÜÇÜNCÜ koşum — #158 5e çoklu seçici (a) · #162 `dusenSatir` · #163/#164 5e levent/lot seçici + d9 cari/oran + d5 ekstre düğmesi sonrası; ikinci koşum `3bba3840`, ilk `792b364b` — koşumlar arası fark yalnız "kodda değil" satırlarıydı). Komutlar kökten koşulur; `R=Teks-Erp/src/routes/reports`, `E=Electron/src/pages/Reports`.

| Ölçü | Faz ÖNCESİ (§0/§1, `dc34c4e3`) | Faz SONRASI (`69ae8b88`) | Kaynak komut |
|---|---|---|---|
...
| Tarih ekseni tek bileşen (R4 `ReportDateFilter`) | dört ad, bileşen yok; her sayfa kendi seçicisini kuruyordu | **30 / 30** sayfa `ReportPageLayout reportKey=` üzerinden (11'i ayrıca doğrudan: dokuma 3 + finans 6 + kalite 2 — tek gün/kesit/ileri pencere ve eksen çubuğu kipleri); URL adları DEĞİŞMEDİ (K7: `dateFrom/dateTo` 19+ · `from/to` 2 · `factoryDay` 1 · `asOf` 1) — birleşme bileşende, sözleşmede değil | `grep -rl 'reportKey=' $E --include='*.tsx' \| grep -v _components \| wc -l` · `grep -rl ReportDateFilter …` |
...
| Seçici kaynağı (çıkışsız kapı sınıfı: liste uçları kendi iznini ister) | — | dokuma 3 `meta.leventler` · satış 5 + müşteri 2 + fason 1 + finans 5 = **13 / 13** `meta.secenekler` (finans: `cariId` · `accountId` · `belgeTipi` · `oran` — d9, `code` sorgu biçiminde, üçüncü şekil açılmadı); süzgeçten bağımsız, ≤200/eksen; süzgeçli istek ×2 beyanlı (WHERE'e inen süzgeçte) | `grep -c leventler $R/dokuma.report.routes.ts` · `grep -l secenekler $R/*.ts \| wc -l` (4 dosya, 13 uç) |
| Süzgecin kestiği satır beyanı (`suzgec.dusenSatir`) | — | dokuma 3 (R5b-b) · finans 4 (cash-book · statement · vat-summary · fx-diff; R5b-d) · satış/müşteri/fason 8 (R5b-c4) = **15 / 18** eksenli uç; yok: kalite 2 (`summary.rollCount` zaten süzülmüş evren) · aging 1 (kesit) | `grep -rl dusenSatir Teks-Erp/src/services/reports Teks-Erp/src/services/cari.service.ts` |
| PANELDE eksen seçicisi çizen yaprak (R5b-c(a)/(b), levent/lot, 5e) | 0 | **13 / 13** eksen beyan eden yaprak: sipariş 5 + müşteri 2 + fason 1 (`ReportAxisBar` + `ReportMultiSelect`, `meta.secenekler`den) · kalite 2 + dokuma 3 (levent seçici `meta.leventler` + `LotInput`); kapı `reportAxisCoverage.test.ts` (beyan ↔ `eksenler=` tutarlılığı). **Finans 5 yaprak `meta.secenekler`i OKUMUYOR (0 / 5)** — kendi süzgeç çubukları (aging'de cari ARAMA, seçici değil; beyanlı, `AgingFilterBar` başlığı) ⇒ sunucu kaynağı hazır, panel seçicisi yok: faz sonrası tek açık kalem | `grep -rl 'ReportAxisBar\|ReportMultiSelect\|LotInput' $E --include='*Page.tsx' \| wc -l` (13) · `grep -rl secenekler $E/Finance` (0) |
| Ekstre diyaloğu (`finance/statement`) kapı + düğme | diyalog kapısız | Aging'deki ekstre düğmesi `isReportOpen("finance/statement")` ile çizilir (d5, `58839ab1`) | `grep -n 'finance/statement' $E/Finance/AgingReportPage.tsx` |
...
| Bekçi (rapor fazı) | 1 (`test_reports`) | backend **8** (`test_rapor_strict` · `test_rapor_kapisi` · `test_rapor_katalogu` · `test_rapor_levent_ekseni` · `test_rapor_satis_ekseni` · `test_finans_rapor_eksenleri` · `test_report_day_boundary` · `test_reports`) · panel `_components` **9** vitest (+`ReportMultiSelect` · `LotInput` · `reportAxisCoverage`) + `lib/report-gate` · `ReportsHub.gate` · `_hooks/reportAxisFilters` | `ls Teks-Erp/scripts \| grep -c 'test_rapor_\|test_finans_rapor\|test_report'` |

**Kodda değil (üçüncü koşum, `69ae8b88`):** önceki üç kalem (5e çoklu seçici (a)/(b) · d9 finans cari/oran · d5 ekstre düğmesi) İNDİ ve tabloya girdi. Kalan TEK kalem: **finans yapraklarında `meta.secenekler` seçicisi** (sunucu 5/5 hazır, panel 0/5 okuyor; aging'in cari araması seçici DEĞİL — çıkışsız kapı gerekçesi aynı: `/api/finance/cari` `finance:read` ister). Sahibi/zamanı 1e kararı; ibare ölçüm tarihiyle kalır.
