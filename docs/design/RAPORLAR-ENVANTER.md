...
**Yöntem:** §0/§1'deki her sayı AYNI yöntemle (grep, kaynak komutu tabloda) yeniden ölçüldü; bir koşumun çıktısından kapsam iddiası türetilmedi (popülasyonu statik tarama sayar). Ölçülen ağaç: origin/main **`465d89b3`** (DÖRDÜNCÜ ve SON koşum — #166 5e finans seçicileri sonrası; öncekiler `792b364b` → `3bba3840` → `69ae8b88`, koşumlar arası fark yalnız "kodda değil" satırlarıydı). **Bu tablo Raporlar fazının kapanış kaydıdır.** Komutlar kökten koşulur; `R=Teks-Erp/src/routes/reports`, `E=Electron/src/pages/Reports`.

| Ölçü | Faz ÖNCESİ (§0/§1, `dc34c4e3`) | Faz SONRASI (`465d89b3`) | Kaynak komut |
|---|---|---|---|
...
| Tarih ekseni tek bileşen (R4 `ReportDateFilter`) | dört ad, bileşen yok; her sayfa kendi seçicisini kuruyordu | **30 / 30** sayfa `ReportPageLayout reportKey=` üzerinden (12'si ayrıca doğrudan: dokuma 3 + finans 7 + kalite 2 — tek gün/kesit/ileri pencere ve eksen çubuğu kipleri); URL adları DEĞİŞMEDİ (K7: `dateFrom/dateTo` 19+ · `from/to` 2 · `factoryDay` 1 · `asOf` 1) — birleşme bileşende, sözleşmede değil | `grep -rl 'reportKey=' $E --include='*.tsx' \| grep -v _components \| wc -l` · `grep -rl ReportDateFilter …` |
...
| PANELDE eksen seçicisi çizen yaprak (R5b-c(a)/(b), levent/lot, finans; 5e) | 0 | **18 / 18** eksen beyan eden yaprak: sipariş 5 + müşteri 2 + fason 1 (`ReportAxisBar` + `ReportMultiSelect`, `meta.secenekler`den) · kalite 2 + dokuma 3 (levent seçici `meta.leventler` + `LotInput`) · finans 5 (`meta.secenekler`: aging cari SEÇİCİ + cari ARAMA ayrıştı, cash-book hesap, ekstre belge tipi, fx-diff cari, KDV oran — KDV'de cari ekseni YOK, beyanlı: şema kabul etmiyor); kapı `reportAxisCoverage.test.ts` (beyan ↔ `eksenler=` tutarlılığı) | `grep -rl 'ReportAxisBar\|ReportMultiSelect\|LotInput' $E --include='*Page.tsx' \| wc -l` (15) · `grep -rl secenekler $E/Finance --include='*.tsx' \| wc -l` (8 dosya = 5 yaprak/diyalog + 2 çubuk + 1 test) |
...
**Kodda değil (dördüncü koşum, `465d89b3`): YOK.** Üçüncü koşumun tek kalemi (finans yapraklarında `meta.secenekler` seçicisi, 0/5) #166 ile 5/5 oldu; KDV'de cari ekseninin olmaması bilinçli ve şemada kapalı (`.strict()`, `cariId` yok). Fazın plan satırlarının (K1–K11, R0–R7) hepsinin karşılığı tabloda ölçülüdür.
