...
4. **`replace()` `targetPropertyIds` GÖNDERİLMEZSE** (2d, 1c ölçtü): çözülmüş liste STOCK'ta boş / ORDER'da sipariş satırından türer ve KİLİTSİZ mevcut hedefler `WO_REPLACE` ile damgalanır — eski "sessiz silme" yolu artık "sessiz damga"dır (iz durur, davranış aynı); kilitli hedef listeye daima birleştiği için kaybolmaz. `undefined = tüm küme` semantiği bilinçli korundu (Electron her zaman tam liste yollar).

**Uygulama sonrası düzeltmeler (2026-09-14, 1c doğrulaması):** G2 — Prisma `@default(now())` ifade başına üretir (PG `now()` değil), miras satırı çocuktan ms sonra doğuyordu ⇒ §3.1 donör yüklemi HEP BOŞTU; miras artık tek yazıcıdan (`inheritRollPropertiesTx`) `createdAt: çocuk.createdAt` ile yazılır, bekçi §8c POZİTİF dal (legacy simülasyonu, restored=1). G1 — `setRollPropertyValueTx` ilk yazım yarışı: kaybeden `skipDuplicates` ile yutulup "created" diyordu; artık count 0 → tx içinde yeniden oku → aynı değer noop, farklı 409 (bekçi §3b kapılı-örtüşme: tutulan tx + `pg_stat_activity` kilit kanıtı).
...
## 12. Çelişmeli doğrulama — Faz 1 + 2a–2e İNDİ (2026-09-14, 47; taban `origin/main` 29fac167)

**Sonuç: sürümleme mekanizması (damga · partial unique · `ACTIVE_*` tek kaynak · fark bazlı yazım · pinli claim · beyan DEFTER{DAMGA}) AYAKTA; `silen 0` ölçüldü; 42P10 çürütmesi doğru. İki İHLAL: G2 donör "doğum-anı" kuralı gerçek veride ÖLÜ (Faz 2b) · G1 ilk-yazım yarışında kaybeden değer sessizce düşüyor (`setRollPropertyValueTx`). Bir küçük not.**
Yöntem: klon DB'de 28 bekçi (`roll_property_revoke` 37/0 · `property_value_selection` 30/0 · `defter_ters_yol` 199/0 · `db_invariants` 190/0 · `roll_relabel` 23/0 · `relabel_context` 22/0 · `station_property_mode` 19/0 · `workorder_order_link` 61/0 · `kursun_bypass` 149/0 · `kursun_unassigned_close` 53/0 · `tambur_undo` 58/0 · `stock_ledger_tambur_undo` 65/0 · `fason_receipt_color_width` 34/0 · `fason_partial_receive` 61/0 · `fason_receive_cancel_rereceive` 16/0 · `fason_undo_transfer` 30/0 · `property_targetable` 22/0 · `wo_cancel_disposition` 38/0 · `kanban_card_projection` 14/0 · `timestamptz_contract` 12/0 · `quality_code_literal` 15/0 · `snapshot_kolonlari` 9/0 · `consistency_derived` 25/0 · `manual_roll_undo` 24/0 · `roll_edit_unified` 15/0 · `identity_ledger` 4/0 · `kural_bekci_atfi` 8/0 · `audit_labels` 22/0) + iki servis düzeyi sonda (scratchpad, commit edilmez: `_sonda_47_is6.ts` yarışlar/42P10 · `_sonda_47_is6b.ts` donör) + statik okuma. Bağımsız Opus okuyucu koşulmadı (1e: bekçi + sonda yeter).

| ne | sonuç | nasıl |
|---|---|---|
| ③a "silen 0": `src/` içinde iki modele `deleteMany`/`delete` yazan yol 0; §13d beklenen silme kümesi BOŞ (yeni silme sitesi beklenmedik → kırmızı); beyan `PIVOT_TICARI → DEFTER {DAMGA revokedAt}` iki borç kapandı | ✅ | `roll_property_revoke §13` · `defter_ters_yol` 199/0 (`eaa45d53`) |
| 42P10 çürütmesi: eski `upsert(rollId_propertyId)` partial unique altında ölür | ✅ doğru | sonda P3 (Prisma P2039); ham SQL `INSERT … ON CONFLICT ("rollId","propertyId")` → PG *"there is no unique or exclusion constraint matching the ON CONFLICT specification"* (42P10) |
| sürümleme yarışı: aktif satır varken iki paralel farklı değer → 1 × `versioned` + 1 × 409 `PROPERTY_VERSION_CONFLICT`, iki satır tek aktif | ✅ | sonda P2 |
| 2c `applyRollFlagSetTx`: çıkanlar okunan id'lere pinli damga (`count` ≠ → 409), girenler `createMany skipDuplicates` (bayrakta değer yok, sessiz kayıp zararsız), değişmeyen satır aynı id; Y6 claim koşulsuz (`updatedAt` pini) | ✅ | `roll_property_revoke §9` · `roll_relabel` 23/0 · statik |
| 2d: `replace` — `replaceClaim` altında taze aktif hedef + kilitli hedef listeye daima birleşir + kapsama yalnız DELTA + `WO_REPLACE` damgası; `updateTargetProperties` — `touchWorkOrderTx` tx'in ilk ifadesi, taze kilit, `WO_TARGET_UPDATE`; Y9 topta yalnız `(eski − yeni)` FLAG kümesi damgalanır, SEÇİM'e dokunmaz | ✅ | `roll_property_revoke §10` · `workorder_order_link` 61/0 · statik |
...
- **G2 · Donör "doğum-anı" kuralı gerçek veride ÖLÜ (Faz 2b, `a7dd52fb`) — orta/yüksek.** `birthPropertiesOf` yüklemi `rollProperty.createdAt <= çocuk.createdAt`; gerekçesi *"PG `now()` tx başlangıcıdır, aynı tx'te yazılan miras satırı çocukla aynı damgayı taşır"*. Öncül YANLIŞ: `createdAt @default(now())` değerini **Prisma istemcisi ifade başına** üretir (DB `DEFAULT CURRENT_TIMESTAMP` yalnız Prisma değer göndermezse devreye girer). Gerçek kesim yolunda (`cutOpenFabric`, çocuk `create` → ayrı `rollProperty.createMany`) ölçüldü: çocuk `…07.346Z`, miras satırı `…07.357Z` — **+11 ms**, yükleme giren 0/1 (sonda D1). Sonuç: donör kümesi her zaman BOŞ ⇒ özelliksiz (legacy) ebeveynin FULL/SINGLE_RESTORE geri kurulumu HİÇ satır döndürmez — `propsRestored=0`, `propsDonorMissing=true` (sonda D2: ebeveyn özelliği damgalanıp legacy taklidi yapıldı, çocuk doğumda bayrak devralmıştı → geri alma sonrası ebeveyn aktif özellik `[]`). "Uydurma" düzeltmesi "hiç dönmez"e dönüştü; §3.1'in amacı (doğum satırı döner, sonradan kazanılan dönmez) yarım karşılandı. **Bekçi kör:** §8 özellikli ebeveynde donör dalına hiç girmiyor (`propsRestored === 0` bekliyor), §8b yalnız NEGATİF'i ölçüyor (sonradan eklenen bayrak gelmedi) — her satır dışlanınca da yeşil. Negatif sonda ("kısıt kaldırıldı → §8b ❌") pozitif dalı göremez. Kalem: (a) doğum satırı ADIYLA işaretlenir — kesim/finalize miras kopyası `revokeReason`/yeni `origin` kolonu ya da `createdAt`i çocuğun `createdAt`ine EŞİTLEYEREK yazar (`createMany` data'sında `createdAt: child.createdAt` — şema değişmez, Prisma değeri kabul eder; en dar dilim) · (b) bekçiye POZİTİF dal: özelliksiz ebeveyn + doğumda bayrak devralmış çocuk → FULL → ebeveyn o bayrağı geri alır (`restored=1`), sonradan eklenen gelmez. Sahaya etkisi: legacy (migration öncesi silinmiş) ebeveynlerin geri kurulumu sessizce boş; migration sonrası ebeveynler kendi satırını taşıdığı için (Y2) donör dalına girmiyor — yani bugün etkilenen yalnız legacy vaka, ama o vaka için yazılmış tek yol bu.
- **G1 · İlk-yazım yarışında kaybeden değer sessizce düşüyor (`setRollPropertyValueTx`, Faz 2a) — orta.** Aktif satır YOKKEN iki tx aynı (top, özellik) için farklı değer yazar: A `createMany` + commit'i bekler, B `findFirst` null → `createMany skipDuplicates` partial unique'te BLOKLANIR, A commit edince `ON CONFLICT DO NOTHING` B'nin satırını yutar ve B yine `"created"` döner (sonda P1′: satır 1, aktif değer A'nın; B'nin 50GR'si kayıtsız). Claim'siz insert yolu "count===0 → 409" doktrininin dışında; Kurşun/KK2 yolu (`copyStationCapabilitiesToRoll`) bu helper'dan geçer (çevrimdışı replay + süpervizör düzeltmesi aynı ana denk gelebilir). Kalem: `createMany` sonucu `count === 0` ise aktif satır yeniden okunur — değer aynıysa `"noop"`, farklıysa sürümleme dalına düşülür (revoke pinli + insert) ya da 409 `PROPERTY_VERSION_CONFLICT`; bekçiye kapılı-örtüşme sondası (P1′ deseni).
- **Not (küçük) · `replace` `targetPropertyIds` gönderilmezken:** STOCK iş emrinde çözülmüş liste boş ⇒ kilitli olmayan tüm aktif hedefler `WO_REPLACE` ile damgalanır — plan "`undefined` semantiği değişmez" diyor, eski "sessiz silme" artık "sessiz damga" (izlenebilir, geri alınmaz). Panel tam liste yolluyor; alanı göndermeyen bir istemci için bu bilinçli mi, §7 API sözleşmesine tek cümle.

**Ölçülemedi:** canlıda CHOICE sürümlemesinin ilk sınavı (#7); HTTP katmanı canlı istekle (statik + bekçi).
