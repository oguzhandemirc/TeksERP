...
| K-5 | **Doğrudan sevkte tahsis opsiyonelliği — `test_consistency §19`un yapısal açığı.** `directShipSchema` `orderLineAllocations`ı `.optional()` tanımlıyor ve Electron modalı kullanıcıya "opsiyonel" diye sunuyor → `totalQty` doluyken tahsis toplamı 0 kalabiliyor ve §19 kırmızıya düşüyor. Bayraktan BAĞIMSIZ, BUGÜN de mümkün (fabrikada drift yok ama sebebi kapı değil ALIŞKANLIK: doğrudan sevk sayısı 0, sahada hep tahsis yazılıyor — ölçüldü). §19 bunu ölçmekte HAKLI: "sevk edilen mal hangi sipariş kalemine sayıldı" sorusu cevapsız kalıyor. **Tercih edilen çözüm: tahsisi ZORUNLU kılmak** (yeni bayrak DEĞİL — sözleşme düzeltmesi); §19'u daraltmak SON ÇARE. Etki analizi gerekiyor: Electron modalı bugün "opsiyonel" sunuyor, mobil yolu var mı ölçülmeli. | 2026-09-03 D2-A doğrulaması | Dilim 2'ye SOKULMADI: kapsamı büyütür ve bugün fabrikada drift yok. Dilim 3 ile ya da ayrı küçük paket. |
