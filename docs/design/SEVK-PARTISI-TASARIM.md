...
| K2 | Ad | **"Sevk Partisi"**; otomatik ad `SP-1, SP-2…` cari başına. Çıplak "Parti" YASAK (üretim partisi `Batch` ile çakışır). |
...
| K5 | Kapanış | **Elle kapat + otomatik bayrak** (`packing.lotAutoClose`). Elle kapatma açık çuval varken de İZİNLİDİR (çuvallar üye kalır, havuzdan sevk edilebilir ya da transfer edilir); kapalı parti her zaman yeniden açılabilir; storno çuvalı geri getirirse kapalı parti kendiliğinden açılır. |
...
- **Sevk partisi modu:** canlılık `status = OPEN`; boş parti YAŞAR; parti sırası (`seq`) carinin BÜTÜN partilerine bakar (kapalı dahil) → geri verilmez; çuval `packageNo` taşır.
...
- Sevk edilmiş çuval partide KALIR (bugünkü kural: sevkte `packingGroupId` temizlenmez); `packageNo` donar — claim'ler `shipmentId IS NULL` ister, sevk edilmiş çuvalın partisi/numarası değişmez. Storno çuvalı aynı numarayla aynı partiye döndürür; parti kapalıysa OTOMATİK yeniden açılır (operatör geri gelen malı görsün).
...
| `packing.lotAutoClose` | bool | false | Son açık çuval sevk edilince parti kendiliğinden kapanır. |
...
Aşağıdaki altı ayar yalnız `groupMode = sevk-partisi`yken okunur; grup modunda bayt bayt bugünkü davranış (negatif sondayla ölçülür).
...
| `POST /packing-groups/:id/close` · `/reopen` | Atomik claim `updateMany WHERE {id, status}`; kapatmada açık çuval varsa 409 değil İZİN (kullanıcı kararı K5: "3 çuval kaldı ama bitirdim"); yeniden açma yalnız CLOSED'dan. |
...
| `createShipment` / `addSacksToShipment` | `lotPartialDispatch=false` → sackIds ≠ partinin açık kümesi ise 400; `lotAutoClose` → son açık çuval gidince aynı tx'te CLOSED. |
...
Yetki: yeni kod `shipping:packing-lot` (parti aç/kapat/yeniden aç/sil/adlandır/transfer/numara ez); çuval içi işlemler mevcut `shipping:write`. Katalog koda, atama panele.
...
1. **Cari seç** (mevcut `SackEntryGate`) → **Parti listesi** (açık partiler; kapalıları göster süzgeci; kolonlar: ad · açık çuval · sevk edilen · m · kg · yaş; "Sevk Partisi Oluştur" · kapat/yeniden aç/adlandır/sil).
