...
> **İNDİ 2026-09-14 (B3, 01) — KISMİ ve sapmalı, hükümle:** `MachineSpec` yazma yüzeyi + gölge mod durum makinesi `services/machine-spec.service.ts` · `routes/machine-spec.routes.ts` (**`/api/machine-specs`, `requireDokumaEnabled` + `loom:spec-manage`** — `/api/tezgah/specs` + `requireTezgahEnabled` DEĞİL: ekransız modül kapısı `test_screen_catalog §10b`de kırmızı, kapı Faz 2 "Devreye Alma" ekranıyla doğar ve router o gün taşınır). Go-live üç şartın sırası **② → ① → ③** (ölçülebilirden ölçülemeyene): ② `SHADOW_TOO_SHORT` ölçülür; ① `PeripheralSignal` yokken FAIL-CLOSED `SIGNAL_NOT_ACCEPTED` ("Faz 2 ile açılır") ⇒ LIVE bugün erişilemez (beyanlı); ③ anomali kaynağı Faz 2. `signals/:id/accept` · `CHANNEL_CHANGE_REQUIRES_SHADOW` · S1/S3/S5/S6 Faz 2 (bekçi ⏭ beyanlı). `baselineRunHours` guard'ı bu yüzeyle doğdu (aşağıdaki (2) borcu KAPANDI). `SHADOW_MIN_SHIFTS = 15` sabit (`constants/loom-shift.ts`), Faz 2'de `tezgah.shadowMinShifts` bayrağına taşınır.
>
...
### 2.7b · Çelişmeli doğrulama — Faz 1b ELLE DURUŞ backend'i (`ece78b66`, 6e) (2026-09-14, 47)

**Sonuç: beş yazma yolu, beş claim'in 409'u, tek açık duruş seddi, replay, reclass from→to defteri + karşı kayıt, "geri alınmış sınıflanamaz", iki iznin üçlüsü ÖLÇÜLDÜ ve ayakta. Bir orta kalem (F1: iptal vardiyada AÇILIŞ kapısız), iki küçük-orta (F2 replay parmak izi yok · F3 reclass sonrası eski not), üç küçük.**
Yöntem: klon DB'de 18 bekçi — `machine_stop_manual` 33/0 · `defter_ters_yol` 191/0 · `role_template` 21/0 · `permission_catalog` 24/0 · `route_auth_coverage` 15/0 · `swagger_spec` 12/0 · `production_regime_gate` 40/0 · `dokuma_regime_gate` 36/0 · `screen_catalog` 37/0 · `audit_labels` 22/0 · `identity_ledger` 4/0 · `advisory_lock_namespaces` 14/0 · `db_invariants` 190/0 · `machine_run` 41/0 · `timestamptz_contract` 12/0 · `hard_delete_guard_coverage` 17/0 · `reason_preset_kind_parity` 43/0 · `loom_stop_ground` 3/0 — + servis düzeyi sonda (`_sonda_47_is5.ts`, scratchpad, commit edilmez; 10 ölçüm) + statik okuma. Bağımsız Opus okuyucu koşulmadı (1e: bekçi + sonda yeter).

| ne | sonuç | nasıl |
|---|---|---|
| tek açık duruş seddi: aynı makinede İKİ PARALEL açılış → 1 × 201 + 1 × 409 `STOP_ALREADY_OPEN`, açık = 1 (DB partial unique `machine_stops_one_open_per_machine_uq`, P2002 → 409) | ✅ | sonda Q1 · bekçi §2a |
| ilk sınıflandırma claim (`reasonCode IS NULL`): iki paralel sınıfla → 1 × 200 + 1 × 409 `STOP_ALREADY_CLASSIFIED`, karar tek | ✅ | sonda Q2 · bekçi §4c/§4d |
| reclass claim (`reasonCode = from`): paralel X→Y ve X→Z → 1 × 200 + 1 × 409 `STOP_RECLASS_STALE`, `MachineStopReclass` **1 satır** (aynı tx) | ✅ | sonda Q3 · bekçi §5a/§5c |
| kapa claim (ham SQL, `endedAt IS NULL ∧ revokedAt IS NULL ∧ startedAt ≤ endedAt`, `durationSec` DB'de): paralel kapa ×2 → 1 × 200 + 1 × 409 `STOP_ALREADY_CLOSED`, süre 3600 | ✅ | sonda Q4 · bekçi §3 |
| geri al claim (`revokedAt IS NULL`): paralel ×2 → 1 × 200 + 1 × 409 `STOP_ALREADY_REVOKED`; damga, satır durur; geri alınmış duruş sınıflanamaz/yeniden sınıflanamaz/kapanamaz (409 `STOP_REVOKED`); geri alma seddi boşaltır | ✅ | sonda Q5 · bekçi §7b–§7f |
| replay: aynı `(machineId, stopKey)` aynı satır (`machine_stops_key_uq` partial, `revokedAt IS NULL`); geri alınmış anahtar 409 `STOP_REVOKED`; P2002 dalı yeniden okur | ✅ | bekçi §2b/§7f; statik |
| reclass defteri: from→to (+ lossClass çifti + gerekçe + actedBy) aynı tx; karşı kayıt = aynı fonksiyon to→from İKİNCİ satır, ilki değişmez; olgular (startedAt/endedAt/durationSec/stopKey) değişmez; `lossClass` preset'ten KOPYA ve DONMUŞ | ✅ | bekçi §5b–§5e/§6 |
| mühür kapısı TEK fonksiyon `assertStopShiftWritableTx` — kapa · sınıfla · yeniden sınıfla · geri al dördü de çağırıyor; iptal vardiya → 409 `SHIFT_CANCELLED`; `MachineShiftStat.sealState` inince aynı yer | ✅ (dört yol) | statik (dört çağrı) · bekçi §8 |
| trigger yokluğu SINIRI: BEFORE DELETE trigger şemada yok (beyanlı, budayıcıyla doğar); `MachineStopReclass.stopEvent` FK `Restrict` — reclass'lı duruş silinemez, reclass'sız duruş `prisma.delete` ile silinebilir (kapı yok); `defter_ters_yol §8a` `scripts/` altındaki sınıfsız silen script'i yakalar | ✅ sınır beyanlı | şema + `hard_delete_guard_coverage` 17/0 |
| iki iznin üçlüsü: `loom:manual-entry` · `loom:classify` katalog + `WEB_PRODUCTION_SUPERVISOR` + `SCREENLESS_PERMISSIONS` gerekçeli; route `verifyToken → requireDokumaEnabled → izin` (`machine-stop.routes.ts`), liste iki iznin herhangi biriyle | ✅ | `permission_catalog` · `role_template` · `screen_catalog` · `route_auth_coverage` · `dokuma_regime_gate` (KAPILI listesine `machine-stop.routes` — `d2d6a970`) |
| audit tx DIŞINDA, best-effort; etiket `MACHINE_STOP_EVENT` (Electron `audit-labels`) | ✅ | `audit_labels` 22/0 |

**Kalemler (6e/01 · hüküm 1e)**

- **F1 · İptal edilmiş vardiyada AÇILIŞ kapısız (orta).** `resolveShiftInstanceId` yalnız `isCancelled: false` vardiyayı arar ⇒ iptal vardiyayı kapsayan `startedAt` ile açılan duruş **`shiftInstanceId = NULL`** olarak doğar, 409 yok (sonda H: `OK`, shift `-`). Kapı (`assertStopShiftWritableTx`) `shiftInstanceId` NULL'ken hiç sorulmaz ⇒ o duruş sonradan mühür/iptal sınırının **dışında** kalır (kapa/sınıfla/geri al hepsi geçer). Bekçi §8 bunu görmüyor (yorumu "açılış: açıldı" olabileceğini kabul ediyor; ölçüm satırı DB'ye elle kurulmuş kayıt üstünde). Hüküm gerekir: (a) iptal vardiyayı kapsayan başlangıçla açılış 409 `SHIFT_CANCELLED` (çözücü iptal vardiyayı da döner, kapı reddeder — "vardiya iptali ≠ mühür, ikisi de burada sorulur" cümlesine uygun) — 47 önerisi; (b) NULL bilinçli ("tahminle yazılmaz") ise sözleşmeye yazılır ve bekçi §8'e açılış dalı eklenir.
- **F2 · Replay parmak izi denetimi yok (küçük-orta).** `openManualStop` aynı `(machineId, stopKey)` için `startedAt`/`reasonCode`/`beamSlot` KARŞILAŞTIRMADAN eski satırı "zaten kayıtlı" diye döner (sonda A: farklı başlangıç + sebep → aynı id, 200). Doff/koşum emsali `assertReplayPayloadMatches` → 409 `CLIENT_TOKEN_COLLISION`. Panel formu token'ı yeniden kullanıp başka bir duruş gönderirse sessizce eski kayıt onaylanır. Kalem: kimlik alanları (makine · startedAt · beamSlot) `assertReplayPayloadMatches` ile; sebep kimlik değil (sınıflandırma sonradan değişir).
- **F3 · Reclass sonrası `reasonNote` eski kararın notu (küçük-orta).** `reclassifyStop` `reasonCode/lossClass/reasonSource/classifiedBy/At`i değiştirir, `reasonNote`a dokunmaz; reclass gerekçesi yalnız defter satırına (`reason`) gider (sonda F: not "ilk sebebin notu" yeni kararın üstünde). Okuyucu satırda yeni sebep + eski notu görür. Kalem: ya reclass `reasonNote`u temizler/yeniler (girdiye `reasonNote` eklenir), ya kolon "ilk sınıflandırma notu" diye belgelenir.
- **F4 · `beamSlot` yuva sayısına karşı doğrulanamıyor (küçük).** Zod 1..8; tasarım §2.7 *"`warpBeamSlots > 1` olan tezgahta hangi yuva sorulur, ≤1 ise alan çizilmez"* der ama `Machine`de yuva sayısı KOLONU YOK (grep 0) ⇒ 1 yuvalı tezgaha `beamSlot=7` 201 (sonda K). Şema penceresi (01/6e): kolon doğunca `resolveStopPreset` yanına yuva doğrulaması.
- **F5 · `requiresReason` şerhi ile yazıcı ayrışıyor (küçük).** Şema: *"SINIFLANDIRMA BORCU — kapanışta tek helper yazar"*; Faz 1b **açılışta** yazıyor (`preset === null`) ve sınıflandırınca **temizlemiyor** — kuyruk yüklemi (`requiresReason ∧ reasonCode IS NULL`) doğru çalışıyor, kolon "sebepsiz açıldı" anlamına kaydı. Şerh ya kod hizalanır.
- **F6 · `endSource` insan ayrımı taşımıyor (küçük).** Kapa `endSource='OPERATOR'` sabit (enum `SIGNAL · OPERATOR · WATCHDOG`); vardiya amiri kapatınca da OPERATOR; insan kaynağı `source`/`reasonSource` kolonlarında (SUPERVISOR). Rapor "kim kapattı" sorarsa `endSource` yetmez — bilinçli mi, şerh.
...
//     UNIQUE NULLS NOT DISTINCT ("statId","sealGeneration","reasonCode","beamSlotNull")
...
//   ⚠️ CANLI ŞEMA (2026-09-14, migration 20260914130000) iki kolon EKLEDİ ve Prisma
//   tarafındaki `@@unique` KALDIRILDI (tek sed ham SQL'de; ikisi yan yana iki yüklem olurdu):
//     `reasonLabel` VARCHAR(100) — katalog etiketi KOPYA, DONAR (d9 ②: katalog değişse
//       geçmiş rapor değişmez) · `beamSlotNull` BOOLEAN — "atanmamış" kovası AYRI satır
//       (§4 çıktı: beamSlot NULL duruşlar tahminle levende yazılmaz), unique'e girer.
