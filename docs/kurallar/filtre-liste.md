...
- **[ÇEKİRDEK]** ÇIPLAK SÜZGEÇ FAIL-CLOSED (kullanıcı bulgusu 2026-09-18): TANINAN bir süzgeç adı `filter[...]` sarmalı olmadan gelirse (`?status=OPEN`) liste sessizce süzmez — yanlış süzülmüş liste hiç süzülmemişten kötüdür ⇒ 400 `BARE_FILTER_PARAM` (mesaj doğru yazımı söyler, `details.hint`). Tek kapı `assertNoBareFilterParams(req, tanınan, çıplakSerbest)`: BaseService listeleri modelin skaler alanlarını tanır (`assertListQueryShape`), süzgeci ELLE okuyan uç kendi okuduğu adları verir (`PURCHASE_ORDER_FILTER_NAMES` — okuyucu ile kapı aynı liste); sözlük adları (`LIST_QUERY_RESERVED`: page · limit · cursor · withTotal…) ve tanınmayan adlar serbest (bilinmeyeni Prisma'ya düşürmez, `safeFilters` süzer). · bekçi: `test_bare_filter_param` (§1 saf · §2 BaseService · §3 alış siparişi; üç sonda) <sub>(6e, 1e; d9 sürücüsü bulgusu)</sub>
- **[PROFİL yüzey]** SEÇİCİ MODALI KALIBI (v3; tedarikçi 2026-09-16, ürün 2026-09-17): form kutusuna tıkla → doğrudan modal, tam liste kaydırılabilir, arama + TEK seçimli Radix `ui/select` süzgeçleri, süzme SUNUCUDA, TEK `useInfiniteQuery` (`listCursor` 50), `useInfiniteScroll` doğrudan, düz `ui/table`; yerleşik `<select>` · DataTable · pagination cast · liste için iki sorgu YASAK ve kaynak taramasıyla ölçülür; `loadAllForPicker` yalnız küçük kataloğa (sığmazsa o süzgeç gizlenir). Ürün modalı `h-[85vh]` sabit (süzünce boy değişmez); tetik "Ad — KOD". · bekçi: `ItemPickerModal.test.tsx` (11; iki sonda) + gerçek Electron ölçümü (fare + klavye, satır sayısı sunucu gerçeğiyle) <sub>(arşiv 2026-09-17)</sub>
...
- **[ÇEKİRDEK]** Topun bir statüye GELİŞ anı `createdAt` DEĞİLDİR (kurtarma/dispozisyon/fason kabulü/finalize eski topu bugün taşır). Giriş sekmeleri (Ham Stok, Yarı Mamul) `createdAt`, diğer sekmeler `updatedAt desc` (`rollTabDefaultSortBy`; index @@index([status, updatedAt])). Yeni 'topu şu statüye çek' yolu sıralama sözleşmesini de karşılar — operatör topu BULABİLMELİ. · bekçi: `Electron/src/pages/Operations/Rolls/service.test.ts (11) :158-162 'giriş sekmeleri createdAt, diğerleri updatedAt ile sıralanır' — RAW_STOCK ve SEMI_FINISHED createdAt, FINISHED_STOCK updatedAt` <sub>(CLAUDE.md:125)</sub>
...
- **[ÇEKİRDEK]** Kesim çocukları `createdMachineId` damgası alır (dört TAMBUR_SPLIT doğum yolu: cutWarehouseRoll · finalizeWarehouseCut · cutOpenFabric · finalizeOpenFabric) — yoksa 'Bu makine' süzgeci kesimleri hiç görmez; eski kesimler geriye doldurulmadı (entryStationId emsali). · bekçi: `Teks-Erp/scripts/test_tambur_cut_idempotency.ts (11; createdMachineId damgası kesim çocuklarına yazılır) + test_tambur_recent_output_filter.ts (12; 'Bu makine' süzgeci o damgadan okur)` <sub>(CLAUDE.md:56)</sub>
...
Backend: `test_bare_filter_param`, `test_item_allowed_filter`, `test_filter_multi_select`

İstemci: `Electron/src/components/forms/ItemPickerModal.test.tsx`
