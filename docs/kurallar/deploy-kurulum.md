...
- **[ÇEKİRDEK]** Bir migration, kendisinden SONRA doğan bir tabloya ALTER/INDEX/FK/INSERT yazamaz: deploy sırası dizin adının leksikografik sırasıdır ve temiz bir veritabanında böyle bir ifade `42P01` ile düşüp migration'ı FAILED bırakır. Parça parça uygulanmış geliştirici veritabanında bu HİÇ görünmez — ölçüm yeri BOŞ DB'dir. Düzeltme: ifadeyi tabloyu yaratan migration'dan SONRAKİ yeni bir migration'a taşı (`IF NOT EXISTS` ile idempotent); uygulanmış dosyayı düzenlemenin bedeli ölçüldü — `migrate deploy` ve `migrate status` checksum farkını umursamaz. · bekçi: `test_migration_order` (hızlı mandal) · CI servis konteyneri (taze `teks_ci` + `migrate deploy`)
