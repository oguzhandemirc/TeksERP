...
- **[ÇEKİRDEK]** PRODUCTION CANLI ve TEKİL DEĞİL: her canlı kurulumda gerçek veri korunur — migrate reset / reseed / toplu DELETE YASAK; migration geri alınamaz kabul edilir (rollback = yedekten restore). Toplu veri düzeltme script'i dry-run varsayılan, --apply öncesi etkilenen her kaydı somut listeler. · bekçi: `yok (script başına dry-run deseni; kur.ps1 [7/9] 'GERI ALINAMAZ' yorumu :48)` · Kapanır: `--apply` bayrağını argv'den okuyan her script'in (popülasyon **25**, ölçüldü 2026-09-13; `"--apply"` GEÇEN dosya 30 — aynı sayı değil) BAYRAKSIZ koşumda YAZMADIĞINI ölçen bir bekçi ısırdığında. ⚠️ Yüklem "audit çağrısı var mı" DEĞİL, ulaşılan YAZMA YOLLARI (servis kapanışı dâhil) — literal arama `setup-ticaret`i haksız yere ihlal gösterir. Sonda: bayraksız yazan bir script → kırmızı. <sub>(CLAUDE.md:119, CLAUDE.md:76)</sub>
