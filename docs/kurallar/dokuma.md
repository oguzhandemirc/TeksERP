...
- **[PROFİL]** `requireDokumaEnabled` / `dokuma.enabled` BU DİLİMDE YAZILMADI (ekransız modül kapısı `test_screen_catalog §10b`'de kırmızı verir; yüzey `requireProductionEnabled` ile kapılı) — **BORÇ:** ekran dilimiyle bayrak + adlandırılmış kapı + `SCREEN_CATALOG` birlikte doğar, `SCREENLESS_PERMISSIONS`taki iki `weavingorder:*` muafı o gün DÜŞER (kapı iki yönlü). <sub>(01 P4 gerekçesi; 1e 2026-09-13)</sub>
- **[PROFİL]** `cancelReason` serbest metin (zorunlu, ≤300), `ReasonPreset`e BAĞLANMADI — **BORÇ kapanma koşulu:** P2b-2 `ReasonPresetKind.MACHINE_STOP`u indirdiğinde dokuma işi iptalinin kendi `kind`ini hak edip etmediği KULLANIM ÖLÇÜLEREK sorulur (geri alınamaz enum değeri ihtiyaç ölçülmeden açılmaz). <sub>(1e hükmü 2026-09-13)</sub>
