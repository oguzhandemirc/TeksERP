...
> ✅ **P2b KOŞUM YAZMA YÜZEYİ İNDİ (2026-09-13, 6e):** `POST /api/machine-runs` · `/:id/close` · `/:id/revoke` — izin `loom:run` / `loom:run-revoke`, kapı `requireProductionEnabled` (dokuma/tezgah anahtarı ekransız ⇒ kapı değil). Ekran YOK; tablet dilimi `requireAnyPermission("loom:run", ...MOBILE)` biçimine geçirir.
...
- **[PROFİL]** `requireDokumaEnabled` / `dokuma.enabled` BU DİLİMDE YAZILMADI (ekransız modül kapısı `test_screen_catalog §10b`'de kırmızı verir; yüzey `requireProductionEnabled` ile kapılı) — **BORÇ:** **Kapanır:** `dokuma.enabled` bayrağı + `requireDokumaEnabled` + `SCREEN_CATALOG` dokuma girdisi AYNI commit'te indiğinde ve `SCREENLESS_PERMISSIONS`taki iki `weavingorder:*` muafı düştüğünde (`test_screen_catalog §10b` + muaf bayatlık kapısı kırmızıdan yeşile). **Çapa:** `SCREENLESS_PERMISSIONS` `weavingorder:read` girdisi. **Öncül:** ölçüldü (`test_screen_catalog` 33/0, `permissionsWithoutScreen()` boş). <sub>(01 P4 gerekçesi; 1e 2026-09-13)</sub>
