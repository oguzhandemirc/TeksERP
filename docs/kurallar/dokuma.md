...
- **`test_machine_doff_source`** (2026-09-13, P3b doff backend ile doğdu; 20 ölçüm) — §1 replay dört durum · §2 koşum bağı (koşumsuz → `warnings` kaybı söyler; başka makine 409) · §3 tx-A stok defterine satır yazmaz · §4 KK1 bağı üç red · §5 `DOFF_CANCEL` (iptal edilmiş topla da 409) · **§5b iki pencere** (tx-B `FOR UPDATE`; kanıt bloklanan çağrının sırası) · §6 kod biçimi. İlk koşumda gerçek kusur: revoke claim'i kilitte bloklanıp ESKİ snapshot'la yüklemi geçiyordu (EvalPlanQual alt sorguyu tazelemez) — kilit ayrı ifadede önce alındı. Üç negatif sonda.
