...
- **[ÇEKİRDEK]** Sürüm notu yazılmadan sürüm ÇIKMAZ: taslağı Claude yazar, kullanıcı ONAYLAR, onaysız paketlemeye geçilmez. Tek kaynak `surum-notlari.json` (operatör dili; kapsam panel|tablet|her-ikisi, 'sunucu' bilerek yok); panele+tablete gömülür. Kapı beklenen sürümü ARGÜMANDAN alır, not dosyası onu üretmez. **Kapı yayına giden HER yolda koşar** (2026-09-14): panel paketleme · OTA · APK derleme (`build-apk.mjs`, `--check` ucuz yolu dahil) · APK sahaya çıkış (`deploy/mobil-yayinla.mjs`, yükleme BAŞLAMADAN) · CI (`--kunyeden`: sürümler `Electron/package.json` + `mobil/app.json`tan, iş akışına sabit YAZILMAZ). Üç sonuçludur: yeşil · kırmızı · **ÖLÇÜLEMEDİ** (künye/sürüm okunamadı ya da bekçi çalıştırılamadı) ve üçüncüsü de DURDURUR — koşmayan kapı geçmiş kapı değildir. · bekçi: `scripts/check-surum-notlari.mjs (paketleme · OTA · APK derleme · APK yayın · CI --kunyeden)` <sub>(CLAUDE.md:182)</sub>
- **[ÇEKİRDEK]** Sürüm notu maddesi yayından ÖNCE KAYNAĞINA geri okutulur ve bu bir DİL denetimi değil KAPI denetimidir — her iddia `dosya:satır` ile eşleşir: yetki cümlesi EKRANIN izniyle yetinmez (ekran-içi YETENEK ayrı kapıdır ve ayrı cümle ister — `mobile:dokuma-geri-al` varsayılan pakette YOK) · "…-den türetilir" cümlesi okuyucusu grep'le gösterilene dek VAADDİR (`unitsPerCm` yazılıyordu, okuyan yoktu) · "şu da kaydedilir" cümlesi kaydı YAZAN satırın hangi yolda koştuğu ölçülmeden yazılamaz (aynı yardımcıyı çağıran iki yol aynı şeyi yaptığını kanıtlamaz) · tırnaklanan ekran/ayar adı koddan KOPYALANIR, hatırlanmaz (ad bir ARAMA ANAHTARIDIR) · "sonraki sürümde" diyen her cümle, yayın kapanmadan yayının KENDİ madde listesine karşı aranır (dilim aynı yayına inerse madde geri güncellenmez, kimse onu okumaz) · adı benzeyen iki ekranın SONUÇLARI farklıysa not ikisini de adıyla anar. · bekçi: `scripts/check-surum-notlari.mjs §8 ("sonraki sürüm" vaadi ↔ aynı yayında o yüzeyi duyuran madde) + §9 (tırnaklı ≤5 kelimelik her ad istemci/sunucu kaynağında BİREBİR geçer ya da beyanlı muaf; yazım kayışı ADIYLA bildirilir, muafiyet sınıfı kapalı küme, iki yönlü, kaynak okunamazsa ÖLÇÜLEMEDİ)` · Öncül: ölçüldü (2026-09-14: 72 maddenin 10'unda 6 kusur — üçü yetki/türetim, biri #58↔#60 aynı yayın içi çelişki; §8 bugün kapsam 0 ile yeşil, beş sonda ile doğrulandı) <sub>(arşiv:2026-09-14 sürüm notu geri-okuma turları)</sub>
...
- **[ÇEKİRDEK]** Kod imzalama AÇIK: OTA anahtarı ve APK mührü `mobil/keystore/` altında, git DIŞI, VPS'e GİTMEZ; imza geçersizse tablet paketi reddeder; sertifika APK'ya gömülü değilse istemci imzayı HİÇ kontrol etmez (build-apk durdurur). Anahtar kaybı = her tablette sil+kur. `codesigning:configure` tuzağı bekçili. · bekçi: `mobil/src/test/update-feed-url.test.ts ("güncelleme AÇIK ve kod imzalama yapılandırılmış": updates.enabled===true + codeSigningCertificate); build-apk.mjs uzaktan-güncelleme tutarlılık kapısı (kesik alan tamamlandı 2026-09-13)` <sub>(CLAUDE.md:19, CLAUDE.md:128, CLAUDE.md:93)</sub>
...
- **[ÇEKİRDEK]** YAYIN GÜNÜ KURULUM ADIMI — çuval etiketinden iç not: canlıda `npx tsx scripts/remove_sack_note_element.ts` ÖNCE dry-run koşulur ve **teşhis çıktısı saklanır** (hangi şablon/varyantta kaç eleman), sonra `--apply`; koşan KULLANICIDIR. Kaldırılan yalnız `LabelKind.SACK` şablon varyantlarındaki `bind === "sackNote"` elemanıdır — `Sack.notes` VERİSİ ve ekrandaki not alanı AYNEN kalır, yalnız müşteriye giden kâğıda basılmaz. Basılmış etiketler DEĞİŞMEZ ve `labelDirty` işaretlenmez (şablon düzenlemesi kaydı bayat işaretlemez — refakat kartındaki aynı asimetri); elde dolaşan eski etiketlerde not kalır. Bu bir kod hatası değil, bir KURULUM VERİSİ kararıydı: alan opsiyonel ve varsayılanı kapalıydı, stüdyodan sürüklenerek açılmıştı; bekçi soruyu sordurdu, kullanıcı 2026-09-15'te kaldırılmasına karar verdi. · bekçi: `Teks-Erp/scripts/test_sack_label.ts §0 (kurulumun SACK şablonu iç notu VARSAYILAN basmıyor — kaldırma sonrası yeşil kalmalı, bekçi değişmez)` <sub>(kullanıcı kararı 2026-09-15, prova bulgusu)</sub>
- **[ÇEKİRDEK]** YAYIN GÜNÜ KURULUM ADIMI — iş ortağı rol göçü: `migrate deploy`ten SONRA, panel/tablet paketinden ÖNCE `npx tsx scripts/migrate_partner_roles.ts` ÖNCE dry-run (çıktı SAKLANIR: hangi profil hangi kartla bağlanacak, hangi hesap taşınacak), sonra kullanıcı onayı, sonra `--apply`, sonunda **ikinci koşum 0 değişiklik** (idempotency kanıtı); koşan KULLANICIDIR. Ad çakışan profil OTOMATİK BAĞLANMAZ — öneri satırı olarak çıkar ve yalnız `--baglan` ile, kullanıcı onayıyla bağlanır; tombstone profile kart üretilmez. Fikstür dışı hedefte `--apply` `--canli-onay` ister, üretim adı bu bayrakla BİLE reddedilir. Runbook: `docs/ops/IS-ORTAGI-ROL-GOCU.md`. · bekçi: `scripts/test_migrate_partner_roles.ts §H1/§H2/§H3 (hedef kapıları), §8 (idempotent), §4/§7 (ad çakışması bağlamaz), §9 (--baglan onayla bağlar)` <sub>(kullanıcı kararı 2026-09-17; faz 1.3.2 penceresinde)</sub>
- **[ÇEKİRDEK]** SÜRÜM NOTUNDA TIRNAKLANAN HER METİN KODDAN KOPYALANIR, HATIRLANMAZ — tırnak bir İDDİADIR: "bu metin ekranda/sunucuda birebir var". Kapı iki kollu: kısa adlar (≤5 kelime) BİREBİR aranır; uzun cümleler NORMALİZE (küçük harf · noktalama/boşluk sadeleşmiş · JSX/HTML etiketleri atılmış) İÇERİLME ile aranır — uzun cümle kodda satır sarması ve şablonla yaşadığı için birebir arama kapıyı ilk gün susturulacak kadar yanlış-pozitif üretirdi. Bilerek özetleyen madde `PARAFRAZ` sınıfıyla ve gerekçesiyle beyan edilir; beyan ↔ madde İKİ YÖNLÜ ölçülür (ölü muafiyet de kırmızı). Doğuş: yayın öncesi geri-okuma turunda bulunan BEŞ kusurun BEŞİ de >5 kelimelik tırnaklarda yaşıyordu, yani kapı o turun bulgularını YAPISAL OLARAK göremiyordu; en ağırı, kodda hiç olmayan bir uyarı metniydi ve uydurma hâli kullanıcıya ne yapacağını söyleyen yarıyı düşürüyordu. · bekçi: `scripts/check-surum-notlari.mjs §9a/§9b (iki sonda: uydurma uzun alıntı · bayat PARAFRAZ beyanı)` <sub>(1e hükmü 2026-09-15, geri-okuma turu)</sub>
- **[ÇEKİRDEK]** YAYIN GÜNÜ **KOŞULMAYACAK** script: `scripts/fix_tambur_undo_full_asim.ts` — kullanıcı kararı "dokunma" (2026-09-15). Sonucu BEYANLIDIR: ilgili kısıt canlı kopyada `NOT VALID` kalır ve `test_db_invariants` o satırda KIRMIZI verir; bu kırmızı bir arıza değil, **beklenen durumdur** ve yayın kontrolünde öyle okunur. Kırmızıyı susturmak için kısıt VALIDATE edilmez, bekçi de değiştirilmez.
