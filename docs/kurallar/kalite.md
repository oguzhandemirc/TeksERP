...
- **[ÇEKİRDEK]** İstasyonlar sayfası (`ProductionStationsPage`) formun kaydettirdiği HER `StationKind` türünü listeler ve dışa aktarır — tür kümesi `visibleStationKinds.ts`te `stationFormSchema.shape.kind.options`tan TÜRETİLİR, sabit allowlist ve dışlama listesi YOK; tek gizleme modül bayrağı (`FLAG_GATED_STATION_KINDS`: WEAVING dokuma kapalıyken çizilmez, kayıt korunur, form mevcut değeri korur). Form seçeneğinde "Diğer" (OTHER) en sonda; Görev Türü seçilince yetenek kutuları `stationKindDefaults.ts` ÖNERİSİYLE gelir (kilit değil) — yalnız YENİ kayıtta, düzenlemede mevcut yetenekler ezilmez. · bekçi: `test_panel_station_kind_aynasi` (ayna iki yönlü + sayfa türetilmiş, iki sonda) · Electron `visibleStationKinds.test` · `stationKindDefaults.test` · `stationKindVisibility.test` <sub>(kullanıcı testi bulgusu #2, 2026-09-16; arşiv)</sub>
