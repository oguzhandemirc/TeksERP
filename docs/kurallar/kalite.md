...
- **[ÇEKİRDEK]** İstasyonlar sayfası (`ProductionStationsPage`) formun kaydettirdiği HER `StationKind` türünü listeler ve dışa aktarır — tür kümesi `visibleStationKinds.ts`te `stationFormSchema.shape.kind.options`tan TÜRETİLİR, sabit allowlist ve dışlama listesi YOK; tek gizleme modül bayrağı (`FLAG_GATED_STATION_KINDS`: WEAVING dokuma kapalıyken çizilmez, kayıt korunur, form mevcut değeri korur). Sayfanın LİSTE görünümü (varsayılan; kart ⇄ liste tercihi cihaza `localStorage` `stations.viewMode`, sunucu ayarı değil) ve dışa aktarım aynı düz satırlardan (`stationMachineRows.ts`: satır = makine, makinesiz istasyon tek satır) çizilir; satır aksiyonları kart tablosuyla tek bileşen (`MachineRowActions`). Form seçeneğinde "Diğer" (OTHER) en sonda; Görev Türü seçilince yetenek kutuları `stationKindDefaults.ts` ÖNERİSİYLE gelir (kilit değil) — yalnız YENİ kayıtta, düzenlemede mevcut yetenekler ezilmez. · bekçi: `test_panel_station_kind_aynasi` (ayna iki yönlü + sayfa türetilmiş, iki sonda) · Electron `visibleStationKinds.test` · `stationKindDefaults.test` · `stationKindVisibility.test` <sub>(kullanıcı testi bulgusu #2, 2026-09-16; arşiv)</sub>
- **[ÇEKİRDEK]** Tipsiz hata girişi kataloğun VARSAYILAN tipine (`DefectType.isDefault`, kurulumda en fazla bir — partial unique `defect_types_one_default`) düşer, varsayılan yoksa yazma 400; istemci listenin ilkini UYDURMAZ. Varsayılan değişimi tek tx (`setDefaultDefectTypeTx`), `POST /api/defect-types/:id/set-default` ya da form `isDefault:true`. · bekçi: `scripts/test_default_defect_type.ts (gerçek DB, tx geri alınır; negatif: varsayılansız katalogda tipsiz → 400)`
- **[ÇEKİRDEK]** 'Bu adım kalite kontrol yürütür mü' sorusunun TEK kaynağı `services/helpers/quality-station.helper.ts`; boğaz İKİZ — bellek-içi `stepCanApplyQuality(station)` + Prisma parçası `QUALITY_STATION_WHERE` AYNI kuralı söyler ve BİRLİKTE değişir. Yeni `kind === PROCESS_QC` karşılaştırması YAZILMAZ. · bekçi: `test_station_quality_capability §3 (ikiz eşdeğerliği, canlı DB) + §4 (karar noktasında kind===PROCESS_QC KALMADI, gerekçeli muaflar; kesik alan tamamlandı 2026-09-13)` <sub>(CLAUDE.md:100, CLAUDE.md:174)</sub>
- **[ÇEKİRDEK]** İstasyon yeteneği istasyonun KENDİ alanıdır: `Station.appliesColor` (default false) / `appliesProperty` (default true) / `appliesQuality` (default false); ortak yüklemler `stepCanApplyColor/Property(station, requiredCategory)` ve tek argümanlı `stepCanApplyQuality` — kategoride kalite karşılığı YOK (fason firma kalite notu yazmaz). · bekçi: `test_route_step_targets · test_station_capability · test_station_quality_capability` <sub>(CLAUDE.md:68, CLAUDE.md:100)</sub>
- **[ÇEKİRDEK]** Yetenek bayrağı PLANLAMA sorusunu yanıtlar ('bu adım verebilir mi'); fason KABULDE rengin/özelliğin FİİLEN uygulandığı hâlâ o sevkin `requiredCategory` bayrağından okunur (`subcontractor.service`). İkisi BİRLEŞTİRİLMEZ: istasyon bayrağı 'yapabilir', kategori bayrağı 'yaptı' der. · bekçi: `test_fason_receipt_color_width.ts B1/C — 'yaptı' yarısı: kabul requiredCategory.appliesColor'dan okur (ölçüldü 2026-09-13; eski alan 'bekçi adı BELİRSİZ' diyordu, arşiv:630 'kaynak taramasıyla kilitler' SÖZÜDÜR, tarama yok)` · Kapanır: `fason kabul yolunun istasyon yetenek bayrağını HİÇ okumadığını ölçen kaynak taraması (subcontractor.service içinde appliesColor/appliesWidth okuması yalnız requiredCategory üstünden; istasyon bayrağı üstünden 0) yazıldığında` · Öncül: ölçüldü <sub>(CLAUDE.md:68)</sub>
...
Backend: `test_default_defect_type`, `test_panel_station_kind_aynasi`, `test_finalize_last_step`⚠️, `test_helpers`, `test_iade_enhancements`, `test_label_element_condition`, `test_manual_move_backflush`, `test_manual_move_qc_reversal`, `test_null_quality_visibility`, `test_p2_kk2reopen`, `test_phase1_uretim_hardening`, `test_phase3_stok_hardening`, `test_phase6_reporterror_concurrency`, `test_produced_buckets`, `test_qc2_idempotency`⚠️, `test_quality_batch_flags`, `test_quality_code_literal`, `test_quality_role_catalog`, `test_quality_scorecard`, `test_scrap_grade_label`, `test_scrap_scorecard`, `test_station_quality_capability`, `test_wo_manual_complete`
