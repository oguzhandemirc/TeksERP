...
- **[ÇEKİRDEK]** Bir raporun TARİH SÖZLEŞMESİ katalogda beyan edilir (`tarih`: `aralik-iso` · `aralik-gun` · `tek-gun` · `kesit` · `ileri-pencere` · `yok`) ve panel bileşeni parametre adını oradan üretir — backend parametre adları değişmez. `ileri-pencere` ayrı bir değerdir çünkü aynı parametre adıyla TERS yöne bakar: geriye bakan bir varsayılanı vade takvimine uygulamak boş takvim çizer. · bekçi: `scripts/test_rapor_katalogu.ts §7` <sub>(arşiv 2026-09-15)</sub>
