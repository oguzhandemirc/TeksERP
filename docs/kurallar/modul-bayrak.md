...
- **[ÇEKİRDEK]** Kapalı modülün ekransız uçları da KAPI TAŞIR (`requireXEnabled`, `verifyToken`DAN SONRA ve o router'ın TÜM uçlarından ÖNCE) ve kapıyı taşıyan her `src/routes/*.routes.ts` rejim bekçisinin `KAPILI` listesinde ANILIR — listede anılmayan bir taşıyıcı kapısını kaybettiğinde hiçbir kontrol kırmızı vermez. · bekçi: `test_production_regime_gate` §2/§2c (kapı ve sıra) + §1e (kapsam: taşıyan ⊆ liste); dokuma üçlüsü için `test_dokuma_regime_gate` §6 <sub>(arşiv:2026-09-13)</sub>
...
- **[ÇEKİRDEK]** Tablette modül kapısı `useVisibleScreens` koşuludur: `constants/screenModules.ts` `SCREEN_MODULE` (ekran → modül; `SCREEN_CATALOG` mobil satırlarının `modul` aynası, bekçi İKİ YÖNLÜ birebirler) kapalı modülün ekranlarını kart VE navigator listesinden AYNI anda düşürür; varsayılan yön alan başına backend okuyucusunun satır-yok değeridir (`DEFAULT_FEATURE_FLAGS.productionEnabled = true`; yeni modül `false`) — bayrak yüklenene dek liste bugünkünün birebir aynısıdır. Kapalı modülde kart yok ⇒ istek yok; `MODULE_DISABLED` 403'ünün özel yüzü hâlâ yoktur (jenerik toast, kabul — yüzey kapalı). · bekçi: `useVisibleScreens.test.ts` (kapalı → 0 · açık → birebir · okunamadı → satır-yok yönü) + `screenModules.test.ts` + `test_screen_catalog §3b` (ayna, negatif sonda 1/1) <sub>(P1 açık maddesi 2026-09-14'te kapandı, 0c; arşiv:2026-09-14)</sub>
