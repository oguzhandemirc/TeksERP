...
- **[ÇEKİRDEK]** Token ön-okumasıyla insert arasında KAZANANIN yazdığını okuyan bir iş kuralı varsa (ör. "zaten aktif bordroda") token okuması `lockClientTokenTx` (8036, `hashtext(token)`) altında yapılır — kilitsiz okumada kaybeden deneme token'ı kaçırır ve replay yerine iş kuralı 409'u alır, panel token'ı bırakıp aynı denemeden ikinci kaydı kestirir. · bekçi: `Teks-Erp/scripts/test_cek_bordro_taslak_token.ts` §4 ③c <sub>(arşiv:2026-09-26)</sub>
