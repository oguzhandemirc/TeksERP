...
- **[ÇEKİRDEK]** Kurşun tableti bayrak açıkken salt-okunurdur ama YALNIZ dağıtıma UYGUN adımda — kör bayrak kilidi Tambur'suz rotada çıkmaz üretir. Tek kapı `assertKursunTabletMayWrite`, beş tablet yazma yolunu da kapsar; guard UI'ya güvenmez (offline kuyruk bayrak çevrildikten sonra flush edilir). · bekçi: `scripts/test_kursun_regime_lock.ts (21 kontrol: bayrak kapalı yazar; bayrak açık + uygun adım 409 ve salt-okunur bandı dolu; uygun olmayan adım yazmaya devam eder — çıkmaz bekçisi; dağıtılmış adımda atama mesajı önce; gerçek giriş noktaları completeQc2/reportError/finishStep 409 + yan etki yok; PROCESS_QC olmayan adımda guard karışmaz; negatif sonda: rejim kilidi kaldırılınca 7, uygunluk süzgeci kaldırılınca 2, bant null'lanınca 2 kırmızı). Beş yoldan üçü testte koşar; açık kumaş açma ve kurşun bitirme yolları yalnız kaynakta guard'lı` <sub>(CLAUDE.md:51 · kaynak: kursun-bypass-eligibility.helper.ts:262-284 · çağrı siteleri kursun-qc.service.ts:461/630/827/884 + inventory.service.ts:4961/5504 · test:285-445 · arşiv:406, ölçüldü 2026-09-13)</sub>
...
- **[PROFİL]** `production.kursunBypassEnabled` REJİM anahtarıdır, görünürlük anahtarı DEĞİL: karo ona bağlanmaz. Karo izinle (`quality:write` ∨ `workorder:distribute`; mobilde `mobile:kursun-dagitim`) ve MODÜL kapısıyla (`production.enabled` → `isKursunPlanningVisible`, ikizi `requireProductionEnabled`) süzülür. · bekçi: `Electron/src/pages/Operations/production-regime.test.ts (saf yüklem isKursunPlanningVisible: modül kapalı false / açık true / başka alana bakmaz; karo yüklemi aynı fonksiyon nesnesi; permissionAny quality:write + workorder:distribute korunur) + tile-visibility.test.ts (Kurşun Planlama TEK karo, kapısı üretim modülü, kursun bayrağına bağlı değil; palet girişi karoyla aynı yüklemi taşır)` <sub>(CLAUDE.md:51 · kaynak: production-regime.test.ts:25-67 · tile-visibility.test.ts:59-92, :296-306 · production-regime.ts:62-64 · tile-config.ts:373-381 · backend ikizi kursun-bypass.routes.ts:23-49, ölçüldü 2026-09-13)</sub>
...
- **[PROFİL]** Kurşun sırası + kurşun dağıtımı TEK ekranda kalır (`/operations/kursun-dagitim`); eski `operations/kursun-queue` adresi oraya yönlendirilir, ikinci bir kurşun ekranı açılmaz. · bekçi: `Electron/src/pages/Operations/tile-visibility.test.ts:60 ve :305 (kursun-queue karosu ve ops:kursun-queue palet girişi YOK) + components/layout/command-entries.test.ts:42 (EXEMPT: /operations/kursun-queue eski adres, palet girişi beklenmez); yönlendirmenin HEDEFİ yalnız kaynakta, testte assert edilmiyor` <sub>(CLAUDE.md:51 · kaynak: content-routes.tsx:851-855 Navigate → /operations/kursun-dagitim · command-entries.test.ts:42, :52-55, ölçüldü 2026-09-13)</sub>
