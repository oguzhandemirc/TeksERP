...
- **[ÇEKİRDEK]** 'Adım renk/özellik verebilir mi' TEK YÜKLEM `stepCanApplyColor`/`stepCanApplyProperty` (istasyon bayrağı VEYA adımın fason hizmeti). Rota hedefi, WO kapsama, renk kilidi, taşıma, split, kursunFinish, send-to-dye ONU kullanır; tekil `requiredCategory.appliesColor` / `kind===` yazımı YASAK. · bekçi: `test_station_capability_flags.ts §3 (ortak yüklem: istasyon verir / kategori verir / ikisi de vermez / özellik simetrik), §4-5 (rota adımı: renk vermeyen iç istasyona red, verene kabul), §6-7 (yüklem üzerinden WO guard, dolaylı) + test_wo_target_color_guard.ts §4 (rota kapsaması: changeTargetColor ve update PATCH uyarır, boyalı rota uyarmaz) ve §8 (kursunFinish renk veren iç adımda hedef rengi yazar, vermeyen yazmaz) + test_tambur_send_to_dye.ts §2 (iç boyahane kanonik yüklemle bulunur). Tekil yazım YASAĞI için kaynak tarayıcı YOK; §8 kaynak taraması yasağı değil fason kabul MUAFİYETİNİ ölçer` <sub>(CLAUDE.md:68 · kaynak: step-capability.helper.ts:45/52 · sekiz tüketici (route.service:354/362 · workorder.service:224/231 · workorder-locks.helper:165 · workorder-manual-move.service:188 · workorder-split.service:111 · inventory.service:5471 · tambur-manual.service:698 · workorder-target-color.helper:221) · test_station_capability_flags.ts:128-180 · test_wo_target_color_guard.ts:292-316, :464-513 · test_tambur_send_to_dye.ts:186-198, ölçüldü 2026-09-13)</sub>
...
- **[ÇEKİRDEK]** Kalite = istasyon YETENEĞİ, boğaz İKİZ: `stepCanApplyQuality(station)` (bellek içi) + `QUALITY_STATION_WHERE` (Prisma, `satisfies`; spread'e ikinci `OR` eklenmez); saf yüklem WHERE'e GİREMEZ (atomik claim bozulur); ikisi birlikte değişir; yeni `kind===PROCESS_QC` YAZMA. · bekçi: `test_station_quality_capability.ts (§1 zemin: yüklem ve where parçası yayıldı; §2 yüklem doğruluk tablosu, Faz A köprüsü dahil; §3 canlı DB'de where ile yüklem AYNI kümeyi verir; §4 karar noktasında PROCESS_QC literali kalmadı — sayım bazlı gerekçeli muaf, ölü muaf iki yönlü kırmızı; §5 STEP_QUALITY_SELECT dosya başına taban sayım; §6 satisfies + spread + tek OR; §7 backfill ve seed; §8 AST: ikiz aynı dosyada yan yana, satisfies düğümü, helper dışında === PROCESS_QC yok; beş negatif sonda S1-S5)` <sub>(arşiv:2116 · kaynak: quality-station.helper.ts:68/88/97-99 · test:92-368 (28 kontrol; başlıktaki "Taban 22/0" bayat), ölçüldü 2026-09-13)</sub>
...
- **[ÇEKİRDEK]** Özellik-BAŞINA kapsama ('ŞU özelliği veren istasyon var mı') HÂLÂ SERT: create 400, replace/updateTargetProperties 409 (`locks.applicablePropertyIds`); `goods` muafiyeti bu yola UYGULANMAZ. 2026-08-27 'uyarı' kararı yalnız kategori düzeyi — 'hepsi uyarır' diye OKUMA. · bekçi: `yok — test_wo_route_coverage_goods.ts yalnız KATEGORİ düzeyini ölçer (renk veren adım yoksa uyarı, mal zaten öyle geliyorsa uyarı da yok); özellik-başına 400/409 yolunu hiçbir bekçi koşmuyor; test_helpers.ts locks bölümü applicablePropertyIds kümesini ölçer kapıyı değil; test_property_targetable.ts CHOICE/FLAG tip kapısıdır kapsama değil` <sub>(CLAUDE.md:129, CLAUDE.md:89 · kaynak: workorder.service.ts:952-968 goods muafiyeti · :982-999 create 400 · :5276-5283 replace 409 · :5955-5962 updateTargetProperties 409 · test_wo_route_coverage_goods.ts:127-190, ölçüldü 2026-09-13)</sub>
