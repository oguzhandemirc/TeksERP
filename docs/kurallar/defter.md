...
- **[ÇEKİRDEK]** Tamamlanmış sayımın fark fişi TEK BELGEDE stornolanır: yalnız deponun en son sayımı (LIFO); sayımın düşürdüğü top hâlâ onun iptaliyle duruyorsa rafına döner, elle geri alınmışsa YALNIZ defter karşılığı yazılır (statüsüne dokunulmaz), ters satırı zaten yazılmışsa yalnız sapma damgalanır — depo defterine `CANCEL_REVERSAL`, sapmaya `reversedAt`, iplikte net ters ADJUST; sayım satırı değişmez. · bekçi: `test_stock_count_reversal.ts` <sub>(arşiv:2026-09-11)</sub>
