...
- **[ÇEKİRDEK]** Aynı stok sorusuna bakan üç yüzey TEK rakam basar: Stok Karnesi ↔ Envanter rozeti ↔ Kanban kolonu. Kanban stok kolonu bu yüzden `currentStepId: null` koşulunu taşır ve kapsam tanımları Envanter `rollScope` zinciriyle BİREBİRDİR; ayrışırlarsa aynı soruya iki rakam doğar. · bekçi: `Teks-Erp/scripts/test_production_flow_columns.ts (kolon toplamı ↔ Envanter kapsamı birebir: 'Ham Stok' sekmesi yarı mamul HARİÇ · 'Yarı Mamul' sekmesi)` <sub>(CLAUDE.md:91)</sub>
...
- **[ÇEKİRDEK]** MAL KABULDE TOP SINIFI SATIR BAZLI ve ÜÇLÜDÜR (EK 7, kullanıcı kararı 2026-09-18): satır gövdesi `lineClass: RAW | SEMI_FINISHED | FINISHED` — ham `STOCK`, yarı mamul `STOCK` + `entrySource=SEMI_FINISHED` (elle yarı mamul girişiyle AYNI doğuş; Yarı Mamul sekmesi, `RAW_STOCK` birleşimi), bitmiş `WAREHOUSE`; yarı mamul DIŞARIDAN ALINABİLİR. Eski `rawStock: boolean` geriye dönük okunur (true→RAW, false→FINISHED); alan yoksa fişin `rawStockEntry`i (eski istemci = bugünkü davranış; panel artık göndermez, fiş kutusu KALKTI — yeni kumaş satırı bir önceki satırın sınıfını miras alır, ilk satır bitmiş); iplik satırında yok sayılır (kg defteri raf taşımaz). · bekçi: `test_goods_receipt` §L5 (sondalar: `receiptLineBirth` SEMI'de PURCHASE_RECEIPT · `resolveReceiptLineClass` rawStock'u yok sayar) <sub>(6e, 1e EK 7)</sub>
