...
28. İzole `git worktree`de gitignore'lu yapılandırma dosyaları EKSİKTİR (`Teks-Erp/.env` → bekçiler `JWT_SECRET zorunlu` der; `mobil/keystore/` → OTA imza sertifikası yok). Bekçi bir dosyanın VARLIĞINI ölçüyorsa oradaki kırmızı ÇEVRESELDİR: ölçmeden "benim değişikliğim kırdı" diye okunmaz, dosya ağaca bağlanır (`ln -s`). <sub>(ölçüm 2026-09-12: `mobil/src/test/update-feed-url.test.ts` izole ağaçta 1 kırmızı — `updates.codeSigningCertificate` dosyası yok; sertifika bağlanınca 10/10 yeşil)</sub>
29. İzole `git worktree` kuran oturum işi bitince KENDİ ağacını kaldırır: `git worktree remove --force <yol>` + `git worktree prune`, **VE o ağaçtan başlattığı süreçleri açık PID ile kapatır, kapandığını PORTLA doğrular** (`lsof -nP -iTCP:<port> -sTCP:LISTEN`) — ağacı kaldırmak süreci ÖLDÜRMEZ: silinmiş dizinde koşmaya devam eder ve ölçümü kirletir (2026-09-12: kaldırılmış bir ağacın sunucusu 4101'i saatlerce tuttu; hedef kapısı onu "başka veritabanına bakıyor" diye gerçek pozitif yakaladı). Başkasının ağacına ve süreçlerine DOKUNULMAZ; `prune` yalnız dizini zaten silinmiş kayıtları temizler, canlı ağaca zarar vermez.
29a. Worktree komut SIRASI: **sil → `prune` → `add`.** Dizin dururken `prune` hiçbir kaydı temizlemez; dizini `rm -rf` ile silip sonra aynı yola `add` denemek `fatal: '<yol>' is a missing but already registered worktree` ile düşer (ölçüldü 2026-09-12, üç kez ısırdı). `git worktree remove --force <yol>` ikisini birden yapar ve tercih edilen yoldur; elle silindiyse `prune` ZORUNLU. — atlanırsa: `add` sessizce düşer ve sebebi kayıt bayatlığı olduğu anlaşılmaz.
29b. İzole worktree'de **`prisma generate` KOŞULMAZ**: şemada generator `output` olmadığı için client `node_modules/.prisma/client`a yazılır ve worktree'ler o dizini ortak ağaca symlink eder (ölçüm 2026-09-12: symlink taşıyan altı ağaç) ⇒ o ağacın HENÜZ İNMEMİŞ şemasıyla ortak client ezilir, başka oturumların `typecheck`i ve bekçileri var olmayan modellere göre kırmızı ya da YANILTICI YEŞİL verir. Şema işi ana ağaçta yürütülür; `npm run prisma:generate` bunu mekanik olarak durdurur (kaçış `PRISMA_GENERATE_WORKTREE_ONAY=1`). · bekçi: `test_script_guards.ts §14` <sub>(ölçüm 2026-09-12: `git worktree list` 42 ağaç gösterdi, her biri 3818 dosyalık tam checkout; tek bir oturumun beş ağacı kaldırılınca 23'e indi — artığın yarısı tek oturuma aitti)</sub> — atlanırsa: disk ve `git status` maliyeti birikir, ve bitmiş bir işin ağacı canlı sanılıp içinde çalışılır.
