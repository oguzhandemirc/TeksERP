...
10. `pm2 start ecosystem.config.js` → `pm2 status` ile `online` teyit et, `pm2 logs tekserp-backend` ile açılış bannerını gör.
    ...
    - **Log rotasyonu pm2'de otomatik DEĞİL** — ama `kur.ps1` [8/9] adımında `pm2 install pm2-logrotate` + `max_size 10M` / `retain 14` / `compress true` kurar (2026-09-06'dan beri; her sürümde tekrarlanır, sahadaki eski kurulumlar ilk güncellemede düzelir). Elle kurulumda bu üç komut ZORUNLU — yoksa log dosyası sınırsız büyür (NSSM bunu kendisi yapıyordu; fabrikada 5 haftada 57 MB tek dosya ölçüldü).
...
43. **Cihaz onay / allowlist + makineye atama** (PairingCode KALDIRILDI):
    1. Tablet boot'ta UUID üretir + otomatik `POST /api/devices/announce` → backend Device tablosuna upsert. Elle Device girmek gerekmez. **Doğuş durumu bayrağa bağlıdır** (2026-09-04): `devicePairingRequired` AÇIK → `PENDING` · KAPALI → `APPROVED` (onay diye bir adım yok).
    2. Admin Electron → **Cihazlar sayfası** → tablet "Onay bekliyor" (yalnız bayrak AÇIKKEN).
    3. **"Onayla & Ata"** → makine seç (boş = atamasız) → status=APPROVED + machineId (**admin:settings izni**). Bayrak kapalıyken de bu adım **makine atamak için** kullanılır (cihaz zaten onaylı doğar).
    4. Tablet `/api/devices/status` poll'lar; cevap `pairingRequired` alanını DA taşır ve onay ekranı koşulu **`pairingRequired && status !== "APPROVED"`**tır (karar sunucuda — istemci iki ucu birleştirmez). `devicePairingRequired` default **KAPALI** → tablet beklemeden login olur.
       - ⚠️ **2026-09-04 öncesi bu vaat GERÇEKLEŞMİYORDU**: `announce` bayrağa bakmadan `PENDING` yaratıyor, istemci de PENDING'i "onay bekleniyor" diye okuyordu. Sahadaki 28 cihazın hepsi zaten APPROVED olduğu için görünmedi; **yeni kurulum** ve **arızalı tablet değişiminde** çıkardı. Yan etki: bayrak kapalı kurulumda `resolveDevice` yalnız APPROVED cihaza atıf döndüğü için tabletin `req.device`'ı hiç dolmuyordu → makine atfı VE **cihaza bağlı BT yazıcı** çözümü sessizce boşa düşüyordu. Artık APPROVED doğar; makine atfı için yine de makineye ATAMA gerekir (atanmamış cihazda `machineId` NULL → raporda makine kırılımı yok).
       - ⚠️ Bayrak **sonradan AÇILIRSA** otomatik onaylanmış cihazlar APPROVED **kalır** (ileriye dönük kapı; retroaktif düşürme vardiya ortasında tüm tabletleri kilitlerdi). İz audit'te: `DEVICE_AUTO_APPROVED`. Gözden geçirme yolu cihaz başına **"Onayı Geri Al"** (revoke). Bayrak kapalıyken public `announce` bir yönetici kararını **geri almaz** — mevcut PENDING/revoke edilmiş cihaz terfi etmez.
